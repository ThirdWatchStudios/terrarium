...
> `x=64`. Southeast, southwest, and northwest form one continuous solid wall mass; northeast remains open
> floor; and all four cardinal sockets remain exact. The complete pair is mirrored without a seam filter,
> companion SVG, stacked ingredient, or second cream owner, preserving the accepted Y-based light, reveal,
> arris, and contact-shadow hierarchy. The accepted ledger now stands at 27 direct / 19 derived /
...
- Accepted single-open northeast cross-junction law (owner acceptance, 2026-07-24): `mask_45` reuses the
  accepted `open_cross_filled_ne_se_sw` pair from `mask_33` through one plain whole-cell X mirror around
  `x=64`. Preserve four exact cardinal sockets, southeast/southwest/northwest as one continuous solid wall
