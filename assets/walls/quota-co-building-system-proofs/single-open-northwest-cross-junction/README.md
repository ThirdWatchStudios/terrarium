...
open to floor. Accepted `mask_25` and `mask_32` constrain the east and south
slabs, while accepted `mask_30` constrains the exposed northwest return. Those
pieces are geometry and material controls only: this source is one independently
authored flattened union, not stacked source art and not a Y mirror.

The two SVGs preserve the proof-layer split:

- `open_cross_filled_ne_se_sw-base.svg` owns one continuous lower structural
  mass.
- `open_cross_filled_ne_se_sw-upper.svg` owns the sole cream shell, the
  northwest reveal light, and the exposed northwest arris seam.

Coral and green are deliberately absent. The southeast and southwest frontage
crooks are buried by occupied neighbors, so adjoining accepted pieces remain
