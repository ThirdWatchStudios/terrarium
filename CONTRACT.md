...
**Outdoor ground — a DISTINCT kind (decision D2, amended).** The natural surfaces (grass / meadow / dirt), paved surfaces (asphalt / sidewalk / gravel), and ornamental water (`pond-water`) ship as ground, **not** interior floor. Water is never represented as a prop. They use the same flat, seamless 128-unit tile machinery (they *are* `FloorTemplate`s, rendered by `composeFloorTile`), but they are exported into their own `ground/<id>/` folder with a distinct **`kind:"ground"`** atlas that carries the ground **sort band (`-20000`, below the floor band)**. This is what lets the sim's `ImportGround` (its story A-S1) import ground as its own layer — drawn under the interior floor and everything else — and keep it **un-paintable as interior floor**. Directory parity with floors: the sim discovers ground the same way it discovers floors (iterate the folder, read each per-tile atlas + `ground.json`), so there is no separate manifest. Ground ships a re-tintable layer atlas too (`kind:"ground-layers"`). Ids: `ground-grass`, `ground-grass-b`, `ground-grass-c`, `ground-meadow`, `ground-meadow-b`, `ground-dirt`, `ground-asphalt`, `ground-sidewalk`, `ground-gravel`, `ground-pond-water` (three curated grass instances + two meadow instances break the macro cadence of an empty field). There is no Terrarium ground editor or ground entry in the prop-only `facility-catalog.json`; these code-owned defaults always ship, while the sim owns ground painting, pond digging, and water walkability.

**The natural/paved/water register split (D2 amendment).** D2 originally gave ALL ground the clinical-drain treatment. Reversed for natural ground and extended to ornamental water: under the clinical look, **grass / meadow / dirt / pond-water keep their saturated authored palettes** — the same Article VIII exemption people get (`applyClinicalLook`, `NATURAL_GROUND_TEMPLATE_IDS`, `WATER_TEMPLATE_IDS`). The felt-target is the game's premise made visible: the office replaces something alive, so every clinical export shows drained plan surfaces displacing vivid living ground. The **paved** surfaces (asphalt / sidewalk / gravel — and the parking-line/car props, which drain as ordinary props) are the focus-grouped output and drain with the building. Sim-side implication: `ImportGround` needs no change (palettes arrive baked in the atlas), but any sim-side runtime drain of the ground layer must honor the same split.

**Ground-edge transition overlays (`ground-overlays/`).** Where two ground kinds meet, the seam is softened by a transition overlay drawn on the **receiving** cell. Every overlay rides the **shared 47-blob autotile contract** — the exact wall-tileset shape: an 8×6 `tileset@Nx.png` whose 47 frames are named `mask_<i>` where `<i>` is the blob index, atlas `meta.autotile: "8-neighbor blob (47)"` (the wall importer's validation marker, matched verbatim), sliced and selected sim-side by the same machinery (`WallBlob.Index`). `grass-fringe` draws encroaching grass-family art on a non-grass receiver; its set bit means "the neighbour on that side is encroaching natural ground." `curb-edge` draws a poured-concrete lip on an asphalt/sidewalk/gravel receiver; its set bit means "the neighbour on that side is natural ground." `pond-shore` draws a soft earthen band **inside a pond-water receiver**; its set bit means "the neighbour on that side is non-water ground." All render at sort band **−19000** (above ground at −20000, below floors at −10000). Overlays are **code-owned and derived**, not authored: `deriveGroundOverlays` builds the grass fringe from the grass palette, the curb from the paved set (preferring the concrete sidewalk palette), and the pond shore from the dirt palette, so each follows its source family's clinical treatment. Corner bits ship in the contract but the current art ignores them. Selection and precedence are **sim-owned** because Terrarium receives no site grid: on a paved–natural seam the sim must select `curb-edge`, and on a water seam it must select `pond-shore`, suppressing `grass-fringe` on the same seam; no extra payload field is introduced.

**Parking kit.** `car` / `car-suv` (`{w:4,h:2}`) and `car-compact` (`{w:3,h:2}`) are plan-projected exterior decor, remain **non-placeable**, and keep their body shell on the exported `$primary` layer for per-instance sim tinting. The legacy `parking-line` bay (`{w:2,h:2}`) also remains non-placeable. CE-21 adds placeable, walkable flat paint decals `lot-marking-accessible`, `lot-marking-arrow`, `lot-marking-reserved`, and `lot-marking-crosswalk`, plus placeable `lamp-post`, two baked `sign-lot` variants, and `bike-rack`; these ride the existing prop/layer/facility-catalog machinery with no payload change. The sim owns car spawning, flat-shadow exemptions, and decal placement semantics.

**Cafeteria kit (CE-22).** The placeable prop inventory is elevation-projected `serving-line` (`{w:4,h:1}`), `service-scanner` (`{w:1,h:1}`), `commercial-range` / `dish-return` / `walk-in-front` (each `{w:2,h:1}`), plus plan-projected `prep-table` (`{w:2,h:1}`), `dining-carrel` / `tray-stack` (each `{w:1,h:1}`), and `cafeteria-table` (`{w:4,h:2}`). These are ordinary blocking interior facilities and follow the building's clinical drain. `service-scanner` is an `AnchoredFacility` with interaction type `service_scanner`; its literal IRIS-green optic remains outside palette tinting, while attendance behavior stays sim-owned. The recipe-only human parts `outfit-service-apron` and `acc-hairnet` remain resolvable but non-selectable and are proven by the code-owned `kitchen-worker` recipe outside `DEFAULT_CAST`. No new staff folder or payload is added until the sim ratifies a staff-config binding; all CE-22 props export through the existing `props/` and facility-catalog paths.

**Quad, pond, and reflection-garden kit (CE-23/24).** `gravel` and `pond-water` are the two ground additions above; neither becomes a prop or changes an export payload. Seven plan-projected `{w:1,h:1}` flat decals ride the existing prop ground-detail convention: `ground-detail-rake-arc-a/-b/-c`, `ground-detail-lilypad-a/-b`, and optional `ground-detail-stepping-stone-a/-b`. All shapes are silhouette-free, remain absent from `facility-catalog.json`, and drain with certified surfaces rather than joining wild nature. Placeable furnishings are elevation-projected `park-bench` (`{w:2,h:1}`) and `stone-lantern` (`{w:1,h:1}`), plus plan-projected `picnic-table` (`{w:3,h:2}`) and `boulder-arrangement` (`{w:2,h:1}`); all drain under the clinical look. Elevation-projected `reeds-cluster` (`{w:1,h:1}`) is non-placeable, joins `NATURE_PROP_TEMPLATE_IDS`, and stays saturated with pond water. No fish or animation content is included. The sim owns ground build controls, water walkability, per-seam overlay precedence, flat-shadow registration (including serialized scene lists), and `lamp-post` / `stone-lantern` practical lights.

**Nature decor (D2 amendment).** Non-placeable exterior scenery the sim scatters over the natural ground and pond banks. Projection is family-specific: mature trees, saplings, and shoreline reeds are **elevation-projected, front-facing props**, rooted at their ground pivot so their crowns/stems rise above the occupied cells; shrubs and herbaceous patches remain **plan-projected low vegetation**. The current wild-field inventory is twelve baked flora silhouettes: mature trees `prop-tree`, `prop-tree-b`, `prop-tree-upright`, `prop-tree-conifer`; saplings/understory `prop-tree-sapling`, `prop-tree-sapling-b`; shrubs `prop-bush-cluster`, `prop-bush-bramble`, `prop-bush-low`; and herbaceous patches `prop-wildflower-patch`, `prop-tall-grass-clump`, `prop-bracken-patch`. `prop-boulder` remains the existing plan-projected rock accent, and `prop-reeds-cluster` is the shoreline addition. The backing template vocabulary is elevation-projected `tree-canopy` (`{w:3,h:3}`), `tree-sapling` (`{w:2,h:2}`), and `reeds-cluster` (`{w:1,h:1}`), plus plan-projected `bush-cluster` (`{w:2,h:1}`), `wildflower-patch` (`{w:2,h:2}`), `tall-grass-clump` (`{w:1,h:1}`), `bracken-patch` (`{w:2,h:1}`), and `boulder` (`{w:1,h:1}`). Several stable prop ids intentionally bake curated profiles/seeds from one template: Terrarium owns the generator range, while Unity receives deterministic sprite instances rather than generator controls.

The wild set uses asymmetrical crowns, broken contours, irregular negative space, and uneven clusters. A future landscaped set will use a separate, visibly maintained grammar — clipped masses, deliberate geometry, repeated rhythm, and explicit bed/edge shapes — instead of making wild plants do double duty. All nature templates are **clinical-EXEMPT** (`NATURE_PROP_TEMPLATE_IDS`, same split as natural ground §above), so they stay saturated while the office, cars, and paved lot drain. These new instances are additive art inventory only: the payload shape and schema do not change, non-placeable props remain absent from `facility-catalog.json`, and Unity scatter/import-catalog adoption is deferred until the art receives visual approval.
...
**Installed IRIS hardware.** The live `iris-installation-unit` is the locked floor-standing rack + console form: elevation-projected, `{w:2,h:1}`, placeable through the facility catalog as `iris_installation_unit`, and registered as the `iris_console` interaction anchor. Its `iris-installation-unit-dormant` twin has the same footprint and silhouette but no live green beacon; it is non-placeable and exists for the sim to swap during the boot sequence. `iris-charging-dock` is a `{w:1,h:1}` plan-projected, non-placeable floor fixture the sim seeds beneath/near the fabrication units. These assets and their export mechanics are present; their sterile chassis/detail language is intentionally still available for later visual refinement. This is descriptive clarification only — it adds no payload, migration, or schema change.
