...
| `characters/<id>/unit@Nx.png` + `unit-atlas@Nx.json` | `unitSheetDesc` / `unitAtlas` | character | **Operational-unit sheet** — the floor rendering: same 4 facings, IRIS pigment (§3.17). Pure palette swap over the identity; geometry/anchors/poses untouched by construction. |
...
|---|---|---|---|
| **Operational unit** | IRIS | `unit@Nx.png` + `unit-atlas@Nx.json`; `renderings.unit` palette in `recipe.json` and each layer `manifest@Nx.json` | the floor. Baked-sheet consumers use the unit sheet; the runtime layer compositor multiplies the same tintable masks by `renderings.unit` instead of `palette` (poses/moods included for free). |
