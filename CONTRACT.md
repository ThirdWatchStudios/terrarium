...
| `ground/<id>/tile@Nx.png` + `atlas@Nx.json` (+ `layers@Nx.png` + `layers-manifest@Nx.json` + `ground.json`) | `groundAtlas` / `groundLayerManifest` | project | **Outdoor ground surfaces** — the DISTINCT *ground kind* (B1.5 "the build site", decision D2): grass ×3 / meadow ×2 / dirt (natural) + asphalt / sidewalk (paved). Same flat-tile machinery as floors, but its own folder + `kind:"ground"` atlas + the −20000 sort band, so the sim's `ImportGround` pulls it in as a separate layer under the interior floor and never treats it as paintable floor (§3.18). **D2 amended:** NATURAL ground is exempt from the clinical drain — it ships lush and saturated under every look; only the PAVED surfaces drain (§3.18). |
| `ground-overlays/<id>/tileset@Nx.png` + `atlas@Nx.json` + `overlay.json` | `groundOverlayAtlas` / `deriveGroundOverlays` | project (derived) | **Ground-edge transition overlays** — `grass-fringe` softens natural seams and `curb-edge` draws a poured lip on paved receivers. Both use the same 8×6 / 47-frame `mask_<i>` tileset shape and blob contract as walls; sort band −19000 (above ground, below floors). Code-owned and palette-derived from their source ground families (§3.18). |
...
**Outdoor ground — a DISTINCT kind (decision D2, amended).** The natural surfaces (grass / meadow / dirt) and the paved surfaces (asphalt / sidewalk) ship as ground, **not** interior floor. They use the same flat, seamless 128-unit tile machinery (they *are* `FloorTemplate`s, rendered by `composeFloorTile`), but they are exported into their own `ground/<id>/` folder with a distinct **`kind:"ground"`** atlas that carries the ground **sort band (`-20000`, below the floor band)**. This is what lets the sim's `ImportGround` (its story A-S1) import ground as its own layer — drawn under the interior floor and everything else — and keep it **un-paintable as interior floor**. Directory parity with floors: the sim discovers ground the same way it discovers floors (iterate the folder, read each per-tile atlas + `ground.json`), so there is no separate manifest. Ground ships a re-tintable layer atlas too (`kind:"ground-layers"`). Ids: `ground-grass`, `ground-grass-b`, `ground-grass-c`, `ground-meadow`, `ground-meadow-b`, `ground-dirt`, `ground-asphalt`, `ground-sidewalk` (three curated grass instances + two meadow instances break the macro cadence of an empty field). Ground is **generated-only** (not player-painted, per the build spec §9), so there is no ground editor — it is a code-owned default the export always ships.

**The natural/paved register split (D2 amendment).** D2 originally gave ALL ground the clinical-drain treatment. Reversed for natural ground: under the clinical look, **grass / meadow / dirt keep their saturated authored palettes** — the same Article VIII exemption people get (`applyClinicalLook`, `NATURAL_GROUND_TEMPLATE_IDS`). The felt-target is the game's premise made visible: the office replaces something alive, so every clinical export shows drained plan surfaces displacing vivid living ground. The **paved** surfaces (asphalt / sidewalk — and the parking-line/car props, which drain as ordinary props) are the focus-grouped output and drain with the building. Sim-side implication: `ImportGround` needs no change (palettes arrive baked in the atlas), but any sim-side runtime drain of the ground layer must honor the same split.

**Ground-edge transition overlays (`ground-overlays/`).** Where two ground kinds meet, the seam is softened by a transition overlay drawn on the **receiving** cell. Every overlay rides the **shared 47-blob autotile contract** — the exact wall-tileset shape: an 8×6 `tileset@Nx.png` whose 47 frames are named `mask_<i>` where `<i>` is the blob index, atlas `meta.autotile: "8-neighbor blob (47)"` (the wall importer's validation marker, matched verbatim), sliced and selected sim-side by the same machinery (`WallBlob.Index`). `grass-fringe` draws encroaching grass-family art on a non-grass receiver; its set bit means "the neighbour on that side is encroaching natural ground." `curb-edge` draws a poured-concrete lip on an asphalt/sidewalk receiver; its set bit means "the neighbour on that side is natural ground." Both render at sort band **−19000** (above ground at −20000, below floors at −10000). Overlays are **code-owned and derived**, not authored: `deriveGroundOverlays` builds the grass fringe from the grass palette and the curb from the paved set (preferring the concrete sidewalk palette), so each follows its source family's clinical treatment. Corner bits ship in the contract but the current art ignores them. Selection and precedence are **sim-owned** because Terrarium receives no site grid: on a paved–natural seam the sim must select `curb-edge` and suppress `grass-fringe` for that seam; no extra payload field is introduced.
...
**Nature decor (D2 amendment).** Non-placeable exterior scenery the sim scatters over the natural ground. Projection is family-specific: mature trees and saplings are **elevation-projected, front-facing tall props**, rooted at their ground pivot so trunk and crown rise above the occupied cells; shrubs and herbaceous patches remain **plan-projected low vegetation**. The current wild-field inventory is twelve baked flora silhouettes: mature trees `prop-tree`, `prop-tree-b`, `prop-tree-upright`, `prop-tree-conifer`; saplings/understory `prop-tree-sapling`, `prop-tree-sapling-b`; shrubs `prop-bush-cluster`, `prop-bush-bramble`, `prop-bush-low`; and herbaceous patches `prop-wildflower-patch`, `prop-tall-grass-clump`, `prop-bracken-patch`. `prop-boulder` remains the existing plan-projected rock accent. The backing template vocabulary is elevation-projected `tree-canopy` (`{w:3,h:3}`) and `tree-sapling` (`{w:2,h:2}`), plus plan-projected `bush-cluster` (`{w:2,h:1}`), `wildflower-patch` (`{w:2,h:2}`), `tall-grass-clump` (`{w:1,h:1}`), `bracken-patch` (`{w:2,h:1}`), and `boulder` (`{w:1,h:1}`). Several stable prop ids intentionally bake curated profiles/seeds from one template: Terrarium owns the generator range, while Unity receives deterministic sprite instances rather than generator controls.
