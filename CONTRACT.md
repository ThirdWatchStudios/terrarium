...
| `ground-overlays/<id>/tileset@Nx.png` + `atlas@Nx.json` + `overlay.json` | `groundOverlayAtlas` / `deriveGroundOverlays` | project (derived) | **Ground-edge transition overlays** — the grass fringe drawn on a receiving ground cell (dirt/asphalt/sidewalk) where natural ground borders it, replacing the hard tile seam with a torn organic edge. Same 8×6 / 47-frame `mask_<i>` tileset shape and blob contract as walls; sort band −19000 (above ground, below floors). Code-owned, derived from the grass instance's palette (§3.18). |
...
**Ground-edge transition overlays (`ground-overlays/`).** Where two ground kinds meet, the seam is softened by a transition overlay drawn on the **receiving** cell (dirt / asphalt / sidewalk) showing the **encroaching** natural ground (grass family: templates `grass` and `meadow`) bleeding over the edge. The overlay rides the **shared 47-blob autotile contract** — the exact wall-tileset shape: an 8×6 `tileset@Nx.png` whose 47 frames are named `mask_<i>` where `<i>` is the blob index, atlas `meta.autotile: "8-neighbor blob (47)"` (the wall importer's validation marker, matched verbatim), sliced and selected sim-side by the same machinery (`WallBlob.Index`). Only the bit *semantics* differ: for a receiving cell, a set neighbour bit means "the ground on that side is encroaching natural ground". Selection is **sim-owned**: for each ground cell whose template is NOT grass-family, compute the 8-neighbour mask over grass-family neighbours and, when non-zero, draw the fringe frame at sort band **−19000** (above the ground layer at −20000, below floors at −10000). Overlays are **code-owned and derived**, not authored: the export builds `overlay-grass-fringe` from the project's grass ground instance palette (`deriveGroundOverlays`), so the fringe matches the field it bleeds from and inherits the natural-ground clinical exemption automatically. Corner bits ship in the contract but the current art ignores them (fringe depth makes the pocket/wrap distinction invisible); same-family seams (grass↔meadow) get no overlay by construction. Additional styles (e.g. a sidewalk curb onto asphalt) are additive: new `ground-overlays/<id>/` folders on the same contract.
