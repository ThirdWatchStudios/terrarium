...
- `wings[]`: `{ id, departmentId, label, roomIds[], bounds:{x,y,cols,rows} }` — rooms grouped into department wings (the physical projection of the org chart; sim reveals wing-by-wing + measures wing distance). Each `rooms[]` entry may carry an optional `departmentId` (the F2.1 catalog id) that `computeWings` groups by. The **default golden export ships a composed multi-department office**: one `cubicle-farm@<dept>` bullpen wing per populated department, plus a `wing-common` holding the shared **sim-bound** rooms (`reception`, `hallway`, `manager-office`, `break-room`, `conference-room`) — so a composed office stays loadable by the sim and the scripted scenario (which binds those room ids + per-agent `desk:` anchors). The single-template (hero) path instead tags its existing rooms onto the cast's departments (bullpen→operations, manager-office→management). **Backward-compatible:** a department-less template office tags nothing and collapses to one implicit wing (`wing-main`, `departmentId: null`).
