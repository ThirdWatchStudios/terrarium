...
|---|---|---|---|
| **Operational unit** | IRIS | `unit@Nx.png` + `unit-atlas@Nx.json` + `unit-poses@Nx.png` + `unit-poses-atlas@Nx.json`; drawing spec (`parts` + coding `palette`) in `recipe.json#renderings.unit` | the floor. A **pictogram** — flat coding hue (derived from the identity's outfit hue, normalized; cold ≠ grey, cold = category coding), featureless `UNIT_INK` head disc, no face (feelings arrive as IRIS claims, never on the head). NOT producible by re-tinting the warm layer masks — different geometry; consume the baked unit sheets or re-compose from the spec. |
