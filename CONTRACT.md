...
**Nature decor (D2 amendment).** Plan-projected, non-placeable exterior scenery the sim scatters over the natural ground: `tree-canopy` (`{w:3,h:3}`, two default tones `prop-tree`/`prop-tree-b`), `tree-sapling` (`{w:2,h:2}`), `bush-cluster` (`{w:2,h:1}`), `wildflower-patch` (`{w:2,h:2}`, a mostly-transparent decal like the parking line), and `boulder` (`{w:1,h:1}`). Same mechanics as the cars (normal `props/` folder, `NON_PLACEABLE_TEMPLATES`), with one register rule: these are **clinical-EXEMPT** (`NATURE_PROP_TEMPLATE_IDS`, same split as natural ground §above) — under the clinical look, trees and flowers stay saturated while the office, the cars and the paved lot drain. Any sim-side runtime drain must honor the same exemption.
