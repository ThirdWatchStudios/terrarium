...
function validateAttributes(
  ...
  if (!ignored && attrs['fill-rule'] !== undefined && attrs['fill-rule'].toLowerCase() !== 'nonzero') {
    fail(source, 'only the nonzero fill rule is supported');
  }
...
export function emitImportedPartArt(imports: readonly ImportedPartArt[]): string {
  ...
  if (imports.length === 0) {
    lines.push('export const IMPORTED_PART_ART = [] as const satisfies readonly ImportedPartArt[];');
    return `${lines.join('\n')}\n`;
  }
  ...
  lines.push('] as const satisfies readonly ImportedPartArt[];');
  return `${lines.join('\n')}\n`;
}
