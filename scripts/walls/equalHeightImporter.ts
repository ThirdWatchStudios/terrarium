...
import { QUOTA_CO_EQUAL_HEIGHT_MIRROR_X_MASKS } from '../../src/tiles/quotaCoEqualHeightWallContract';
 ...
 * presentations for these three vertical wall rows. The production atlas
 * intentionally bakes the west/direct presentation and leaves the east choice
 * to Unity's SpriteRenderer.flipX context.
 */
export const QUOTA_CO_EQUAL_HEIGHT_FLIP_REQUIRED_MASKS =
...
/**
 * Emit the accepted all-47 bank as browser-safe production data. Source
 * provenance remains in the authored SVG bank and ledger; the runtime registry
 * contains only deterministic rendering and facing fields.
 */
export function emitQuotaCoEqualHeightWallRegistry(
  frames: readonly CompiledEqualHeightFrame[],
): string {
  if (frames.length !== BLOB_TILE_COUNT) {
    throw new EqualHeightWallImportError(
      `production registry has ${frames.length} frames; expected ${BLOB_TILE_COUNT}`,
    );
  }
  ...
  for (let index = 0; index < BLOB_TILE_COUNT; index += 1) {
    ...
    if (
      ...
      throw new EqualHeightWallImportError(
        `production registry drift at mask_${index}`,
      );
    }
    lines.push('  {');
    lines.push(`    id: ${quote(frame.id)},`);
    lines.push(`    index: ${frame.index},`);
    lines.push(`    canonicalMask: ${frame.canonicalMask},`);
    lines.push(
  ...
  lines.push('] as const satisfies QuotaCoEqualHeightWallRegistry;');
  return `${lines.join('\n')}\n`;
}
