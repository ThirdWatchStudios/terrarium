...
AcceptedSystemGate = {
  ...
  summary: 'Accepted one independently authored fixed-view four-way union with northeast, southeast, and northwest solid while the southwest floor crook remains open. Its shade, coral, green, highlight, and seam share one curved return into the south socket; mask_44 inherits the same pixels through its plain X mirror.',
...
export function renderStyleWorkbenchPage(diagnosticStems: readonly string[]): string {
  ...
  const acceptedSingleOpenSouthwestCrossJunctionStatus =
    ...
    '<p>mask_41 directly reuses one independently authored fixed-view union with the approved shared south-return curve; mask_44 inherits it through the existing plain X mirror.</p></div>';
  ...
  const acceptedSingleFilledNorthwestCrossJunctionStatus =
    ...
    '<p>mask_37 directly reuses one east-fixed authored four-way union with the approved shared southwest curve while every full-resolution socket pixel remains fixed.</p></div>';
  const acceptedSingleFilledSouthwestCrossJunctionStatus =
    ...
    '<p>mask_29 is the accepted plain whole-cell X mirror of mask_23 and inherits its approved shared open-crook curve without changing any socket pixel.</p></div>';
  ...
  const acceptedSingleFilledSoutheastCrossJunctionStatus =
    ...
    '<p>mask_23 directly reuses one west-fixed authored four-way union with the approved nested cream, arris, shade, coral, green, and seam curve while every socket pixel remains fixed.</p></div>';
  ...
  const acceptedHorizontalPartialTJunctionStatus =
    ...
    '<p>mask_18/mask_22 are direct; mask_35/mask_28 are approved X derivations. Masks 22/28 share the approved shade, cream lip, dark outline, coral, and green hierarchy.</p></div>';
