...
export const ACCEPTED_MAPPING_GATE: AcceptedSystemGate = {
  ...
  summary: 'Accepted topology map: 21 direct reuses, 14 approved derivations, 12 synthetic cross-junction candidates, and 0 authored-geometry gaps.',
...
const acceptedSystemGate = (
  ...
  refreshGroup: 'double-filled-east-cross-junction' | 'single-filled-southeast-cross-junction' | 'single-filled-cross-junction' | 'open-pocket-cross-junction' | 'horizontal-partial-t-junction' | 'east-partial-t-junction' | 'single-filled-pocket-t-junction' | 'horizontal-open-pocket-t-junction' | 'open-pocket-t-junction' | 'thick-wall-horizontal-repeat' | 'thick-wall-repeat' | 'thick-wall-block' | 'isolated-shell' | 'vertical-terminus' | 'terminus' | 'mapping' | 'corridor',
  gateId: 'double-filled-east-cross-junction' | 'single-filled-southeast-cross-junction' | 'single-filled-cross-junction' | 'open-pocket-cross-junction' | 'horizontal-partial-t-junction' | 'east-partial-t-junction' | 'single-filled-pocket-t-junction' | 'horizontal-open-pocket-t-junction' | 'open-pocket-t-junction' | 'thick-wall-horizontal-repeat' | 'thick-wall-repeat' | 'thick-wall-block' | 'isolated-shell' | 'vertical-terminus' | 'terminus' | 'mapping' | 'corridor',
): string => (
...
export function renderStyleWorkbenchPage(diagnosticStems: readonly string[]): string {
  ...
  const acceptedMappingGateStatus = '<div class="status-card accepted"><span>Accepted system mapping</span>' +
    ...
    '<p>The 47-row topology plan is locked; its 12 remaining synthetic cross-junction candidates remain proof-only.</p></div>';
  ...
  return (
    ...
    '<p class="lede">No proposal is currently active. mask_25 is accepted as the direct east-filled slab-junction source; the ledger now stands at 21 direct / 14 derived / 12 synthetic.</p></header>' +
