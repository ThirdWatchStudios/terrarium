...
/**
 * The current equal-height decision sheets. No wall proposal is active after
 * accepting the independently authored mask_41 source.
 */
...
export function renderStyleWorkbenchPage(diagnosticStems: readonly string[]): string {
  ...
  const acceptedMappingGateStatus = '<div class="status-card accepted"><span>Accepted system mapping</span>' +
    ...
    '<p>The 47-row topology plan is locked; its 4 remaining synthetic cross-junction candidates remain proof-only.</p></div>';
  ...
  return (
    ...
    '<p class="lede">mask_41 is accepted as one authored single-open southwest cross-junction. The accepted proof ledger now stands at 26 direct / 17 derived / 4 synthetic.</p></header>' +
