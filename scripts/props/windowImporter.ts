...
export async function compileQuotaCoWindowArt(
  inputDir: string,
  sourcePathPrefix: string,
): Promise<readonly ImportedQuotaCoWindowArt[]> {
  ...
  for (const definition of QUOTA_CO_WINDOW_SOURCE_DEFINITIONS) {
    ...
    variants[definition.key] = compileStaticPropSource(sourceFile, source, {
      id: 'window',
      projection: 'plan',
      paletteDefaults: QUOTA_CO_WINDOW_PALETTE,
    });
...
export function emitQuotaCoWindowArt(
  imports: readonly ImportedQuotaCoWindowArt[],
): string {
  const lines = [
