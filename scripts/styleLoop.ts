 ...
 * The open page is a current-state decision surface: the final accepted
 * mask_46 direct proof source gate first, then the accepted source and
 * derivation gates that constrain it. Historical mixed-profile gates and
...
} from './highOblique/a1bDoubleFilledNorthCrossJunctionProposal';
import {
...
} from './highOblique/a1bDoubleFilledSouthCrossJunctionProposal';
import {
...
} from './highOblique/a1bSingleOpenSouthwestCrossJunctionProposal';
import {
...
} from './highOblique/a1bSingleOpenNorthwestCrossJunctionProposal';
import {
  A1B_FULLY_FILLED_CROSS_JUNCTION_PROPOSAL_SOURCE_INVENTORY,
  compileA1bFullyFilledCrossJunctionProposalDirectory,
} from './highOblique/a1bFullyFilledCrossJunctionProposal';
import {
...
} from './highOblique/a1bDoubleFilledDiagonalCrossJunctionProposal';
import {
...
} from './highOblique/equalHeightSingleFilledSoutheastCrossJunctionGate';
import {
...
} from './highOblique/equalHeightDoubleFilledNorthCrossJunctionGate';
import {
  EQUAL_HEIGHT_DOUBLE_FILLED_SOUTH_CROSS_JUNCTION_GATE,
} from './highOblique/equalHeightDoubleFilledSouthCrossJunctionGate';
import {
  doubleFilledDiagonalCrossJunctionVerticalFacing,
  EQUAL_HEIGHT_DOUBLE_FILLED_DIAGONAL_CROSS_JUNCTION_GATE,
  type EqualHeightDoubleFilledDiagonalCrossJunctionVerticalFacing,
} from './highOblique/equalHeightDoubleFilledDiagonalCrossJunctionGate';
import {
  doubleFilledOppositeDiagonalCrossJunctionVerticalFacing,
  EQUAL_HEIGHT_DOUBLE_FILLED_OPPOSITE_DIAGONAL_CROSS_JUNCTION_GATE,
} from './highOblique/equalHeightDoubleFilledOppositeDiagonalCrossJunctionGate';
import {
...
} from './highOblique/equalHeightSingleOpenSouthwestCrossJunctionGate';
import {
  EQUAL_HEIGHT_SINGLE_OPEN_NORTHWEST_CROSS_JUNCTION_GATE,
  type EqualHeightSingleOpenNorthwestCrossJunctionMatrixMask,
} from './highOblique/equalHeightSingleOpenNorthwestCrossJunctionGate';
import {
  EQUAL_HEIGHT_SINGLE_OPEN_NORTHEAST_CROSS_JUNCTION_GATE,
  type EqualHeightSingleOpenNortheastCrossJunctionMatrixMask,
} from './highOblique/equalHeightSingleOpenNortheastCrossJunctionGate';
import {
  EQUAL_HEIGHT_FULLY_FILLED_CROSS_JUNCTION_GATE,
  type EqualHeightFullyFilledCrossJunctionMatrixMask,
} from './highOblique/equalHeightFullyFilledCrossJunctionGate';
import {
...
const doubleFilledNorthCrossJunctionProposalDirectory = (input: string): string =>
  path.join(crossSectionProofsDirectory(input), 'double-filled-north-cross-junction');

const doubleFilledSouthCrossJunctionProposalDirectory = (input: string): string =>
  path.join(crossSectionProofsDirectory(input), 'double-filled-south-cross-junction');

const singleOpenSouthwestCrossJunctionProposalDirectory = (input: string): string =>
  path.join(crossSectionProofsDirectory(input), 'single-open-southwest-cross-junction');

const singleOpenNorthwestCrossJunctionProposalDirectory = (input: string): string =>
  path.join(crossSectionProofsDirectory(input), 'single-open-northwest-cross-junction');

const fullyFilledCrossJunctionProposalDirectory = (input: string): string =>
  path.join(crossSectionProofsDirectory(input), 'fully-filled-cross-junction');

const doubleFilledDiagonalCrossJunctionProposalDirectory = (input: string): string =>
  path.join(
    crossSectionProofsDirectory(input),
    'double-filled-diagonal-cross-junction',
  );
...
const DOUBLE_FILLED_OPPOSITE_DIAGONAL_FILTERED_BASE_FILE =
  '__proof__/open-cross-filled-nw-se-filtered-base.svg';
const DOUBLE_FILLED_OPPOSITE_DIAGONAL_FILTERED_UPPER_FILE =
  '__proof__/open-cross-filled-nw-se-filtered-upper.svg';
...
async function render(
  ...
  await renderEqualHeightFullyFilledCrossJunctionGate(options, root);
  await renderEqualHeightSingleOpenNorthwestCrossJunctionGate(
    options,
    root,
  );
  await renderEqualHeightSingleOpenNortheastCrossJunctionGate(
    options,
    root,
  );
  await renderEqualHeightSingleOpenSoutheastCrossJunctionGate(
    options,
    root,
  );
  await renderEqualHeightSingleOpenSouthwestCrossJunctionGate(
    options,
    root,
  );
  await renderEqualHeightDoubleFilledSouthCrossJunctionGate(options, root);
  await renderEqualHeightDoubleFilledOppositeDiagonalCrossJunctionProposal(
    options,
    root,
  );
  await renderEqualHeightDoubleFilledDiagonalCrossJunctionProposal(
    options,
    root,
  );
  await renderEqualHeightDoubleFilledNorthCrossJunctionGate(options, root);
  ...
  await renderEqualHeightDoubleFilledEastCrossJunctionProposal(options, root);
  await renderEqualHeightSingleFilledSoutheastCrossJunctionProposal(options, root);
  ...
  const status = {
    ...
    fullyFilledCrossJunctionRenderedAt: renderedAt,
    singleOpenNorthwestCrossJunctionRenderedAt: renderedAt,
    singleOpenNortheastCrossJunctionRenderedAt: renderedAt,
    singleOpenSoutheastCrossJunctionRenderedAt: renderedAt,
    singleOpenSouthwestCrossJunctionRenderedAt: renderedAt,
    doubleFilledOppositeDiagonalCrossJunctionRenderedAt: renderedAt,
    doubleFilledDiagonalCrossJunctionRenderedAt: renderedAt,
    ...
    singleFilledSoutheastCrossJunctionRenderedAt: renderedAt,
...
async function singleFilledSoutheastCrossJunctionProposalFileOverrides(
  options: CliOptions,
  root: string,
): Promise<CompositionFileOverrides> {
  const directory = singleFilledSoutheastCrossJunctionProposalDirectory(options.input);
  const compiled = await compileA1bSingleFilledSoutheastCrossJunctionProposalDirectory({
    inputDir: directory,
    sourcePathPrefix: path.relative(root, directory).replaceAll(path.sep, '/'),
  });
  return Object.fromEntries([
...
async function doubleFilledNorthCrossJunctionProposalFileOverrides(
  options: CliOptions,
  root: string,
): Promise<CompositionFileOverrides> {
  const directory =
    doubleFilledNorthCrossJunctionProposalDirectory(options.input);
  const compiled =
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    throw new Error(
      'Double-filled north cross-junction compiler inventory drift',
    );
  }
  return Object.fromEntries([
    [EMPTY_WORKBENCH_FILE, EMPTY_WORKBENCH_SOURCE],
...
async function doubleFilledSouthCrossJunctionProposalFileOverrides(
  options: CliOptions,
  root: string,
): Promise<CompositionFileOverrides> {
  const directory =
    doubleFilledSouthCrossJunctionProposalDirectory(options.input);
  const compiled =
    await compileA1bDoubleFilledSouthCrossJunctionProposalDirectory({
      inputDir: directory,
      sourcePathPrefix: path.relative(root, directory).replaceAll(path.sep, '/'),
    });
  const actual = compiled.map(({ filename }) => filename).sort();
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    throw new Error(
      'Double-filled south cross-junction compiler inventory drift',
    );
  }
  return Object.fromEntries([
    [EMPTY_WORKBENCH_FILE, EMPTY_WORKBENCH_SOURCE],
...
async function singleOpenSouthwestCrossJunctionProposalFileOverrides(
  options: CliOptions,
  root: string,
): Promise<CompositionFileOverrides> {
  const directory =
    singleOpenSouthwestCrossJunctionProposalDirectory(options.input);
  const compiled =
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    throw new Error(
      'Single-open southwest cross-junction compiler inventory drift',
    );
  }
  return Object.fromEntries([
    [EMPTY_WORKBENCH_FILE, EMPTY_WORKBENCH_SOURCE],
...
async function singleOpenNorthwestCrossJunctionProposalFileOverrides(
  options: CliOptions,
  root: string,
): Promise<CompositionFileOverrides> {
  const directory =
    singleOpenNorthwestCrossJunctionProposalDirectory(options.input);
  const compiled =
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    throw new Error(
      'Single-open northwest cross-junction compiler inventory drift',
    );
  }
  return Object.fromEntries([
    [EMPTY_WORKBENCH_FILE, EMPTY_WORKBENCH_SOURCE],
...
async function fullyFilledCrossJunctionProposalFileOverrides(
  options: CliOptions,
  root: string,
): Promise<CompositionFileOverrides> {
  const directory =
    fullyFilledCrossJunctionProposalDirectory(options.input);
  const compiled =
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    throw new Error(
      'Fully filled cross-junction compiler inventory drift',
    );
  }
  return Object.fromEntries([
    [EMPTY_WORKBENCH_FILE, EMPTY_WORKBENCH_SOURCE],
...
async function doubleFilledDiagonalCrossJunctionProposalFileOverrides(
  options: CliOptions,
  root: string,
): Promise<CompositionFileOverrides> {
  const directory =
    doubleFilledDiagonalCrossJunctionProposalDirectory(options.input);
  const compiled =
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    throw new Error(
      'Double-filled diagonal cross-junction compiler inventory drift',
    );
  }
  return Object.fromEntries([
    [EMPTY_WORKBENCH_FILE, EMPTY_WORKBENCH_SOURCE],
...
async function doubleFilledOppositeDiagonalCrossJunctionFileOverrides(
  options: CliOptions,
  root: string,
): Promise<CompositionFileOverrides> {
  const sourceOverrides =
    await doubleFilledDiagonalCrossJunctionProposalFileOverrides(options, root);
  const source =
    EQUAL_HEIGHT_DOUBLE_FILLED_DIAGONAL_CROSS_JUNCTION_GATE.candidate;
  const baseSource = sourceOverrides[source.baseFile];
  const upperSource = sourceOverrides[source.upperFile];
  if (!baseSource || !upperSource) {
    throw new Error(
      'Accepted mask_30 source bank omitted the diagonal cross-junction pair',
    );
  }
  const filtered = derivePromotedSoutheastSourcePair(
    baseSource,
  ...
  return {
    ...
    [DOUBLE_FILLED_OPPOSITE_DIAGONAL_FILTERED_UPPER_FILE]:
      filtered.upperSource,
  };
}

async function doubleFilledEastCrossJunctionProposalFileOverrides(
  options: CliOptions,
  root: string,
): Promise<CompositionFileOverrides> {
  const directory = doubleFilledEastCrossJunctionProposalDirectory(options.input);
  const compiled = await compileA1bDoubleFilledEastCrossJunctionProposalDirectory({
    inputDir: directory,
    sourcePathPrefix: path.relative(root, directory).replaceAll(path.sep, '/'),
  });
  return Object.fromEntries([
...
async function renderTransitionWestSouthFocus(options: CliOptions, root: string): Promise<void> {
  ...
  const parts: string[] = [
    ...
    text(390, 579, '• the accepted 112-unit vertical mass', 11, 600, MUTED),
...
async function renderFullHeightEastMirrorProof(options: CliOptions): Promise<void> {
  ...
  const parts: string[] = [
    ...
    text(564, 132, 'Accepted 112-unit vertical master; room-facing contact at right.', 11, 600, MUTED),
    ...
    text(790, 418, 'FULL WEST · x11.5..123.5', 11, 800, MUTED, 'middle'),
    text(1320, 418, 'FULL EAST · x4.5..116.5', 11, 800, A1A_PALETTE.green, 'middle'),
    ...
    text(1090, 626, '• 112-unit envelope remains inside the wall cell', 11, 600, MUTED),
...
async function renderFullHeightNortheastProof(options: CliOptions): Promise<void> {
  ...
  const parts: string[] = [
    ...
    text(430, 618, '• y11.5..123.5 profile preserved', 11, 600, MUTED),
    ...
    text(824, 522, 'The mirrored corner exits into the same x4.5..116.5 full-east socket.', 11, 600, MUTED),
    text(1190, 590, 'EAST SOCKET', 11, 800),
    text(1190, 618, '• x4.5..116.5 profile preserved', 11, 600, MUTED),
...
async function renderFullHeightSouthwestProof(options: CliOptions): Promise<void> {
  ...
  const parts: string[] = [
    ...
    text(430, 618, '• x11.5..123.5 envelope preserved', 11, 600, MUTED),
    ...
    text(1190, 618, '• y11.5..123.5 envelope preserved', 11, 600, MUTED),
...
async function renderFullHeightSoutheastProof(options: CliOptions): Promise<void> {
  ...
  const parts: string[] = [
    ...
    text(430, 618, '• x4.5..116.5 envelope preserved', 11, 600, MUTED),
    text(430, 646, '• cream / coral / green registers meet', 11, 600, MUTED),
...
async function renderEqualHeightCorridorGate(options: CliOptions): Promise<void> {
  ...
  const parts: string[] = [
    ...
    text(624, 1028, 'All 47 proof-layer rows now have accepted direct or derived provenance.', 12, 750, A1A_PALETTE.green),
  ];
...
type DoubleFilledEastCrossJunctionMatrixMask =
  | 2 | 10
  | 16 | 20 | 24 | 25 | 26 | 34 | 42;

type DoubleFilledWestCrossJunctionMatrixMask =
  | 8 | 10
  | 16 | 20 | 24 | 26 | 34 | 42 | 43;

function doubleFilledWestCrossJunctionCandidateCell(
  col: number,
  row: number,
  layer: 'base' | 'upper' | 'composed' = 'composed',
): CompositionCell {
  const candidate = EQUAL_HEIGHT_DOUBLE_FILLED_WEST_CROSS_JUNCTION_GATE.candidate;
  if (layer === 'base') {
    return [col, row, candidate.baseFile, null, candidate.transform];
  }
  if (layer === 'upper') {
...
function doubleFilledEastCrossJunctionCandidateCell(
  col: number,
  row: number,
  layer: 'base' | 'upper' | 'composed' = 'composed',
): CompositionCell {
  const candidate = EQUAL_HEIGHT_DOUBLE_FILLED_EAST_CROSS_JUNCTION_GATE.candidate;
  if (layer === 'base') return [col, row, candidate.baseFile, null];
  if (layer === 'upper') return [col, row, EMPTY_WORKBENCH_FILE, candidate.upperFile];
  return [col, row, candidate.baseFile, candidate.upperFile];
}

function acceptedThickWallRepeatCell(
  maskIndex: 24 | 42,
  col: number,
  row: number,
): CompositionCell {
  const candidate = EQUAL_HEIGHT_THICK_WALL_REPEAT_GATE.candidates.find(
    (entry) => entry.maskIndex === maskIndex,
  );
  if (!candidate) throw new Error(`Accepted thick-wall repeat source missing for mask_${maskIndex}`);
  return thickWallRepeatCandidateCell(candidate, col, row);
}

function doubleFilledEastCrossJunctionMatrixCell(
  maskIndex: DoubleFilledEastCrossJunctionMatrixMask,
  col: number,
  row: number,
): CompositionCell {
  if (maskIndex === 25) {
    return doubleFilledEastCrossJunctionCandidateCell(col, row);
  }
  if (maskIndex === 16 || maskIndex === 20 || maskIndex === 26 || maskIndex === 34) {
    return acceptedFilledBlockCell(maskIndex, col, row);
  }
  if (maskIndex === 24 || maskIndex === 42) {
    return acceptedThickWallRepeatCell(maskIndex, col, row);
  }
  if (maskIndex === 2) {
    return openPocketHorizontalEndCell(maskIndex, col, row);
  }
...
function doubleFilledEastCrossJunctionMatrixCells(
  matrix: readonly (readonly (number | null)[])[],
): readonly CompositionCell[] {
  const cells: CompositionCell[] = [];
  for (const [row, masks] of matrix.entries()) {
    for (const [col, maskIndex] of masks.entries()) {
      if (maskIndex === null) continue;
      cells.push(doubleFilledEastCrossJunctionMatrixCell(
        maskIndex as DoubleFilledEastCrossJunctionMatrixMask,
        col,
        row,
      ));
    }
...
async function doubleFilledEastCrossJunctionMatrixWindow(
  options: CliOptions,
  matrix: readonly (readonly (number | null)[])[],
  x: number,
  y: number,
  ...
  return compositionWindow(
    options,
    doubleFilledEastCrossJunctionMatrixCells(matrix),
    matrix[0].length,
    matrix.length,
    x,
    y,
...
function doubleFilledWestCrossJunctionMatrixCell(
  maskIndex: DoubleFilledWestCrossJunctionMatrixMask,
  col: number,
  row: number,
): CompositionCell {
  if (maskIndex === 43) {
    return doubleFilledWestCrossJunctionCandidateCell(col, row);
  }
  if (maskIndex === 16 || maskIndex === 20 || maskIndex === 26 || maskIndex === 34) {
    return acceptedFilledBlockCell(maskIndex, col, row);
  }
...
function doubleFilledWestCrossJunctionMatrixCells(
  matrix: readonly (readonly (number | null)[])[],
): readonly CompositionCell[] {
  const cells: CompositionCell[] = [];
  for (const [row, masks] of matrix.entries()) {
    for (const [col, maskIndex] of masks.entries()) {
      if (maskIndex === null) continue;
      cells.push(doubleFilledWestCrossJunctionMatrixCell(
        maskIndex as DoubleFilledWestCrossJunctionMatrixMask,
        col,
        row,
      ));
    }
...
async function doubleFilledWestCrossJunctionMatrixWindow(
  options: CliOptions,
  matrix: readonly (readonly (number | null)[])[],
  x: number,
  y: number,
  ...
  return compositionWindow(
    options,
    doubleFilledWestCrossJunctionMatrixCells(matrix),
    matrix[0].length,
    matrix.length,
    x,
    y,
...
type SingleFilledSoutheastCrossJunctionMatrixMask =
  | 1 | 2 | 4 | 5 | 8 | 10
  | 16 | 17 | 23 | 26 | 28 | 34;

function singleFilledSoutheastCrossJunctionCandidateCell(
  col: number,
  row: number,
  layer: 'base' | 'upper' | 'composed' = 'composed',
): CompositionCell {
  const candidate = EQUAL_HEIGHT_SINGLE_FILLED_SOUTHEAST_CROSS_JUNCTION_GATE.candidate;
  if (layer === 'base') return [col, row, candidate.baseFile, null];
  if (layer === 'upper') return [col, row, EMPTY_WORKBENCH_FILE, candidate.upperFile];
  return [col, row, candidate.baseFile, candidate.upperFile];
}

function singleFilledSoutheastCrossJunctionMatrixCell(
  maskIndex: SingleFilledSoutheastCrossJunctionMatrixMask,
  col: number,
  row: number,
): CompositionCell {
  if (maskIndex === 23) {
    return singleFilledSoutheastCrossJunctionCandidateCell(col, row);
  }
  if (maskIndex === 16 || maskIndex === 26 || maskIndex === 34) {
    return acceptedFilledBlockCell(maskIndex, col, row);
  }
  if (maskIndex === 17) {
    return westPartialTJunctionCandidateCell(maskIndex, col, row);
...
function singleFilledSoutheastCrossJunctionMatrixCells(
  matrix: readonly (readonly (number | null)[])[],
): readonly CompositionCell[] {
  const cells: CompositionCell[] = [];
  for (const [row, masks] of matrix.entries()) {
    for (const [col, maskIndex] of masks.entries()) {
      if (maskIndex === null) continue;
      cells.push(singleFilledSoutheastCrossJunctionMatrixCell(
        maskIndex as SingleFilledSoutheastCrossJunctionMatrixMask,
        col,
        row,
      ));
    }
...
async function singleFilledSoutheastCrossJunctionMatrixWindow(
  options: CliOptions,
  matrix: readonly (readonly (number | null)[])[],
  x: number,
  y: number,
  ...
  return compositionWindow(
    options,
    singleFilledSoutheastCrossJunctionMatrixCells(matrix),
    matrix[0].length,
    matrix.length,
    x,
    y,
...
type DoubleFilledNorthCrossJunctionMatrixMask =
  | 1 | 5 | 16 | 20 | 26 | 31 | 34 | 38 | 39;

function doubleFilledNorthCrossJunctionCandidateCell(
  col: number,
  row: number,
  layer: 'base' | 'upper' | 'composed' = 'composed',
): CompositionCell {
  const candidate =
    EQUAL_HEIGHT_DOUBLE_FILLED_NORTH_CROSS_JUNCTION_GATE.candidate;
  if (layer === 'base') return [col, row, candidate.baseFile, null];
  if (layer === 'upper') {
...
function doubleFilledNorthCrossJunctionMatrixCell(
  maskIndex: DoubleFilledNorthCrossJunctionMatrixMask,
  col: number,
  row: number,
): CompositionCell {
  if (maskIndex === 39) {
    return doubleFilledNorthCrossJunctionCandidateCell(col, row);
  }
  if (
    maskIndex === 16 ||
    maskIndex === 20 ||
  ...
  if (maskIndex === 31 || maskIndex === 38) {
    const candidate =
      EQUAL_HEIGHT_THICK_WALL_HORIZONTAL_REPEAT_GATE.candidates.find(
        ({ maskIndex: candidateIndex }) => candidateIndex === maskIndex,
      );
    if (!candidate) {
      throw new Error(
        `Accepted horizontal repeat source missing for mask_${maskIndex}`,
      );
    }
    return thickWallHorizontalRepeatCandidateCell(candidate, col, row);
  }
...
function doubleFilledNorthCrossJunctionMatrixCells(
  matrix: readonly (readonly (number | null)[])[],
): readonly CompositionCell[] {
  const cells: CompositionCell[] = [];
  for (const [row, masks] of matrix.entries()) {
    for (const [col, maskIndex] of masks.entries()) {
      if (maskIndex === null) continue;
      cells.push(
        doubleFilledNorthCrossJunctionMatrixCell(
          maskIndex as DoubleFilledNorthCrossJunctionMatrixMask,
          col,
          row,
        ),
      );
...
async function doubleFilledNorthCrossJunctionMatrixWindow(
  options: CliOptions,
  matrix: readonly (readonly (number | null)[])[],
  x: number,
  y: number,
  ...
  return compositionWindow(
    options,
    doubleFilledNorthCrossJunctionMatrixCells(matrix),
    matrix[0].length,
    matrix.length,
    x,
    y,
...
type DoubleFilledSouthCrossJunctionMatrixMask =
  | 1 | 2 | 4 | 5 | 8 | 10
  | 16 | 20 | 22 | 26 | 28 | 32 | 34 | 38 | 39;

function doubleFilledSouthCrossJunctionCandidateCell(
  col: number,
  row: number,
  layer: 'base' | 'upper' | 'composed' = 'composed',
): CompositionCell {
  const candidate =
    EQUAL_HEIGHT_DOUBLE_FILLED_SOUTH_CROSS_JUNCTION_GATE.candidate;
  if (layer === 'base') return [col, row, candidate.baseFile, null];
  if (layer === 'upper') {
    return [col, row, EMPTY_WORKBENCH_FILE, candidate.upperFile];
  }
...
function doubleFilledSouthCrossJunctionMatrixCell(
  maskIndex: DoubleFilledSouthCrossJunctionMatrixMask,
  col: number,
  row: number,
): CompositionCell {
  if (maskIndex === 32) {
    return doubleFilledSouthCrossJunctionCandidateCell(col, row);
  }
  if (maskIndex === 39) {
    return doubleFilledNorthCrossJunctionCandidateCell(col, row);
  }
...
function doubleFilledSouthCrossJunctionMatrixCells(
  matrix: readonly (readonly (number | null)[])[],
): readonly CompositionCell[] {
  const cells: CompositionCell[] = [];
  for (const [row, masks] of matrix.entries()) {
    for (const [col, maskIndex] of masks.entries()) {
      if (maskIndex === null) continue;
      cells.push(
        doubleFilledSouthCrossJunctionMatrixCell(
          maskIndex as DoubleFilledSouthCrossJunctionMatrixMask,
          col,
          row,
        ),
      );
...
async function doubleFilledSouthCrossJunctionMatrixWindow(
  options: CliOptions,
  matrix: readonly (readonly (number | null)[])[],
  x: number,
  y: number,
  ...
  return compositionWindow(
    options,
    doubleFilledSouthCrossJunctionMatrixCells(matrix),
    matrix[0].length,
    matrix.length,
    x,
    y,
...
function singleOpenNorthwestCrossJunctionCandidateCell(
  col: number,
  row: number,
  layer: 'base' | 'upper' | 'composed' = 'composed',
): CompositionCell {
  const candidate =
    EQUAL_HEIGHT_SINGLE_OPEN_NORTHWEST_CROSS_JUNCTION_GATE.candidate;
  if (layer === 'base') return [col, row, candidate.baseFile, null];
  if (layer === 'upper') {
    return [col, row, EMPTY_WORKBENCH_FILE, candidate.upperFile];
  }
...
function singleOpenNorthwestCrossJunctionMatrixCell(
  maskIndex: EqualHeightSingleOpenNorthwestCrossJunctionMatrixMask,
  col: number,
  row: number,
): CompositionCell {
  if (maskIndex === 33) {
    return singleOpenNorthwestCrossJunctionCandidateCell(col, row);
  }
  if (
    maskIndex === 16 ||
    maskIndex === 20 ||
  ...
  if (maskIndex === 38) {
    ...
    if (!candidate) {
      throw new Error(
        'Accepted horizontal repeat source missing for mask_38',
      );
    }
    return thickWallHorizontalRepeatCandidateCell(candidate, col, row);
  }
...
function singleOpenNorthwestCrossJunctionMatrixCells(
  matrix: readonly (readonly (number | null)[])[],
): readonly CompositionCell[] {
  const cells: CompositionCell[] = [];
  for (const [row, masks] of matrix.entries()) {
    for (const [col, maskIndex] of masks.entries()) {
      if (maskIndex === null) continue;
      cells.push(
        singleOpenNorthwestCrossJunctionMatrixCell(
          maskIndex as EqualHeightSingleOpenNorthwestCrossJunctionMatrixMask,
          col,
          row,
        ),
      );
...
async function singleOpenNorthwestCrossJunctionMatrixWindow(
  options: CliOptions,
  matrix: readonly (readonly (number | null)[])[],
  x: number,
  y: number,
  ...
  return compositionWindow(
    options,
    singleOpenNorthwestCrossJunctionMatrixCells(matrix),
    matrix[0].length,
    matrix.length,
    x,
    y,
...
function fullyFilledCrossJunctionCandidateCell(
  col: number,
  row: number,
  layer: 'base' | 'upper' | 'composed' = 'composed',
): CompositionCell {
  ...
  if (layer === 'base') {
    return [col, row, candidate.baseFile, null];
  }
  if (layer === 'upper') {
    return [col, row, EMPTY_WORKBENCH_FILE, candidate.upperFile];
  }
...
function fullyFilledCrossJunctionMatrixCell(
  maskIndex: EqualHeightFullyFilledCrossJunctionMatrixMask,
  col: number,
  row: number,
): CompositionCell {
  if (maskIndex === 46) {
    return fullyFilledCrossJunctionCandidateCell(col, row);
  }
  if (
    maskIndex === 16 ||
    maskIndex === 20 ||
...
function fullyFilledCrossJunctionMatrixCells(
  ...
  for (const [row, masks] of matrix.entries()) {
    for (const [col, maskIndex] of masks.entries()) {
      cells.push(
        fullyFilledCrossJunctionMatrixCell(
          maskIndex as EqualHeightFullyFilledCrossJunctionMatrixMask,
          col,
          row,
        ),
      );
...
async function fullyFilledCrossJunctionMatrixWindow(
  options: CliOptions,
  matrix: readonly (readonly number[])[],
  x: number,
  y: number,
  ...
  return compositionWindow(
    options,
    fullyFilledCrossJunctionMatrixCells(matrix),
    matrix[0].length,
    matrix.length,
    x,
    y,
...
function singleOpenNortheastCrossJunctionCandidateCell(
  col: number,
  row: number,
  layer: 'base' | 'upper' | 'composed' = 'composed',
): CompositionCell {
  const candidate =
    EQUAL_HEIGHT_SINGLE_OPEN_NORTHEAST_CROSS_JUNCTION_GATE.candidate;
  if (layer === 'base') {
    return [col, row, candidate.baseFile, null, candidate.transform];
  }
  if (layer === 'upper') {
...
function singleOpenNortheastCrossJunctionMatrixCell(
  maskIndex: EqualHeightSingleOpenNortheastCrossJunctionMatrixMask,
  col: number,
  row: number,
): CompositionCell {
  if (maskIndex === 45) {
    return singleOpenNortheastCrossJunctionCandidateCell(col, row);
  }
  if (
    maskIndex === 16 ||
    maskIndex === 20 ||
...
function singleOpenNortheastCrossJunctionMatrixCells(
  matrix: readonly (readonly (number | null)[])[],
): readonly CompositionCell[] {
  const cells: CompositionCell[] = [];
  for (const [row, masks] of matrix.entries()) {
    for (const [col, maskIndex] of masks.entries()) {
      if (maskIndex === null) continue;
      cells.push(
        singleOpenNortheastCrossJunctionMatrixCell(
          maskIndex as EqualHeightSingleOpenNortheastCrossJunctionMatrixMask,
          col,
          row,
        ),
      );
...
async function singleOpenNortheastCrossJunctionMatrixWindow(
  options: CliOptions,
  matrix: readonly (readonly (number | null)[])[],
  x: number,
  y: number,
  ...
  return compositionWindow(
    options,
    singleOpenNortheastCrossJunctionMatrixCells(matrix),
    matrix[0].length,
    matrix.length,
    x,
    y,
...
type SingleOpenSouthwestCrossJunctionMatrixMask =
  | 1 | 2 | 4 | 5 | 8 | 10
  | 16 | 17 | 18 | 20 | 26 | 31 | 32 | 34 | 41 | 42 | 43;

function singleOpenSouthwestCrossJunctionCandidateCell(
  col: number,
  row: number,
  layer: 'base' | 'upper' | 'composed' = 'composed',
): CompositionCell {
  const candidate =
    EQUAL_HEIGHT_SINGLE_OPEN_SOUTHWEST_CROSS_JUNCTION_GATE.candidate;
  if (layer === 'base') return [col, row, candidate.baseFile, null];
  if (layer === 'upper') {
    return [col, row, EMPTY_WORKBENCH_FILE, candidate.upperFile];
  }
...
function singleOpenSouthwestCrossJunctionMatrixCell(
  maskIndex: SingleOpenSouthwestCrossJunctionMatrixMask,
  col: number,
  row: number,
): CompositionCell {
  if (maskIndex === 41) {
    return singleOpenSouthwestCrossJunctionCandidateCell(col, row);
  }
  if (
    maskIndex === 16 ||
    maskIndex === 20 ||
  ...
  if (maskIndex === 31) {
    ...
    if (!candidate) {
      throw new Error(
        'Accepted horizontal repeat source missing for mask_31',
      );
    }
    return thickWallHorizontalRepeatCandidateCell(candidate, col, row);
  }
...
function singleOpenSouthwestCrossJunctionMatrixCells(
  matrix: readonly (readonly (number | null)[])[],
): readonly CompositionCell[] {
  const cells: CompositionCell[] = [];
  for (const [row, masks] of matrix.entries()) {
    for (const [col, maskIndex] of masks.entries()) {
      if (maskIndex === null) continue;
      cells.push(
        singleOpenSouthwestCrossJunctionMatrixCell(
          maskIndex as SingleOpenSouthwestCrossJunctionMatrixMask,
          col,
          row,
        ),
      );
...
async function singleOpenSouthwestCrossJunctionMatrixWindow(
  options: CliOptions,
  matrix: readonly (readonly (number | null)[])[],
  x: number,
  y: number,
  ...
  return compositionWindow(
    options,
    singleOpenSouthwestCrossJunctionMatrixCells(matrix),
    matrix[0].length,
    matrix.length,
    x,
    y,
...
type SingleOpenSoutheastCrossJunctionMatrixMask =
  | 1 | 2 | 4 | 5 | 8 | 10
  | 16 | 20 | 24 | 25 | 26 | 31 | 32 | 34 | 35 | 36 | 44;

function singleOpenSoutheastCrossJunctionCandidateCell(
  col: number,
  row: number,
  layer: 'base' | 'upper' | 'composed' = 'composed',
): CompositionCell {
  const candidate =
    EQUAL_HEIGHT_SINGLE_OPEN_SOUTHEAST_CROSS_JUNCTION_GATE.candidate;
  if (layer === 'base') {
    return [col, row, candidate.baseFile, null, candidate.transform];
  }
  if (layer === 'upper') {
...
function singleOpenSoutheastCrossJunctionMatrixCell(
  maskIndex: SingleOpenSoutheastCrossJunctionMatrixMask,
  col: number,
  row: number,
): CompositionCell {
  if (maskIndex === 44) {
    return singleOpenSoutheastCrossJunctionCandidateCell(col, row);
  }
  if (
    maskIndex === 16 ||
    maskIndex === 20 ||
...
function singleOpenSoutheastCrossJunctionMatrixCells(
  matrix: readonly (readonly (number | null)[])[],
): readonly CompositionCell[] {
  const cells: CompositionCell[] = [];
  for (const [row, masks] of matrix.entries()) {
    for (const [col, maskIndex] of masks.entries()) {
      if (maskIndex === null) continue;
      cells.push(
        singleOpenSoutheastCrossJunctionMatrixCell(
          maskIndex as SingleOpenSoutheastCrossJunctionMatrixMask,
          col,
          row,
        ),
      );
...
async function singleOpenSoutheastCrossJunctionMatrixWindow(
  options: CliOptions,
  matrix: readonly (readonly (number | null)[])[],
  x: number,
  y: number,
  ...
  return compositionWindow(
    options,
    singleOpenSoutheastCrossJunctionMatrixCells(matrix),
    matrix[0].length,
    matrix.length,
    x,
    y,
...
type DoubleFilledDiagonalCrossJunctionMatrixMask =
  | 1 | 2 | 4 | 5 | 8 | 10
  | 16 | 20 | 21 | 22 | 26 | 30 | 34 | 35 | 36;

function doubleFilledDiagonalCrossJunctionCandidateCell(
  col: number,
  row: number,
  layer: 'base' | 'upper' | 'composed' = 'composed',
): CompositionCell {
...
function doubleFilledDiagonalCrossJunctionMatrixCell(
  maskIndex: DoubleFilledDiagonalCrossJunctionMatrixMask,
  col: number,
  row: number,
  verticalFacing: EqualHeightDoubleFilledDiagonalCrossJunctionVerticalFacing,
): CompositionCell {
  if (maskIndex === 30) {
    return doubleFilledDiagonalCrossJunctionCandidateCell(col, row);
  }
  if (
    maskIndex === 16 ||
    maskIndex === 20 ||
  ...
  if (maskIndex === 5) {
    return [
      ...
      'full_w_straight-base.svg',
      'full_w_straight-upper.svg',
      verticalFacing === 'east' ? 'mirror-x' : 'none',
    ];
  }
  return [col, row, 'full_n_straight-base.svg', 'full_n_straight-upper.svg'];
}

function doubleFilledDiagonalCrossJunctionMatrixCells(
  matrix: readonly (readonly (number | null)[])[],
): readonly CompositionCell[] {
  const cells: CompositionCell[] = [];
  for (const [row, masks] of matrix.entries()) {
    for (const [col, maskIndex] of masks.entries()) {
      if (maskIndex === null) continue;
      cells.push(
        doubleFilledDiagonalCrossJunctionMatrixCell(
          maskIndex as DoubleFilledDiagonalCrossJunctionMatrixMask,
          col,
          row,
          doubleFilledDiagonalCrossJunctionVerticalFacing(
            row,
...
async function doubleFilledDiagonalCrossJunctionMatrixWindow(
  options: CliOptions,
  matrix: readonly (readonly (number | null)[])[],
  x: number,
  y: number,
  ...
  return compositionWindow(
    options,
    doubleFilledDiagonalCrossJunctionMatrixCells(matrix),
    matrix[0].length,
    matrix.length,
    x,
    y,
...
type DoubleFilledOppositeDiagonalCrossJunctionMatrixMask =
  | 1 | 2 | 4 | 5 | 8 | 10
  | 16 | 17 | 18 | 20 | 26 | 27 | 28 | 34 | 40;

function doubleFilledOppositeDiagonalCrossJunctionCandidateCell(
  col: number,
  row: number,
  layer: 'base' | 'upper' | 'composed' = 'composed',
  filtered = true,
  ...
  const source =
    EQUAL_HEIGHT_DOUBLE_FILLED_DIAGONAL_CROSS_JUNCTION_GATE.candidate;
  const baseFile = filtered
    ? DOUBLE_FILLED_OPPOSITE_DIAGONAL_FILTERED_BASE_FILE
    : source.baseFile;
  const upperFile = filtered
    ? DOUBLE_FILLED_OPPOSITE_DIAGONAL_FILTERED_UPPER_FILE
    : source.upperFile;
  if (layer === 'base') return [col, row, baseFile, null, 'mirror-x'];
  if (layer === 'upper') {
...
function doubleFilledOppositeDiagonalCrossJunctionMatrixCell(
  maskIndex: DoubleFilledOppositeDiagonalCrossJunctionMatrixMask,
  col: number,
  row: number,
  verticalFacing: ReturnType<
    typeof doubleFilledOppositeDiagonalCrossJunctionVerticalFacing
  >,
): CompositionCell {
  if (maskIndex === 40) {
    return doubleFilledOppositeDiagonalCrossJunctionCandidateCell(col, row);
  }
  if (
    maskIndex === 16 ||
    maskIndex === 20 ||
...
function doubleFilledOppositeDiagonalCrossJunctionMatrixCells(
  matrix: readonly (readonly (number | null)[])[],
): readonly CompositionCell[] {
  const cells: CompositionCell[] = [];
  for (const [row, masks] of matrix.entries()) {
    for (const [col, maskIndex] of masks.entries()) {
      if (maskIndex === null) continue;
      cells.push(
        doubleFilledOppositeDiagonalCrossJunctionMatrixCell(
          maskIndex as DoubleFilledOppositeDiagonalCrossJunctionMatrixMask,
          col,
          row,
          doubleFilledOppositeDiagonalCrossJunctionVerticalFacing(
            row,
...
async function doubleFilledOppositeDiagonalCrossJunctionMatrixWindow(
  options: CliOptions,
  matrix: readonly (readonly (number | null)[])[],
  x: number,
  y: number,
  ...
  return compositionWindow(
    options,
    doubleFilledOppositeDiagonalCrossJunctionMatrixCells(matrix),
    matrix[0].length,
    matrix.length,
    x,
    y,
...
function singleFilledSouthwestCrossJunctionCandidateCell(
  col: number,
  row: number,
  layer: 'base' | 'upper' | 'composed' = 'composed',
): CompositionCell {
  const candidate =
    EQUAL_HEIGHT_SINGLE_FILLED_SOUTHWEST_CROSS_JUNCTION_GATE.candidate;
  if (layer === 'base') {
    return [col, row, candidate.baseFile, null, candidate.transform];
  }
  if (layer === 'upper') {
...
function singleFilledSouthwestCrossJunctionMatrixCell(
  maskIndex: SingleFilledSouthwestCrossJunctionMatrixMask,
  col: number,
  row: number,
): CompositionCell {
  if (maskIndex === 29) {
    return singleFilledSouthwestCrossJunctionCandidateCell(col, row);
  }
  if (maskIndex === 16 || maskIndex === 20 || maskIndex === 34) {
    return acceptedFilledBlockCell(maskIndex, col, row);
  }
  if (maskIndex === 22) {
    return horizontalPartialTJunctionCandidateCell(maskIndex, col, row);
...
function singleFilledSouthwestCrossJunctionMatrixCells(
  matrix: readonly (readonly (number | null)[])[],
): readonly CompositionCell[] {
  const cells: CompositionCell[] = [];
  for (const [row, masks] of matrix.entries()) {
    for (const [col, maskIndex] of masks.entries()) {
      if (maskIndex === null) continue;
      cells.push(singleFilledSouthwestCrossJunctionMatrixCell(
        maskIndex as SingleFilledSouthwestCrossJunctionMatrixMask,
        col,
        row,
      ));
    }
...
async function singleFilledSouthwestCrossJunctionMatrixWindow(
  options: CliOptions,
  matrix: readonly (readonly (number | null)[])[],
  x: number,
  y: number,
  ...
  return compositionWindow(
    options,
    singleFilledSouthwestCrossJunctionMatrixCells(matrix),
    matrix[0].length,
    matrix.length,
    x,
    y,
...
const installedMassFrame = (
  ...
  const left = x - offset;
  const top = y - offset;
  const right = x + cellSize * columns + offset;
  const bottom = y + cellSize * rows + offset;
  const bracket = Math.max(10, cellSize * 0.32);
  return `<path d="M${left + bracket} ${top}H${left}V${top + bracket}` +
    ` M${right - bracket} ${top}H${right}V${top + bracket}` +
    ` M${left} ${bottom - bracket}V${bottom}H${left + bracket}` +
...
async function renderEqualHeightDoubleFilledDiagonalCrossJunctionProposal(
  options: CliOptions,
  root: string,
): Promise<void> {
  const gate = EQUAL_HEIGHT_DOUBLE_FILLED_DIAGONAL_CROSS_JUNCTION_GATE;
  const width = 1600;
  const height = 3310;
  const panelFill = '#ECE5D5';
  const panel = (x: number, y: number, w: number, h: number): string =>
  ...
  const parts: string[] = [
    ...
    text(
      24,
      38,
      'QUOTACO EQUAL-HEIGHT WALLS — NE/SW DIAGONAL-FILLED CROSS-JUNCTION',
      23,
      820,
    ),
    text(
      24,
      68,
      'OWNER ACCEPTED · one new fixed-view union for mask_30; direct proof-layer source, no production registration',
      13,
      650,
      MUTED,
    ),
    text(1576, 38, 'MASK_30 · ACCEPTED', 11, 820, '#4E7D79', 'end'),
    text(
      1576,
      62,
      'accepted ledger · 28 direct · 19 derived · 0 synthetic',
      10,
      700,
      MUTED,
      'end',
    ...
    panel(20, 92, 1560, 520),
    text(
      44,
      126,
      'STATE DIAMOND — TWO OPPOSITE CROOKS CLOSE; TWO REMAIN FLOOR',
      14,
      820,
    ),
    text(
      44,
      150,
      'mask_15 governs the hub. mask_19 supplies the NE register and mask_29 supplies only the SW closure law; neither source is stacked.',
      10,
      650,
      MUTED,
    ),

    panel(20, 632, 1560, 620),
    text(
      44,
      666,
      'SOURCE READ — ONE BASE / UPPER UNION AT 240; COMPOSED AT 90 / 40',
      14,
      820,
    ),
    text(
      44,
      690,
      'One S-shaped cream owner reaches every socket. Coral, green, and foreground shade survive only around the open southeast return.',
      10,
      650,
      MUTED,
    ),

    panel(20, 1272, 1560, 620),
    text(
      44,
      1306,
      'COMPACT INSTALLED MASS — BOTH DIAGONAL FILLS MUST READ AT ONE CELL',
      14,
      820,
    ),
    text(
      44,
      1330,
      'Exact occupancy: floor / 20 / 26; 20 / 30 / 34; 16 / 34 / floor. Teal brackets mark the two overlapping 2×2 wall masses.',
      10,
      650,
      MUTED,
    ),

    panel(20, 1912, 1560, 900),
    text(
      44,
      1946,
      'EXTENT GATE — SAME SOURCE WITH THREE-CELL AND SIX-CELL ARMS',
      14,
      820,
    ),
    text(
      44,
      1970,
      'Long runs may not conceal a pinched hourglass, center ridge, doubled belt, socket gap, or lost northwest/southeast floor crook.',
      10,
      650,
      MUTED,
    ),

    panel(20, 2832, 1560, 454),
    text(44, 2868, 'ACCEPTANCE CONTRACT', 14, 820),
    text(824, 2868, 'PROOF BOUNDARY', 14, 820),
  ];
  ...
  > = [
    ...
    [
      ...
      430,
      'MASK_19',
      'NE FILLED · REGISTER LAW',
    ],
    [
      singleFilledSouthwestCrossJunctionCandidateCell(0, 0),
      800,
      'MASK_29',
      'SW FILLED · CLOSURE LAW',
    ],
    [
      doubleFilledDiagonalCrossJunctionCandidateCell(0, 0),
      1170,
      'MASK_30',
      'NE + SW · ACCEPTED',
    ],
  ];
  for (const [cell, x, label, caption] of controls) {
    const isCandidate = label === 'MASK_30';
    parts.push(
      text(
        x + 120,
        184,
    ...
    parts.push(
      isCandidate
        ? crossSourceCellFrame(x, 204, 240)
        : crossAcceptedFrame(x, 204, 240),
    );
    parts.push(
      text(
        x + 120,
  ...
  parts.push(
    text(
      800,
      534,
      'AUTHOR ONE MOLDED S-UNION · DO NOT OVERLAY MASK_19 + MASK_29',
      11,
      840,
      '#294B3C',
      'middle',
  ...
  parts.push(
    text(
      800,
      568,
      'NW + SE remain real floor crooks · mask_40 is the accepted filtered mirror-X derivation',
      10,
      760,
      '#9A493D',
      'middle',
  ...
  > = [
    ['base', 60, 'BASE · 240'],
    ['upper', 340, 'UPPER · 240'],
    ['composed', 620, 'COMPOSED · 240'],
  ];
  for (const [layer, x, label] of sourceCases) {
    parts.push(
      await compositionWindow(
        options,
        [doubleFilledDiagonalCrossJunctionCandidateCell(0, 0, layer)],
        1,
        1,
        x,
        728,
  ...
  parts.push(crossSourceCellFrame(920, 742, 90, 2));
  parts.push(
    await compositionWindow(
      options,
      [doubleFilledDiagonalCrossJunctionCandidateCell(0, 0)],
  ...
  parts.push(crossSourceCellFrame(1042, 767, 40, 1));
  parts.push(text(965, 852, '90 PX', 9, 820, '#B65F4D', 'middle'));
  parts.push(
    text(1062, 830, '40 PX · DARK', 9, 820, '#4E7D79', 'middle'),
  );
  const sourceNotes = [
    'one continuous S-shaped cream plane; no internal dividing ridge',
    'exact N / E / S / W sockets remain flush',
    'NW and SE keep the accepted mask_15 concave radii',
    'NE and SW contain no buried coral, green, or frontage rail',
    'only the SE return owns coral, green, shadow, and service ticks',
  ];
  ...
  parts.push(
    text(
      800,
      1184,
      'REJECT · TWO ELBOWS KISSING · CENTER PEAK · HOURGLASS PINCH · DUPLICATE MATERIAL BELTS',
      10,
      840,
      '#9A493D',
      'middle',
  ...
  parts.push(
    await doubleFilledDiagonalCrossJunctionMatrixWindow(
      options,
      gate.compactMatrix,
      80,
      1384,
  ...
  parts.push(crossInstalledUnionFrame(170, 1384, 90));
  parts.push(crossInstalledUnionFrame(80, 1474, 90));
  parts.push(crossSourceCellFrame(170, 1474, 90, 3));
  parts.push(
    text(
      500,
      1364,
  ...
  parts.push(crossInstalledUnionFrame(480, 1384, 40));
  parts.push(crossInstalledUnionFrame(440, 1424, 40));
  parts.push(crossSourceCellFrame(480, 1424, 40, 2));
  const compactNotes = [
    'NE wall mass: mask_20 / 26 / 30 / 34',
    'SW wall mass: mask_20 / 30 / 16 / 34',
    'both open crooks stay floor-shaped at 40 px',
    'the center reads as one connector, never two elbows',
    'adjacent accepted pieces meet without duplicate service seams',
  ];
  ...
  parts.push(
    text(
      700,
      1702,
      'PASS ONLY IF THE TWO OPEN CROOKS STAY DISTINCT WITHOUT PINCHING THE CREAM PLANE.',
      10,
      840,
      '#B65F4D',
    ),
  ...
  parts.push(
    text(
      80,
      1830,
      'TEAL = TWO INSTALLED 2×2 MASSES · CORAL = MASK_30 ACCEPTED SOURCE',
      9,
      820,
      '#4E7D79',
    ),
  ...
  parts.push(crossInstalledUnionFrame(160, 2110, 40));
  parts.push(crossInstalledUnionFrame(120, 2150, 40));
  parts.push(crossSourceCellFrame(160, 2150, 40, 2));
  parts.push(
    text(
      630,
      2010,
      '6-CELL ARMS · 40 PX/CELL · LIGHT',
      10,
      840,
      '#4E7D79',
      'middle',
  ...
  parts.push(crossInstalledUnionFrame(610, 2230, 40));
  parts.push(crossInstalledUnionFrame(570, 2270, 40));
  parts.push(crossSourceCellFrame(610, 2270, 40, 2));
  parts.push(
    text(
      1270,
      2010,
      '6-CELL ARMS · 40 PX/CELL · DARK',
      10,
      840,
      '#B65F4D',
      'middle',
  ...
  parts.push(crossInstalledUnionFrame(1250, 2230, 40));
  parts.push(crossInstalledUnionFrame(1210, 2270, 40));
  parts.push(crossSourceCellFrame(1250, 2270, 40, 2));
  parts.push(
    text(
      800,
      2634,
      'SAME MASK_30 SOURCE AT EVERY EXTENT · LONG ARMS MAY NOT CAMOUFLAGE A BAD CENTER',
      10,
      840,
      '#294B3C',
      'middle',
  ...
  parts.push(
    text(
      800,
      2680,
      'Local joins: 21 / 26 above; 22 / 30 / 35 center; 16 / 36 below',
      10,
      740,
      MUTED,
      'middle',
  ...
  const reviewLines = [
    'Judge one direct 128×128 base / upper source with no transform',
    'Keep exact N/E/S/W sockets, solid NE/SW, and open NW/SE',
    'Require one continuous S-shaped cream plane at 240/90/40 px',
    'Keep frontage shade and material belts off every top-to-top join',
    'Reject peaks, posts, patches, doubled seams, or an hourglass pinch',
  ];
  for (const [index, line] of reviewLines.entries()) {
    parts.push(
      text(
        44,
        2910 + index * 48,
  ...
  const boundaryLines: ReadonlyArray<readonly [string, string]> = [
    ['OWNER ACCEPTED · MASK_30 DIRECT PROOF SOURCE', '#4E7D79'],
    ['accepted ledger · 28 direct / 19 derived / 0 synthetic', MUTED],
    ['2 EXTERNAL PROOF SVGs · TEMPORARY FRAMES', '#4E7D79'],
    ['MASK_40 ACCEPTED SEPARATELY · FILTERED MIRROR-X DERIVATION', MUTED],
...
async function renderEqualHeightDoubleFilledOppositeDiagonalCrossJunctionProposal(
  options: CliOptions,
  root: string,
): Promise<void> {
  const gate =
  ...
  const parts: string[] = [
    ...
    text(
      24,
      38,
      'QUOTACO EQUAL-HEIGHT WALLS — NW/SE DIAGONAL-FILLED CROSS-JUNCTION',
      23,
      820,
    ),
    text(
      24,
      68,
      'OWNER ACCEPTED · mask_40 reuses accepted mask_30 through one filtered X mirror; only two duplicated boundary seams are omitted',
      13,
      650,
      MUTED,
    ),
    text(1576, 38, 'MASK_40 · ACCEPTED', 11, 820, '#4E7D79', 'end'),
    text(
      1576,
      62,
      'accepted ledger · 28 direct · 19 derived · 0 synthetic',
    ...
    panel(20, 92, 1560, 460),
    text(
      44,
      126,
      'STATE DIAMOND — ACCEPTED CONTROLS CONSTRAIN THE OPPOSITE-DIAGONAL DERIVATION',
      14,
      820,
    ),
    text(
      44,
      150,
      'mask_15 owns the open hub; mask_37 and mask_23 constrain the NW and SE closures; mask_30 remains the sole source pair.',
      10,
      650,
      MUTED,
    ),

    panel(20, 572, 1560, 510),
    text(
      44,
      606,
      'ACCEPTANCE EVIDENCE — RAW WHOLE-CELL CONTROL VS WEST-JOIN FILTER',
      14,
      820,
    ),
    text(
      44,
      630,
      'Both variants mirror the same complete 128×128 pair. The accepted derivation removes only base-boundary-seam and upper-boundary-seam before mirroring.',
      10,
      650,
      MUTED,
    ),

    panel(20, 1102, 1560, 570),
    text(
      44,
      1136,
      'FILTERED SOURCE READ — BASE / UPPER / COMPOSED AT 240; COMPOSED AT 90 / 40',
      14,
      820,
    ),
    text(
      44,
      1160,
      'The reverse-S cream owner stays intact. Fixed-light Y levels do not flip; only the lateral registers and the exposed SW frontage exchange sides.',
      10,
      650,
      MUTED,
    ),

    panel(20, 1692, 1560, 510),
    text(
      44,
      1726,
      'COMPACT INSTALLED MASS — CANONICAL THREE-BY-THREE OCCUPANCY',
      14,
      820,
    ),
    text(
      44,
      1750,
      'Exact matrix: 20 / 26 / floor; 16 / 40 / 26; floor / 16 / 34. Teal brackets contain the NW and SE 2×2 masses.',
      10,
      650,
      MUTED,
    ),

    panel(20, 2222, 1560, 780),
    text(
      44,
      2256,
      'EXTENT GATE — SAME FILTERED SOURCE WITH THREE-CELL AND SIX-CELL ARMS',
      14,
      820,
    ),
    text(
      44,
      2280,
      'North uses the east register; south uses the west register. Long arms may not hide a socket step, double seam, lost crook, or center pinch.',
      10,
      650,
      MUTED,
    ),

    panel(20, 3022, 1560, 520),
    text(44, 3058, 'ACCEPTED DERIVATION CONTRACT', 14, 820),
    text(824, 3058, 'PROOF BOUNDARY', 14, 820),
  ];
  ...
  > = [
    ...
    [
      doubleFilledOppositeDiagonalCrossJunctionCandidateCell(0, 0),
      1270,
      'MASK_40',
      'ACCEPTED DERIVATION',
    ],
  ...
  for (const [cell, x, label, caption] of controls) {
    const isAcceptedDerivation = label === 'MASK_40';
    parts.push(
      text(
        x + 110,
        184,
    ...
    parts.push(
      text(
        ...
        9,
        800,
        isAcceptedDerivation ? '#4E7D79' : MUTED,
        'middle',
      ),
    );
  }
  parts.push(
    text(
      800,
      510,
      'AUTHOR NOTHING NEW · MIRROR ONE MOLDED UNION · KEEP NE + SW AS REAL FLOOR',
      10,
      840,
      '#294B3C',
      'middle',
  ...
  parts.push(
    text(190, 672, 'RAW X MIRROR · CONTROL', 10, 840, '#9A493D', 'middle'),
  );
  parts.push(
    await compositionWindow(
      options,
      [
        doubleFilledOppositeDiagonalCrossJunctionCandidateCell(
          0,
          0,
          'composed',
          false,
  ...
  parts.push(crossSourceCellFrame(70, 692, 240));
  parts.push(
    text(
      480,
      672,
      'FILTERED X MIRROR · ACCEPTED',
      10,
      840,
      '#4E7D79',
      'middle',
  ...
  parts.push(crossSourceCellFrame(360, 692, 240));

  const rawWestJoinCells: readonly CompositionCell[] = [
  ...
  const filteredWestJoinCells: readonly CompositionCell[] = [
    horizontalPartialTJunctionCandidateCell(18, 0, 0),
    doubleFilledOppositeDiagonalCrossJunctionCandidateCell(1, 0),
  ];
  parts.push(text(770, 690, 'RAW WEST JOIN · 90 PX/CELL', 9, 820, '#9A493D', 'middle'));
  parts.push(
    await compositionWindow(
      options,
      rawWestJoinCells,
  ...
  parts.push(text(910, 944, 'SEAM ZOOM', 8, 800, '#9A493D', 'middle'));
  parts.push(
    text(
      1100,
      690,
      'FILTERED WEST JOIN · 90 PX/CELL',
      9,
      820,
      '#4E7D79',
      'middle',
  ...
  parts.push(text(1240, 944, 'SEAM ZOOM', 8, 800, '#4E7D79', 'middle'));
  const mirrorNotes = [
    'raw mirror puts its seam at x=2 beside mask_18 ownership at x=126',
    'filter removes only the two boundary-seam paths',
    'south service tick, arris, bands, shadow, and contact shade remain',
    'the filtered mirror is the accepted proof-layer derivation for mask_40',
  ];
  for (const [index, note] of mirrorNotes.entries()) {
    parts.push(
      text(
        1300,
        706 + index * 62,
  ...
  parts.push(
    text(
      800,
      1038,
      'PASS ONLY IF THE WEST JOIN READS ONCE WITHOUT CHANGING THE SOURCE SILHOUETTE.',
      10,
      840,
      '#B65F4D',
      'middle',
  ...
  readonly ['base' | 'upper' | 'composed', number, string]
  > = [
    ['base', 60, 'FILTERED BASE · 240'],
    ['upper', 340, 'FILTERED UPPER · 240'],
    ['composed', 620, 'FILTERED COMPOSED · 240'],
  ];
  for (const [layer, x, label] of sourceCases) {
    parts.push(
      await compositionWindow(
  ...
  parts.push(crossSourceCellFrame(930, 1208, 90, 2));
  parts.push(
    await compositionWindow(
      options,
      [doubleFilledOppositeDiagonalCrossJunctionCandidateCell(0, 0)],
  ...
  parts.push(crossSourceCellFrame(1050, 1233, 40, 1));
  parts.push(text(975, 1318, '90 PX', 9, 820, '#B65F4D', 'middle'));
  parts.push(text(1070, 1296, '40 PX · DARK', 9, 820, '#4E7D79', 'middle'));
  const sourceNotes = [
    'one reverse-S cream plane; NW + SE remain solid',
    'NE + SW retain the accepted concave floor radii',
    'north socket is east-register; south socket is west-register',
    'south-facing cream / coral / green / shadow exists on SW only',
    'no buried fascia, duplicate belt, center peak, patch, or post',
  ];
  for (const [index, note] of sourceNotes.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      800,
      1618,
      'FIXED LIGHT MEANS MIRROR X ONLY · NEVER FLIP Y OR MOVE THE SOUTH-FACE STACK ONTO A TOP JOIN',
      10,
      840,
      '#9A493D',
      'middle',
  ...
  parts.push(text(215, 1784, '90 PX/CELL · LIGHT', 10, 840, '#B65F4D', 'middle'));
  parts.push(
    await doubleFilledOppositeDiagonalCrossJunctionMatrixWindow(
      options,
      gate.compactMatrix,
      80,
      1804,
  ...
  parts.push(installedMassFrame(80, 1804, 2, 2, 90));
  parts.push(installedMassFrame(170, 1894, 2, 2, 90));
  parts.push(crossSourceCellFrame(170, 1894, 90, 3));
  parts.push(text(500, 1784, '40 PX/CELL · DARK', 10, 840, '#4E7D79', 'middle'));
  parts.push(
    await doubleFilledOppositeDiagonalCrossJunctionMatrixWindow(
      options,
      gate.compactMatrix,
  ...
  parts.push(installedMassFrame(440, 1804, 2, 2, 40));
  parts.push(installedMassFrame(480, 1844, 2, 2, 40));
  parts.push(crossSourceCellFrame(480, 1844, 40, 2));
  const compactNotes = [
    'NW mass: mask_20 / 26 / 16 / 40',
    'SE mass: mask_40 / 26 / 16 / 34',
    'NE and SW remain unmistakable floor at 40 px',
    'mask_18 owns the west boundary service seam',
    'all four sockets disappear flush into accepted neighbors',
  ];
  for (const [index, note] of compactNotes.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      690,
      2110,
      'PASS ONLY IF ONE CONNECTOR OWNS THE CENTER AND BOTH OPEN CROOKS STAY DISTINCT.',
      10,
      840,
      '#B65F4D',
    ),
  ...
  parts.push(text(180, 2320, '3-CELL ARMS · 40 PX/CELL · LIGHT', 10, 840, '#B65F4D', 'middle'));
  parts.push(
    await doubleFilledOppositeDiagonalCrossJunctionMatrixWindow(
      options,
      gate.threeCellArmMatrix,
  ...
  parts.push(crossSourceCellFrame(160, 2460, 40, 2));
  parts.push(text(630, 2320, '6-CELL ARMS · 40 PX/CELL · LIGHT', 10, 840, '#4E7D79', 'middle'));
  parts.push(
    await doubleFilledOppositeDiagonalCrossJunctionMatrixWindow(
      options,
      gate.sixCellArmMatrix,
  ...
  parts.push(crossSourceCellFrame(610, 2580, 40, 2));
  parts.push(text(1270, 2320, '6-CELL ARMS · 40 PX/CELL · DARK', 10, 840, '#B65F4D', 'middle'));
  parts.push(
    await doubleFilledOppositeDiagonalCrossJunctionMatrixWindow(
      options,
      gate.sixCellArmMatrix,
  ...
  parts.push(crossSourceCellFrame(1250, 2580, 40, 2));
  parts.push(
    text(
      800,
      2910,
      'SAME ACCEPTED MASK_40 DERIVATION AT EVERY EXTENT · NORTH EAST-REGISTER / SOUTH WEST-REGISTER',
      10,
      840,
      '#294B3C',
      'middle',
  ...
  parts.push(
    text(
      800,
      2954,
      'Local joins: 20 / 27 above · 18 / 40 / 28 center · 17 / 34 below',
      10,
      740,
      MUTED,
      'middle',
  ...
  const acceptedLines = [
    'Lock the filtered whole-cell X mirror of accepted mask_30',
    'Keep exact N/E/S/W sockets, solid NW/SE, and open NE/SW',
    'Require one continuous reverse-S cream owner at 240/90/40 px',
    'Keep mask_18 as the sole owner of the west boundary service seam',
    'Reject a Y flip, duplicate belt, center peak, patch, post, or pinch',
  ];
  for (const [index, line] of acceptedLines.entries()) {
    parts.push(
      text(
        44,
        3102 + index * 58,
  ...
  const boundaryLines: ReadonlyArray<readonly [string, string]> = [
    ['OWNER ACCEPTED · MASK_40 DERIVED PROOF SOURCE', '#4E7D79'],
    ['accepted ledger · 28 direct / 19 derived / 0 synthetic', MUTED],
    ['SOURCE: ACCEPTED MASK_30 + WHOLE-CELL MIRROR-X', '#4E7D79'],
    ['ACCEPTED FILTER: 2 BOUNDARY-SEAM PATHS · NO NEW SVG', '#4E7D79'],
    ['NO CANONICAL / EXPORT / ATLAS / SCHEMA / BLOB / UNITY', '#9A493D'],
  ];
  for (const [index, [line, color]] of boundaryLines.entries()) {
    parts.push(
...
async function renderEqualHeightSingleOpenNorthwestCrossJunctionGate(
  options: CliOptions,
  root: string,
): Promise<void> {
  const gate =
  ...
  const parts: string[] = [
    ...
    text(
      24,
      38,
      'QUOTACO EQUAL-HEIGHT WALLS — MASK_33 SINGLE-OPEN NORTHWEST GATE',
      22,
      820,
    ),
    text(
      24,
      68,
      'OWNER ACCEPTED · one flattened fixed-view base / upper source · no transform or derivation',
      13,
      650,
      MUTED,
    ),
    text(1576, 38, 'MASK_33 · ACCEPTED DIRECT', 11, 820, '#4E7D79', 'end'),
    text(
      1576,
      62,
      'accepted ledger · 28 direct · 19 derived · 0 synthetic',
    ...
    text(
      44,
      126,
      'GEOMETRY CONTROLS — MASKS 25 / 32 / 30 ARE CUES, NEVER A SOURCE STACK',
      14,
      820,
    ),
    text(
      44,
      150,
      'The accepted piece is one direct fixed-view source. Accepted mask_41 is shown only as the same-class three-filled-crook comparison; no X or Y mirror is used.',
      10,
      650,
      MUTED,
    ),

    panel(20, 612, 1560, 570),
    text(
      44,
      646,
      'ISOLATED READ — BASE / UPPER / COMPOSED AT 240; COMPOSED AT 90 AND 40 ON BOTH GROUNDS',
      14,
      820,
    ),
    text(
      44,
      670,
      'One cream owner fills northeast, southeast, and southwest. Only the northwest crook exposes a restrained reveal and arris; no local coral or green is repainted.',
      10,
      650,
      MUTED,
    ),

    panel(20, 1202, 1560, 550),
    text(
      44,
      1236,
      'COMPACT OCCUPANCY — EXACT .## / ### / ### MASS WITH NORTHWEST OPEN',
      14,
      820,
    ),
    text(
      44,
      1260,
      'Exact matrix: floor / 20 / 26; 20 / 33 / 42; 16 / 38 / 34. Every non-center cell is already accepted.',
      10,
      650,
      MUTED,
    ),

    panel(20, 1772, 1560, 500),
    text(
      44,
      1806,
      'SOCKET + PERSPECTIVE AUDIT — EACH EDGE AGAINST ITS ACCEPTED NEIGHBOR',
      14,
      820,
    ),
    text(
      44,
      1830,
      'Direct west-facing vertical straights carry the long N/S arms. Accepted local neighbors must erase each socket without moving the center or introducing a dogleg.',
      10,
      650,
      MUTED,
    ),

    panel(20, 2292, 1560, 1180),
    text(
      44,
      2326,
      'EXTENT GATE — SAME CENTER AT 1 / 3 / 6-CELL CONNECTED RUNS, LIGHT AND DARK',
      14,
      820,
    ),
    text(
      44,
      2350,
      'Long arms may not hide a post, waist, peak, stair-step, duplicate outline, duplicate belt, socket shift, or northwest crook closure.',
      10,
      650,
      MUTED,
    ),

    panel(20, 3492, 1560, 500),
    text(44, 3528, 'ACCEPTED CONTRACT', 14, 820),
    text(824, 3528, 'BOUNDARY', 14, 820),
  ];

  const controls: ReadonlyArray<
    readonly [CompositionCell, number, string, string, 'control' | 'source' | 'comparison']
  > = [
    [
      doubleFilledEastCrossJunctionCandidateCell(0, 0),
      44,
      'MASK_25',
      'EAST SLAB CUE',
      'control',
    ...
    [
      doubleFilledSouthCrossJunctionCandidateCell(0, 0),
      264,
      'MASK_32',
      'SOUTH SLAB CUE',
      'control',
    ...
    [
      singleOpenNorthwestCrossJunctionCandidateCell(0, 0),
      704,
      'MASK_33',
      'ACCEPTED DIRECT',
      'source',
    ...
    [
      singleOpenSouthwestCrossJunctionCandidateCell(0, 0),
      924,
      'MASK_41',
      'DIRECT SAME-CLASS',
      'comparison',
  ...
  for (const [cell, x, label, subtitle, role] of controls) {
    const color = role === 'source' ? '#B65F4D' : '#4E7D79';
    parts.push(text(x + 90, 184, label, 9, 840, color, 'middle'));
    parts.push(
      await compositionWindow(
        options,
        [cell],
    ...
    parts.push(
      role === 'source'
        ? crossSourceCellFrame(x, 202, 180)
        : crossAcceptedFrame(x, 202, 180),
    );
    parts.push(text(x + 90, 408, subtitle, 8, 820, color, 'middle'));
  }
  const controlNotes = [
    '25 / 32 constrain buried east + south mass',
    '30 constrains only the open NW reveal + arris',
    '33 is one accepted flattened source, not overpainted controls',
    '41 checks catalog scale and same-class socket law',
  ];
  for (const [index, note] of controlNotes.entries()) {
    parts.push(
      text(
        1160,
        208 + index * 48,
  ...
  parts.push(
    text(
      800,
      548,
      'DIRECT FIXED VIEW · NO X MIRROR · NO Y MIRROR · NO STACKED CONTROL GEOMETRY',
      10,
      840,
      '#294B3C',
      'middle',
  ...
  const isolatedViews: ReadonlyArray<
    readonly ['base' | 'upper' | 'composed', number, string]
  > = [
    ['base', 54, 'BASE · 240'],
    ['upper', 314, 'UPPER · 240'],
  ...
  for (const [layer, x, label] of isolatedViews) {
    parts.push(
      await compositionWindow(
        options,
        [singleOpenNorthwestCrossJunctionCandidateCell(0, 0, layer)],
        1,
        1,
        x,
        704,
    ...
    parts.push(crossSourceCellFrame(x, 704, 240, layer === 'composed' ? 2 : 0));
    parts.push(
      text(
        x + 120,
        970,
  ...
  > = [
    [884, 724, 90, A1A_PALETTE.floor, '90 · LIGHT'],
    [994, 724, 90, A1A_PALETTE.charcoal, '90 · DARK'],
    [1104, 749, 40, A1A_PALETTE.floor, '40 · LIGHT'],
    [1164, 749, 40, A1A_PALETTE.charcoal, '40 · DARK'],
  ];
  for (const [x, y, size, floorFill, label] of distanceViews) {
    ...
    parts.push(crossSourceCellFrame(x, y, size, size === 40 ? 1 : 2));
    parts.push(
      text(
        x + size / 2,
        y + size + 24,
  ...
  const isolatedNotes = [
    'one cream shell owns all three filled crooks',
    'east and south remain one uninterrupted mass',
    'only NW carries a restrained cream reveal + arris',
    'source paths contain no coral or green',
    'NW radius and open floor survive at 40 px',
  ];
  for (const [index, note] of isolatedNotes.entries()) {
    parts.push(
      text(
        1240,
        716 + index * 50,
  ...
  parts.push(
    text(
      884,
      1138,
      'REJECT · CENTER POST · PEAK · HOURGLASS · STAIR-STEP · DOUBLED OUTLINE · DUPLICATE BELTS · CLOSED NW CROOK',
      9,
      840,
      '#9A493D',
    ),
  ...
  const compactViews: ReadonlyArray<
    readonly [number, number, number, string, string, number]
  > = [
    [54, 1318, 90, A1A_PALETTE.floor, '90 PX/CELL · LIGHT', 1294],
    [344, 1318, 90, A1A_PALETTE.charcoal, '90 PX/CELL · DARK', 1294],
    [654, 1393, 40, A1A_PALETTE.floor, '40 PX/CELL · LIGHT', 1368],
    [804, 1393, 40, A1A_PALETTE.charcoal, '40 PX/CELL · DARK', 1368],
  ];
  for (const [x, y, cellPixels, floorFill, label, labelY] of compactViews) {
    const matrixPixels = cellPixels * gate.compactMatrix.length;
    parts.push(
      text(
        x + matrixPixels / 2,
        labelY,
        ...
        9,
        840,
        floorFill === A1A_PALETTE.charcoal ? '#4E7D79' : '#B65F4D',
        'middle',
      ),
    );
    parts.push(
      await singleOpenNorthwestCrossJunctionMatrixWindow(
        options,
        gate.compactMatrix,
        x,
        y,
  ...
  const compactNotes = [
    'northwest cell remains genuine visible floor',
    'north join: mask_20 beside accepted mask_26',
    'east join: accepted mask_42',
    'south join: direct mask_38 between 16 and 34',
    'coral frame identifies only the accepted mask_33 source',
  ];
  for (const [index, note] of compactNotes.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      1000,
      1624,
      'PASS ONLY IF THREE FILLED CROOKS READ AS ONE MASS AND NW STILL READS AS FLOOR.',
      10,
      840,
      '#B65F4D',
    ),
  ...
  const socketAudits: ReadonlyArray<
    readonly [
  ...
  > = [
    [[[21], [33]], 74, 1882, 160, 320, 'N · MASK_21', 74, 2042],
    [[[22, 33]], 314, 1922, 320, 160, 'W · MASK_22', 474, 1922],
    [[[33, 43]], 744, 1922, 320, 160, 'E · MASK_43', 744, 1922],
    [[[33], [39]], 1256, 1882, 160, 320, 'S · MASK_39', 1256, 1882],
  ];
  for (
    const [matrix, x, y, w, h, label, candidateX, candidateY]
    of socketAudits
  ) {
    parts.push(text(x + w / 2, 1850, label, 9, 840, '#4E7D79', 'middle'));
    parts.push(
      await singleOpenNorthwestCrossJunctionMatrixWindow(
        options,
        matrix,
    ...
    const candidateSize = matrix.length === 1 ? w / 2 : h / 2;
    parts.push(
      crossSourceCellFrame(candidateX, candidateY, candidateSize, 2),
    );
  }
  parts.push(
    text(
      800,
      2216,
      'ACCEPTED NEIGHBORS MUST ERASE THE SOCKET, NOT CHANGE THE CENTER SILHOUETTE',
      10,
      840,
      '#294B3C',
      'middle',
  ...
  const extentRows: ReadonlyArray<
    readonly [number, string, string, string]
  > = [
    [2400, A1A_PALETTE.floor, 'LIGHT FLOOR', '#B65F4D'],
    [2940, A1A_PALETTE.charcoal, 'DARK FLOOR', '#4E7D79'],
  ];
  for (const [y, floorFill, groundLabel, labelColor] of extentRows) {
    parts.push(text(100, y, `1-CELL · 40 PX · ${groundLabel}`, 9, 840, labelColor, 'middle'));
    parts.push(
      await singleOpenNorthwestCrossJunctionMatrixWindow(
        options,
        gate.compactMatrix,
    ...
    parts.push(crossSourceCellFrame(80, y + 60, 40, 2));

    parts.push(text(350, y, `3-CELL ARMS · 40 PX · ${groundLabel}`, 9, 840, labelColor, 'middle'));
    parts.push(
      await singleOpenNorthwestCrossJunctionMatrixWindow(
        options,
        gate.threeCellArmMatrix,
    ...
    parts.push(crossSourceCellFrame(330, y + 140, 40, 2));

    parts.push(text(770, y, `6-CELL ARMS · 40 PX · ${groundLabel}`, 9, 840, labelColor, 'middle'));
    parts.push(
      await singleOpenNorthwestCrossJunctionMatrixWindow(
        options,
        gate.sixCellArmMatrix,
    ...
    parts.push(crossSourceCellFrame(750, y + 260, 40, 2));
    ...
    parts.push(crossSourceCellFrame(1290, y + 260, 40, 2));
    parts.push(
      text(
        1310,
        y,
        `6-CELL SOCKET DETAIL · ${groundLabel}`,
        9,
        840,
        labelColor,
        'middle',
  ...
  parts.push(
    text(
      800,
      3440,
      'SAME ACCEPTED MASK_33 SOURCE AT EVERY EXTENT · IMMEDIATE NEIGHBORS: 21 NORTH · 43 EAST · 22 WEST · 39 SOUTH',
      10,
      840,
      '#294B3C',
      'middle',
  ...
  const acceptedLines = [
    'Lock one flattened 128 by 128 base / upper source with no transform',
    'Keep exact N/E/S/W sockets, solid NE/SE/SW, and open NW',
    'Require one uninterrupted three-quarter cream mass at 240/90/40 px',
    'Keep the center invariant as accepted arms extend to 1 / 3 / 6 cells',
    'Reject a post, peak, hourglass, stair-step, duplicate outline, or closed crook',
  ];
  for (const [index, line] of acceptedLines.entries()) {
    parts.push(
      text(
  ...
  const boundaryLines: ReadonlyArray<readonly [string, string]> = [
    ['OWNER ACCEPTED · MASK_33 DIRECT PROOF SOURCE', '#4E7D79'],
    ['accepted ledger · 28 direct / 19 derived / 0 synthetic', MUTED],
    ['SOURCE: ONE EXTERNAL PROOF-LAYER BASE / UPPER PAIR', '#4E7D79'],
    ['NO MIRROR · NO CORAL / GREEN LOCAL PAINT', '#4E7D79'],
    ['NO PRODUCTION / EXPORT / ATLAS / SCHEMA / BLOB / UNITY', '#9A493D'],
  ];
  for (const [index, [line, color]] of boundaryLines.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      824,
      3890,
      'Owner acceptance locks proof-layer source and ledger provenance only; downstream registration remains separate.',
      9,
      720,
      MUTED,
    ),
...
async function renderEqualHeightFullyFilledCrossJunctionGate(
  options: CliOptions,
  root: string,
): Promise<void> {
  const gate =
  ...
  const parts: string[] = [
    ...
    text(
      24,
      38,
      'QUOTACO EQUAL-HEIGHT WALLS — MASK_46 FULLY FILLED CENTER GATE',
      22,
      820,
    ),
    text(
      24,
      68,
      'OWNER ACCEPTED · one flattened buried-center pair · no stack, mirror, filter, or downstream promotion',
      13,
      650,
      MUTED,
    ),
    text(1576, 38, 'MASK_46 · ACCEPTED', 11, 820, '#294B3C', 'end'),
    text(
      1576,
      62,
      'accepted ledger · 28 direct · 19 derived · 0 synthetic',
    ...
    text(
      44,
      126,
      'SOURCE DECISION — CLOSE THE LAST CROOK, THEN REMOVE EVERY BURIED FACE CUE',
      14,
      820,
    ),
    text(
      44,
      150,
      'Masks 33 and 45 constrain the last exposed returns. They are geometry controls only; neither is stacked into the accepted center.',
      10,
      650,
      MUTED,
    ),
    ...
    text(
      44,
      616,
      'SOURCE AUDIT — BASE / UPPER / COMPOSED AT 240; COMPOSED AT 90 AND 40',
      14,
      820,
    ),
    text(
      44,
      640,
      'The isolated square is inspection evidence only. In real occupancy every edge is buried by an accepted neighbor.',
      10,
      650,
      MUTED,
    ),

    panel(20, 1242, 1560, 900),
    text(
      44,
      1276,
      'MINIMUM + REPEAT AUDIT — EXACT 3×3 AND 4×4 SOLID MASSES, LIGHT AND DARK',
      14,
      820,
    ),
    text(
      44,
      1300,
      'The 4×4 mass is the critical test: four adjacent mask_46 cells must remain one cream plane without a charcoal lattice or coping belt.',
      10,
      650,
      MUTED,
    ),

    panel(20, 2162, 1560, 980),
    text(
      44,
      2196,
      'EXTENT GATE — 3×3 / 4×4 / 6×6 SOLID MASSES AT 40 PX PER CELL',
      14,
      820,
    ),
    text(
      44,
      2220,
      'No composition grid is drawn. Any line inside the cream field therefore belongs to the art and fails this gate.',
      10,
      650,
      MUTED,
    ),

    panel(20, 3162, 1560, 490),
    text(44, 3198, 'ACCEPTED CONTRACT', 14, 820),
    text(824, 3198, 'BOUNDARY', 14, 820),
  ];

  const sourceX = 44;
  const mirrorX = 284;
  const candidateX = 564;
  parts.push(
    await compositionWindow(
      options,
      [singleOpenNorthwestCrossJunctionCandidateCell(0, 0)],
  ...
  parts.push(crossAcceptedFrame(sourceX, 202, 200));
  parts.push(
    text(
      sourceX + 100,
      184,
      'MASK_33 · NW OPEN CONTROL',
      8,
      840,
      '#4E7D79',
      'middle',
  ...
  parts.push(
    await compositionWindow(
      options,
      [singleOpenNortheastCrossJunctionCandidateCell(0, 0)],
      1,
      1,
      mirrorX,
      202,
  ...
  parts.push(crossAcceptedFrame(mirrorX, 202, 200));
  parts.push(
    text(
      mirrorX + 100,
      184,
      'MASK_45 · NE OPEN CONTROL',
      8,
      840,
      '#4E7D79',
      'middle',
  ...
  parts.push(
    await compositionWindow(
      options,
      [fullyFilledCrossJunctionCandidateCell(0, 0)],
      1,
      1,
      candidateX,
      182,
  ...
  parts.push(crossSourceCellFrame(candidateX, 182, 240, 3));
  parts.push(
    text(
      candidateX + 120,
      164,
      'MASK_46 · ACCEPTED BURIED CENTER',
      9,
      840,
      '#B65F4D',
      'middle',
  ...
  parts.push(
    '<path d="M498 302H548" fill="none" stroke="#B65F4D" stroke-width="4" stroke-linecap="round"/>' +
    '<path d="M548 302L534 292V312Z" fill="#B65F4D"/>',
  );
  const decisionNotes = [
    'not a mirror: the fully occupied cell has no exposed return',
    'not a stack: overlapping masks 24/42 leave a 2 px charcoal seam',
    'no filter: there is no valid local contour or belt to preserve',
    'base owns one full-cell structural underlay',
    'upper owns one full-cell cream top and nothing else',
  ];
  for (const [index, note] of decisionNotes.entries()) {
    parts.push(
      text(
        860,
        200 + index * 50,
  ...
  parts.push(
    text(
      800,
      532,
      'ACCEPTED CROOK GEOMETRY CONVERGES ON ONE FULLY BURIED CELL · NO EXPOSED LIGHT / SHADOW / FACE HIERARCHY REMAINS',
      10,
      840,
      '#294B3C',
      'middle',
  ...
  > = [
    ['base', 44, A1A_PALETTE.floor, 'BASE · 240'],
    ['upper', 304, A1A_PALETTE.floor, 'UPPER · 240'],
    ['composed', 564, A1A_PALETTE.floor, '240 · LIGHT'],
    ['composed', 824, A1A_PALETTE.charcoal, '240 · DARK'],
  ];
  for (const [layer, x, floorFill, label] of sourceViews) {
    parts.push(
      await compositionWindow(
        options,
        [fullyFilledCrossJunctionCandidateCell(0, 0, layer)],
        1,
        1,
        x,
        674,
    ...
    parts.push(crossSourceCellFrame(x, 674, 240, layer === 'composed' ? 2 : 0));
    parts.push(
      text(
        x + 120,
        940,
  ...
  const distanceViews: ReadonlyArray<
    readonly [number, number, number, string, string]
  > = [
    [1094, 694, 90, A1A_PALETTE.floor, '90 · LIGHT'],
    [1204, 694, 90, A1A_PALETTE.charcoal, '90 · DARK'],
    [1314, 719, 40, A1A_PALETTE.floor, '40 · LIGHT'],
    [1374, 719, 40, A1A_PALETTE.charcoal, '40 · DARK'],
  ];
  for (const [x, y, size, floorFill, label] of distanceViews) {
    parts.push(
      await compositionWindow(
  ...
  const sourceNotes = [
    'the base is fully buried beneath the cream field',
    'the upper reaches all four canvas edges exactly',
    'no contour, highlight, coral, green, shade, plinth, or seam',
    'flatness is required here; visible catalog character comes from the perimeter',
    'reject added detail merely intended to decorate the large cream field',
  ];
  for (const [index, note] of sourceNotes.entries()) {
    parts.push(
      text(
        44 + (index % 2) * 780,
        1000 + Math.floor(index / 2) * 52,
        `• ${note}`,
        9,
        720,
        index < 4 ? '#294B3C' : '#B65F4D',
  ...
  parts.push(
    text(
      44,
      1188,
      'REJECT · INTERNAL CHARCOAL · COPING BELT · FASCIA · SHADE · POST · PEAK · HOURGLASS · STAIR-STEP · VIEWPORT HAIRLINE',
      9,
      840,
      '#9A493D',
    ),
  ...
  for (
    const [matrix, x, y, cellPixels, floorFill, label, labelY]
    of compactViews
  ) {
    const matrixPixels = cellPixels * matrix.length;
    parts.push(
      text(
        x + matrixPixels / 2,
        labelY,
    ...
    parts.push(
      await fullyFilledCrossJunctionMatrixWindow(
        options,
        matrix,
        x,
        y,
    ...
    const inset = cellPixels;
    const interior = matrix.length - 2;
    parts.push(
      `<rect x="${x + inset + 2}" y="${y + inset + 2}" ` +
      `width="${interior * cellPixels - 4}" height="${interior * cellPixels - 4}" ` +
      'fill="none" stroke="#B65F4D" stroke-width="2" stroke-dasharray="8 6" opacity="0.86"/>',
    );
  }
  parts.push(
    text(
      284,
      1682,
      'MINIMUM MATRIX · 20 31 26 / 24 46 42 / 16 38 34',
      9,
      840,
      '#294B3C',
      'middle',
  ...
  parts.push(crossSourceCellFrame(204, 1750, 160, 2));
  parts.push(
    text(
      284,
      1730,
      'W / E JOIN STRIP · MASK_24 / 46 / 42',
      9,
      840,
      '#4E7D79',
      'middle',
  ...
  parts.push(crossSourceCellFrame(584, 1820, 160, 2));
  parts.push(
    text(
      664,
      1640,
      'N / S JOIN STRIP · 31 / 46 / 38',
      9,
      840,
      '#4E7D79',
      'middle',
  ...
  const compactNotes = [
    'all eight immediate neighbors are already accepted',
    'outer corners: 20 / 26 / 16 / 34',
    'west / east sides: 24 / 42',
    'rear / foreground sides: 31 / 38',
    'dashed coral region identifies accepted source cells; it is not source art',
    'pass only if the cream center disappears into every join',
  ];
  for (const [index, note] of compactNotes.entries()) {
    parts.push(
      text(
  ...
  const extentRows: ReadonlyArray<
    readonly [number, string, string, string]
  > = [
    [2280, A1A_PALETTE.floor, 'LIGHT FLOOR', '#B65F4D'],
    [2700, A1A_PALETTE.charcoal, 'DARK FLOOR', '#4E7D79'],
  ];
  for (const [y, floorFill, groundLabel, labelColor] of extentRows) {
    const extents: ReadonlyArray<
      readonly [
        readonly (readonly number[])[],
        number,
        string,
    ...
    > = [
      [gate.threeByThreeMatrix, 44, '3×3 · 1 CENTER'],
      [gate.fourByFourMatrix, 214, '4×4 · 4 CENTERS'],
      [gate.sixBySixMatrix, 424, '6×6 · 16 CENTERS'],
    ];
    for (const [matrix, x, label] of extents) {
      const matrixPixels = matrix.length * 40;
      parts.push(
        text(
          x + matrixPixels / 2,
          y,
    ...
    const extentNotes = [
      'same filled_center source at every interior position',
      '4×4 is the adjacent-center seam gate',
      '6×6 is the repeated-plane and 40 px stability gate',
      'only the perimeter owns outlines and dimensional color bands',
      'no grid overlay is present to disguise or invent a seam',
    ];
    for (const [index, note] of extentNotes.entries()) {
      parts.push(
        text(
          760,
          y + 72 + index * 52,
  ...
  parts.push(
    text(
      800,
      3108,
      'PASS ONLY IF THE ENTIRE CREAM FIELD READS AS ONE MOLDED SLAB AND THE ACCEPTED SOUTH EDGE REMAINS THE SOLE CORAL / GREEN / SHADOW OWNER',
      10,
      840,
      '#294B3C',
      'middle',
  ...
  const acceptedLines = [
    'Lock one owner-accepted fixed-view filled_center base / upper pair',
    'Keep N/E/S/W connected and all four diagonal crooks solid',
    'Require exact full-cell cream continuity at 240 / 90 / 40 px',
    'Require stable 3×3 / 4×4 / 6×6 masses on both grounds',
    'Reject any internal face cue, lattice, belt, seam, post, or peak',
  ];
  for (const [index, line] of acceptedLines.entries()) {
    parts.push(
      text(
  ...
  const boundaryLines: ReadonlyArray<readonly [string, string]> = [
    ['PROOF-LAYER ACCEPTED · MASK_46 IS DIRECT', '#294B3C'],
    ['accepted ledger · 28 direct / 19 derived / 0 synthetic', MUTED],
    ['SOURCE: ACCEPTED EXTERNAL PROOF-LAYER PAIR', '#4E7D79'],
    ['NO STACK · NO MIRROR · NO FILTER', '#4E7D79'],
    ['NO PRODUCTION / EXPORT / ATLAS / SCHEMA / BLOB / UNITY', '#9A493D'],
  ];
  for (const [index, [line, color]] of boundaryLines.entries()) {
    parts.push(
  ...
  parts.push(
    text(
      824,
      3570,
      'Owner approval locks row 46 as one direct proof-layer source. Every production, export, atlas, schema, blob, and Unity boundary remains unchanged.',
      9,
      720,
      MUTED,
    ),
...
async function renderEqualHeightSingleOpenNortheastCrossJunctionGate(
  options: CliOptions,
  root: string,
): Promise<void> {
  const gate =
  ...
  const parts: string[] = [
    ...
    text(
      24,
      38,
      'QUOTACO EQUAL-HEIGHT WALLS — MASK_45 SINGLE-OPEN NORTHEAST GATE',
      22,
      820,
    ),
    text(
      24,
      68,
      'OWNER ACCEPTED · plain whole-cell X mirror of accepted mask_33 · no filter and no new SVG bank',
      13,
      650,
      MUTED,
    ),
    text(1576, 38, 'MASK_45 · ACCEPTED', 11, 820, '#4E7D79', 'end'),
    text(
      1576,
      62,
      'accepted ledger · 28 direct · 19 derived · 0 synthetic',
    ...
    text(
      44,
      126,
      'MIRROR DECISION — ACCEPTED MASK_33 SOURCE, ACCEPTED PLAIN X, AND ACCEPTED MASK_40 NE RETURN',
      14,
      820,
    ),
    text(
      44,
      150,
      'Mask_33 supplies the geometry. Mask_40 verifies the accepted northeast reveal / arris treatment; neither is stacked into mask_45.',
      10,
      650,
      MUTED,
    ),

    panel(20, 612, 1560, 620),
    text(
      44,
      646,
      'ISOLATED READ — BASE / UPPER PLUS COMPOSED 240 / 90 / 40 ON LIGHT AND DARK GROUNDS',
      14,
      820,
    ),
    text(
      44,
      670,
      'One mirrored cream owner fills northwest, southwest, and southeast. Only the northeast crook remains open floor.',
      10,
      650,
      MUTED,
    ),

    panel(20, 1252, 1560, 540),
    text(
      44,
      1286,
      'COMPACT OCCUPANCY — EXACT ##. / ### / ### MASS WITH NORTHEAST OPEN',
      14,
      820,
    ),
    text(
      44,
      1310,
      'Exact matrix: 20 / 26 / floor; 24 / 45 / 26; 16 / 38 / 34. Every non-center cell is already accepted.',
      10,
      650,
      MUTED,
    ),

    panel(20, 1812, 1560, 500),
    text(
      44,
      1846,
      'FOUR SOCKET AUDITS — ACCEPTED STRAIGHTS AT THE EXACT MIRRORED REGISTRATION',
      14,
      820,
    ),
    text(
      44,
      1870,
      'North / south use the east-registered vertical source; west / east use the shared horizontal source. Each join must erase the socket.',
      10,
      650,
      MUTED,
    ),

    panel(20, 2332, 1560, 1200),
    text(
      44,
      2366,
      'EXTENT GATE — SAME CENTER AT 1 / 3 / 6-CELL CONNECTED RUNS, LIGHT AND DARK',
    ...
    text(
      44,
      2390,
      'Long arms may not hide a center post, waist, peak, stair-step, duplicate outline, socket shift, or northeast crook closure.',
      10,
      650,
      MUTED,
    ),

    panel(20, 3552, 1560, 660),
    text(44, 3588, 'ACCEPTED CONTRACT', 14, 820),
    text(824, 3588, 'BOUNDARY', 14, 820),
  ];

  const sourceX = 44;
  const candidateX = 344;
  const controlX = 684;
  parts.push(
    await compositionWindow(
      options,
  ...
  parts.push(crossAcceptedFrame(sourceX, 202, 240));
  parts.push(
    text(
      sourceX + 120,
      184,
      'MASK_33 · ACCEPTED DIRECT SOURCE',
      9,
      840,
      '#4E7D79',
      'middle',
  ...
  parts.push(crossSourceCellFrame(candidateX, 202, 240, 3));
  parts.push(
    text(
      candidateX + 120,
      184,
      'MASK_45 · ACCEPTED PLAIN X',
      9,
      840,
      '#4E7D79',
      'middle',
  ...
  parts.push(
    '<path d="M300 322H332" fill="none" stroke="#B65F4D" stroke-width="4" stroke-linecap="round"/>' +
    '<path d="M332 322L318 312V332Z" fill="#B65F4D"/>',
  );
  parts.push(text(316, 304, 'X', 10, 900, '#B65F4D', 'middle'));
  parts.push(
    await compositionWindow(
      options,
      [doubleFilledOppositeDiagonalCrossJunctionCandidateCell(0, 0)],
  ...
  parts.push(crossAcceptedFrame(controlX, 222, 200));
  parts.push(
    text(
      controlX + 100,
      204,
      'MASK_40 · ACCEPTED NE RETURN CONTROL',
      8,
      840,
      '#4E7D79',
      'middle',
  ...
  const mirrorNotes = [
    'whole-cell matrix(-1 0 0 1 128 0)',
    'Y-based light and shadow hierarchy stays fixed',
    'NW reveal and arris move together to NE',
    'mask_33 contains no boundary-seam path to omit',
    'plain X is accepted at proof layer; no new source art',
  ];
  for (const [index, note] of mirrorNotes.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      800,
      558,
      'MASK_33 GEOMETRY IS MIRRORED ON X · MASK_40 CONFIRMS THE NE RETURN · NO FILTER OR SOURCE COPY',
      10,
      840,
      '#294B3C',
      'middle',
  ...
  for (const [layer, x, floorFill, label] of isolatedViews) {
    parts.push(
      await compositionWindow(
        options,
        [singleOpenNortheastCrossJunctionCandidateCell(0, 0, layer)],
        1,
        1,
        x,
        704,
  ...
  const distanceViews: ReadonlyArray<
    readonly [number, number, number, string, string]
  > = [
    [1094, 724, 90, A1A_PALETTE.floor, '90 · LIGHT'],
    [1204, 724, 90, A1A_PALETTE.charcoal, '90 · DARK'],
    [1314, 749, 40, A1A_PALETTE.floor, '40 · LIGHT'],
    [1374, 749, 40, A1A_PALETTE.charcoal, '40 · DARK'],
  ];
  for (const [x, y, size, floorFill, label] of distanceViews) {
    parts.push(
      await compositionWindow(
  ...
  const isolatedNotes = [
    'one cream shell owns all three filled crooks',
    'west and south remain one uninterrupted molded mass',
    'only NE carries the mirrored reveal and arris',
    'no coral, green, boundary seam, or duplicate outline is added',
    'NE radius and open floor must survive at 40 px on both grounds',
  ];
  for (const [index, note] of isolatedNotes.entries()) {
    parts.push(
      text(
        44 + (index % 2) * 780,
        1022 + Math.floor(index / 2) * 54,
        `• ${note}`,
        9,
        720,
        index < 4 ? '#294B3C' : '#B65F4D',
  ...
  parts.push(
    text(
      44,
      1202,
      'REJECT · CENTER POST · PEAK · HOURGLASS · STAIR-STEP · DOUBLED OUTLINE · DUPLICATE BELTS · CLOSED NE CROOK',
      9,
      840,
      '#9A493D',
    ),
  ...
  const compactViews: ReadonlyArray<
    readonly [number, number, number, string, string, number]
  > = [
    [54, 1368, 90, A1A_PALETTE.floor, '90 PX/CELL · LIGHT', 1344],
    [344, 1368, 90, A1A_PALETTE.charcoal, '90 PX/CELL · DARK', 1344],
    [654, 1443, 40, A1A_PALETTE.floor, '40 PX/CELL · LIGHT', 1418],
    [804, 1443, 40, A1A_PALETTE.charcoal, '40 PX/CELL · DARK', 1418],
  ];
  for (const [x, y, cellPixels, floorFill, label, labelY] of compactViews) {
    const matrixPixels = cellPixels * gate.compactMatrix.length;
    parts.push(
    ...
    parts.push(
      await singleOpenNortheastCrossJunctionMatrixWindow(
        options,
        gate.compactMatrix,
        x,
        y,
  ...
  const compactNotes = [
    'northeast cell remains genuine visible floor',
    'north / east join: accepted mask_26',
    'west join: accepted direct mask_24',
    'south join: accepted direct mask_38 between 16 and 34',
    'coral frame identifies the accepted mask_45 derivation cell',
  ];
  for (const [index, note] of compactNotes.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      1000,
      1674,
      'PASS ONLY IF THREE FILLED CROOKS READ AS ONE MASS AND NE STILL READS AS FLOOR.',
      10,
      840,
      '#B65F4D',
    ),
  ...
  > = [
    [[[5], [45]], 74, 1900, 160, 320, 'N · MASK_5 EAST REGISTER', 74, 2060],
    [[[10, 45]], 314, 1940, 320, 160, 'W · SHARED MASK_10', 474, 1940],
    [[[45, 10]], 744, 1940, 320, 160, 'E · SHARED MASK_10', 744, 1940],
    [[[45], [5]], 1256, 1900, 160, 320, 'S · MASK_5 EAST REGISTER', 1256, 1900],
  ];
  for (
    const [matrix, x, y, w, h, label, candidateX_, candidateY_]
    of socketAudits
  ) {
    parts.push(text(x + w / 2, 1892, label, 9, 840, '#4E7D79', 'middle'));
    parts.push(
      await singleOpenNortheastCrossJunctionMatrixWindow(
        options,
        matrix,
  ...
  parts.push(
    text(
      800,
      2270,
      'EAST-REGISTER VERTICALS AND SHARED HORIZONTALS MUST ERASE THE SOCKET WITHOUT CHANGING THE CENTER',
      10,
      840,
      '#294B3C',
      'middle',
  ...
  const extentRows: ReadonlyArray<
    readonly [number, string, string, string]
  > = [
    [2420, A1A_PALETTE.floor, 'LIGHT FLOOR', '#B65F4D'],
    [2960, A1A_PALETTE.charcoal, 'DARK FLOOR', '#4E7D79'],
  ];
  for (const [y, floorFill, groundLabel, labelColor] of extentRows) {
    parts.push(
      text(
    ...
    parts.push(crossSourceCellFrame(780, y + 260, 40, 2));

    const extentNotes = [
      'same mask_45 center at every extent',
      'mask_5 vertical runs stay east-registered',
      'immediate 3/6 joins: 27 N · 28 E · 25 W · 39 S',
      'cream mass, sockets, and open NE crook may not drift',
    ];
    for (const [index, note] of extentNotes.entries()) {
      parts.push(
        text(
  ...
  const reviewLines = [
    'Accept one plain whole-cell X mirror of accepted mask_33',
    'Keep exact N/E/S/W sockets, solid NW/SW/SE, and open NE',
    'Require one uninterrupted three-quarter cream mass at 240/90/40 px',
    'Keep the center invariant as accepted arms extend to 1 / 3 / 6 cells',
    'Reject a post, peak, hourglass, stair-step, duplicate outline, or closed crook',
  ];
  ...
  const boundaryLines: ReadonlyArray<readonly [string, string]> = [
    ['OWNER ACCEPTED · MASK_45 PLAIN X DERIVATION', '#4E7D79'],
    ['accepted ledger · 28 direct / 19 derived / 0 synthetic', MUTED],
    ['SOURCE: ACCEPTED MASK_33 PROOF BANK + WHOLE-CELL X', '#4E7D79'],
    ['NO FILTER · NO NEW SVG · NO SOURCE OR BOUNDARY COPY', '#4E7D79'],
    ['NO PRODUCTION / EXPORT / ATLAS / SCHEMA / BLOB / UNITY', '#9A493D'],
  ];
  for (const [index, [line, color]] of boundaryLines.entries()) {
    parts.push(
  ...
  parts.push(
    text(
      824,
      4028,
      'Owner acceptance locks the proof-layer derivation and ledger provenance only; production art, exporter, atlas, schema, blob mapping, and Unity remain separate.',
      9,
      720,
      MUTED,
    ),
...
async function renderEqualHeightSingleOpenSoutheastCrossJunctionGate(
  options: CliOptions,
  root: string,
): Promise<void> {
  const gate =
  ...
  const parts: string[] = [
    ...
    text(
      24,
      38,
      'QUOTACO EQUAL-HEIGHT WALLS — MASK_44 SINGLE-OPEN SOUTHEAST GATE',
      22,
      820,
    ),
    text(
      24,
      68,
      'OWNER ACCEPTED · plain whole-cell X mirror of accepted mask_41 · no filter and no new SVG bank',
      13,
      650,
      MUTED,
    ),
    text(1576, 38, 'MASK_44 · ACCEPTED', 11, 820, '#4E7D79', 'end'),
    text(
      1576,
      62,
      'accepted ledger · 28 direct · 19 derived · 0 synthetic',
    ...
    panel(20, 92, 1560, 478),
    text(44, 126, 'MIRROR DECISION — ACCEPTED SOURCE BESIDE THE RAW WHOLE-CELL X RESULT', 14, 820),
    text(
      44,
      150,
      'The transform changes X only. Cream, coral, green, face shade, contact shade, and seam keep their accepted Y hierarchy.',
      10,
      650,
      MUTED,
    ),

    panel(20, 590, 1560, 570),
    text(44, 624, 'ISOLATED READ — BASE / UPPER / COMPOSED AT 240; COMPOSED AT 90 AND 40', 14, 820),
    text(
      44,
      648,
      'One cream owner fills northwest, northeast, and southwest. Only the southeast floor crook exposes the mirrored material return.',
      10,
      650,
      MUTED,
    ),

    panel(20, 1180, 1560, 550),
    text(44, 1214, 'COMPACT OCCUPANCY — THREE FILLED CROOKS, SOUTHEAST OPEN', 14, 820),
    text(
      44,
      1238,
      'Exact matrix: 20 / 31 / 26; 24 / 44 / 34; 16 / 34 / floor. Every non-center cell is already accepted.',
      10,
      650,
      MUTED,
    ),

    panel(20, 1750, 1560, 500),
    text(44, 1784, 'SOCKET + PERSPECTIVE AUDIT — EACH EDGE AGAINST ITS ACCEPTED NEIGHBOR', 14, 820),
    text(
      44,
      1808,
      'The accepted derivation must disappear into mask_31 north, mask_24 west, and mask_34 east/south without a step, doubled outline, or false cap.',
      10,
      650,
      MUTED,
    ),

    panel(20, 2270, 1560, 700),
    text(44, 2304, 'EXTENT GATE — SAME CENTER AT 1 / 3 / 6-CELL CONNECTED RUNS', 14, 820),
    text(
      44,
      2328,
      'Long arms may not hide a center post, waist, stair-step, belt duplication, socket shift, or southeast crook closure.',
      10,
      650,
      MUTED,
    ),

    panel(20, 2990, 1560, 500),
    text(44, 3026, 'ACCEPTED CONTRACT', 14, 820),
    text(824, 3026, 'BOUNDARY', 14, 820),
  ];

  const sourceX = 64;
  const candidateX = 374;
  parts.push(
    await compositionWindow(
      options,
      [singleOpenSouthwestCrossJunctionCandidateCell(0, 0)],
  ...
  parts.push(crossAcceptedFrame(sourceX, 190, 240));
  parts.push(text(sourceX + 120, 174, 'MASK_41 · ACCEPTED DIRECT SOURCE', 9, 840, '#4E7D79', 'middle'));
  parts.push(
    await compositionWindow(
      options,
      [singleOpenSoutheastCrossJunctionCandidateCell(0, 0)],
      1,
      1,
      candidateX,
      190,
  ...
  parts.push(crossSourceCellFrame(candidateX, 190, 240, 2));
  parts.push(text(candidateX + 120, 174, 'MASK_44 · ACCEPTED PLAIN X', 9, 840, '#4E7D79', 'middle'));
  parts.push(
    '<path d="M318 310H358" fill="none" stroke="#B65F4D" stroke-width="4" stroke-linecap="round"/>' +
    '<path d="M358 310L344 300V320Z" fill="#B65F4D"/>',
  );
  parts.push(text(338, 292, 'X', 10, 900, '#B65F4D', 'middle'));
  parts.push(
    await compositionWindow(
      options,
      [doubleFilledDiagonalCrossJunctionCandidateCell(0, 0)],
  ...
  parts.push(crossAcceptedFrame(684, 220, 180));
  parts.push(text(774, 202, 'MASK_30 · SE RETURN CONTROL', 8, 840, '#4E7D79', 'middle'));
  const mirrorNotes = [
    'whole-cell matrix(-1 0 0 1 128 0)',
    'Y-based light and shadow strata stay fixed',
    'complete SW return moves to SE as one unit',
    'no boundary-seam path exists to filter',
    'plain X accepted as the proof-layer derivation',
  ];
  for (const [index, note] of mirrorNotes.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      800,
      538,
      'GEOMETRY IS MIRRORED · LIGHTING ORDER IS NOT · FILTERING WOULD REMOVE VALID SOURCE DETAIL',
      10,
      840,
      '#294B3C',
      'middle',
  ...
  for (const [layer, x, label] of isolatedViews) {
    parts.push(
      await compositionWindow(
        options,
        [singleOpenSoutheastCrossJunctionCandidateCell(0, 0, layer)],
        1,
        1,
        x,
        682,
    ...
    parts.push(crossSourceCellFrame(x, 682, 240, layer === 'composed' ? 2 : 0));
    parts.push(
      text(
        x + 120,
        948,
  ...
  const distanceViews: ReadonlyArray<
    readonly [number, number, number, string, string]
  > = [
    [884, 702, 90, A1A_PALETTE.floor, '90 · LIGHT'],
    [994, 702, 90, A1A_PALETTE.charcoal, '90 · DARK'],
    [1104, 727, 40, A1A_PALETTE.floor, '40 · LIGHT'],
    [1164, 727, 40, A1A_PALETTE.charcoal, '40 · DARK'],
  ];
  for (const [x, y, size, floorFill, label] of distanceViews) {
    parts.push(
      await compositionWindow(
  ...
  const isolatedNotes = [
    'one cream shell owns all three filled crooks',
    'NW and NE remain a continuous north slab',
    'SW joins the same mass without a center peak',
    'coral / green / shade occur once at SE',
    'SE radius and open floor survive at 40 px',
  ];
  for (const [index, note] of isolatedNotes.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      884,
      1112,
      'REJECT · CENTRAL POST · PEAK · HOURGLASS · STAIR-STEP · DOUBLED OUTLINE · DUPLICATE BELTS · CLOSED SE CROOK',
      9,
      840,
      '#9A493D',
    ),
  ...
  const compactViews: ReadonlyArray<
    readonly [number, number, number, string, string, number]
  > = [
    [54, 1298, 90, A1A_PALETTE.floor, '90 PX/CELL · LIGHT', 1274],
    [344, 1298, 90, A1A_PALETTE.charcoal, '90 PX/CELL · DARK', 1274],
    [654, 1373, 40, A1A_PALETTE.floor, '40 PX/CELL · LIGHT', 1348],
    [804, 1373, 40, A1A_PALETTE.charcoal, '40 PX/CELL · DARK', 1348],
  ];
  for (const [x, y, cellPixels, floorFill, label, labelY] of compactViews) {
    const matrixPixels = cellPixels * gate.compactMatrix.length;
    parts.push(
    ...
    parts.push(
      await singleOpenSoutheastCrossJunctionMatrixWindow(
        options,
        gate.compactMatrix,
        x,
        y,
  ...
  const compactNotes = [
    'north join: mask_31 between masks 20 and 26',
    'west join: mask_24 above mask_16',
    'east / south joins: accepted mask_34',
    'southeast cell remains genuine visible floor',
    'coral frame identifies only accepted mask_44',
  ];
  for (const [index, note] of compactNotes.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      1000,
      1604,
      'PASS ONLY IF THE THREE FILLED CROOKS READ AS ONE MASS AND SE STILL READS AS FLOOR.',
      10,
      840,
      '#B65F4D',
    ),
  ...
  > = [
    [[[31], [44]], 74, 1860, 160, 320, 'N · MASK_31', 74, 2020],
    [[[24, 44]], 314, 1900, 320, 160, 'W · MASK_24', 474, 1900],
    [[[44, 34]], 744, 1900, 320, 160, 'E · MASK_34', 744, 1900],
    [[[44], [34]], 1256, 1860, 160, 320, 'S · MASK_34', 1256, 1860],
  ];
  for (
    const [matrix, x, y, w, h, label, candidateX_, candidateY_]
    of socketAudits
  ) {
    parts.push(text(x + w / 2, 1828, label, 9, 840, '#4E7D79', 'middle'));
    parts.push(
      await singleOpenSoutheastCrossJunctionMatrixWindow(
        options,
        matrix,
  ...
  parts.push(text(800, 2194, 'ACCEPTED NEIGHBORS MUST ERASE THE SOCKET, NOT CHANGE THE CENTER SILHOUETTE', 10, 840, '#294B3C', 'middle'));

  parts.push(
    text(100, 2368, '1-CELL · 40 PX · LIGHT', 9, 840, '#B65F4D', 'middle'),
  );
  parts.push(
    await singleOpenSoutheastCrossJunctionMatrixWindow(
      options,
  ...
  parts.push(crossSourceCellFrame(80, 2428, 40, 2));
  parts.push(
    text(350, 2368, '3-CELL ARMS · 40 PX · LIGHT', 9, 840, '#4E7D79', 'middle'),
  );
  parts.push(
    await singleOpenSoutheastCrossJunctionMatrixWindow(
      options,
  ...
  parts.push(crossSourceCellFrame(330, 2508, 40, 2));
  parts.push(
    text(770, 2368, '6-CELL ARMS · 40 PX · LIGHT', 9, 840, '#B65F4D', 'middle'),
  );
  parts.push(
    await singleOpenSoutheastCrossJunctionMatrixWindow(
      options,
  ...
  parts.push(crossSourceCellFrame(750, 2628, 40, 2));
  parts.push(
    text(1310, 2368, '6-CELL ARMS · 40 PX · DARK', 9, 840, '#4E7D79', 'middle'),
  );
  parts.push(
    await singleOpenSoutheastCrossJunctionMatrixWindow(
      options,
  ...
  parts.push(crossSourceCellFrame(1290, 2628, 40, 2));
  parts.push(
    text(
      800,
      2944,
      'SAME MASK_44 DERIVATION AT EVERY EXTENT · 3/6-CELL IMMEDIATE NEIGHBORS: 32 NORTH · 35 EAST · 25 WEST · 36 SOUTH',
      10,
      840,
      '#294B3C',
      'middle',
  ...
  const acceptanceLines = [
    'Lock one plain whole-cell X mirror of accepted mask_41',
    'Keep exact N/E/S/W sockets, solid NW/NE/SW, and open SE',
    'Require one uninterrupted three-quarter cream mass at 240/90/40 px',
    'Keep the southeast return as one clean coral / green / shade stack',
    'Reject a post, peak, hourglass, stair-step, duplicate outline, or closed crook',
  ];
  ...
  const boundaryLines: ReadonlyArray<readonly [string, string]> = [
    ['OWNER ACCEPTED · MASK_44 PLAIN X DERIVATION', '#4E7D79'],
    ['accepted ledger · 28 direct / 19 derived / 0 synthetic', MUTED],
    ['SOURCE: ACCEPTED MASK_41 + PLAIN WHOLE-CELL X', '#4E7D79'],
    ['NO FILTER · NO NEW SVG BANK', '#4E7D79'],
    ['NO PRODUCTION / EXPORT / ATLAS / SCHEMA / BLOB / UNITY', '#9A493D'],
  ];
  for (const [index, [line, color]] of boundaryLines.entries()) {
    parts.push(
  ...
  parts.push(
    text(
      824,
      3390,
      'Acceptance covers one proof-layer derivation only. Production registration, export, atlas, schema, blob mapping, and Unity remain unchanged.',
      9,
      720,
      MUTED,
    ),
...
async function renderEqualHeightSingleOpenSouthwestCrossJunctionGate(
  options: CliOptions,
  root: string,
): Promise<void> {
  ...
  const parts: string[] = [
    ...
    text(
      24,
      38,
      'QUOTACO EQUAL-HEIGHT WALLS — SINGLE-OPEN SOUTHWEST CROSS-JUNCTION GATE',
      22,
      820,
    ),
    text(
      24,
      68,
      'OWNER ACCEPTED · one independently authored fixed-view union; mask_41 is a direct proof-layer source',
      13,
      650,
      MUTED,
    ),
    text(1576, 38, 'MASK_41 · ACCEPTED', 11, 820, '#4E7D79', 'end'),
    text(
      1576,
      62,
      'accepted ledger · 28 direct · 19 derived · 0 synthetic',
    ...
    panel(20, 92, 1560, 408),
    text(44, 126, 'GEOMETRY CONTROLS — CUES ONLY, NEVER A SOURCE STACK', 14, 820),
    text(
      44,
      150,
      'Accepted mask_40 constrains the southwest return; mask_39 constrains the north slab; mask_25 constrains the east slab. The accepted source resolves all three as one cell.',
      10,
      650,
      MUTED,
    ),

    panel(20, 520, 1560, 610),
    text(
      44,
      554,
      'SOURCE READ — BASE / UPPER / COMPOSED AT 240; COMPOSED AT 90 AND 40',
      14,
      820,
    ),
    text(
      44,
      578,
      'One cream owner wraps the north and east mass. Coral, green, and south-facing depth appear only on the exposed southwest return.',
      10,
      650,
      MUTED,
    ),

    panel(20, 1150, 1560, 560),
    text(
      44,
      1184,
      'COMPACT INSTALLED MASS — THREE SOLID QUADRANTS, SOUTHWEST CROOK OPEN',
      14,
      820,
    ),
    text(
      44,
      1208,
      'Exact matrix: 20 / 31 / 26; 16 / 41 / 42; floor / 16 / 34. Every non-center cell is already accepted.',
      10,
      650,
      MUTED,
    ),

    panel(20, 1730, 1560, 810),
    text(
      44,
      1764,
      'EXTENT GATE — SAME ACCEPTED SOURCE AT 1 / 3 / 6-CELL ARMS',
      14,
      820,
    ),
    text(
      44,
      1788,
      'Long runs may not camouflage a peak, post, doubled seam, buried frontage, broken socket, or a southwest crook that closes at 40 px.',
      10,
      650,
      MUTED,
    ),

    panel(20, 2560, 1560, 570),
    text(44, 2596, 'ACCEPTED CONTRACT', 14, 820),
    text(824, 2596, 'PROOF BOUNDARY', 14, 820),
  ];

  const controlCells: ReadonlyArray<
    readonly [CompositionCell, number, string, string, boolean]
  > = [
    [
      doubleFilledOppositeDiagonalCrossJunctionCandidateCell(0, 0),
      54,
    ...
    [
      doubleFilledNorthCrossJunctionCandidateCell(0, 0),
      294,
      'MASK_39',
      'NORTH SLAB CONTROL',
      false,
    ...
    [
      ...
      774,
      'MASK_41',
      'ACCEPTED AUTHORED UNION',
      true,
    ],
  ];
  for (const [cell, x, label, subtitle, sourceFocus] of controlCells) {
    const color = sourceFocus ? '#B65F4D' : '#4E7D79';
    parts.push(text(x + 90, 188, label, 9, 840, color, 'middle'));
    parts.push(
      await compositionWindow(
        options,
        [cell],
    ...
    parts.push(
      sourceFocus
        ? crossSourceCellFrame(x, 206, 180)
        : crossAcceptedFrame(x, 206, 180),
    );
    parts.push(text(x + 90, 416, subtitle, 8, 820, color, 'middle'));
  }
  const controlNotes = [
    'N / E / S / W sockets remain exact',
    'NE / SE / NW are one cream-led mass',
    'SW alone remains a floor crook',
    'no stacked controls or transformed reuse',
  ];
  for (const [index, note] of controlNotes.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      800,
      474,
      'AUTHOR ONE THREE-QUARTER SLAB UNION · CONTROLS DEFINE EDGES, NOT LAYERS',
      10,
      840,
      '#294B3C',
      'middle',
  ...
  for (const [layer, x, label] of sourceViews) {
    parts.push(
      await compositionWindow(
        options,
        [singleOpenSouthwestCrossJunctionCandidateCell(0, 0, layer)],
        1,
        1,
        x,
        612,
  ...
  const distanceViews: ReadonlyArray<
    readonly [number, number, number, string, string]
  > = [
    [884, 632, 90, A1A_PALETTE.floor, '90 · LIGHT'],
    [994, 632, 90, A1A_PALETTE.charcoal, '90 · DARK'],
    [1104, 657, 40, A1A_PALETTE.floor, '40 · LIGHT'],
    [1164, 657, 40, A1A_PALETTE.charcoal, '40 · DARK'],
  ];
  for (const [x, y, size, floorFill, label] of distanceViews) {
    parts.push(
      await compositionWindow(
  ...
  const sourceNotes = [
    'one continuous cream shell owns all three solid diagonals',
    'north and east top joins remain cream-led and fascia-free',
    'mask_18 alone owns the west-boundary service seam',
    'mask_17 owns the south west-register transition',
    'SW radius, local belts, and local depth survive at 40 px',
  ];
  for (const [index, note] of sourceNotes.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      884,
      1084,
      'REJECT · CENTER PEAK · POST · CAP · STAIR-STEP · WAIST · INTERNAL SEAM · CLOSED SW CROOK',
      10,
      840,
      '#9A493D',
    ),
  ...
  const compactViews: ReadonlyArray<
    readonly [number, number, number, string, string, number]
  > = [
    [54, 1268, 90, A1A_PALETTE.floor, '90 PX/CELL · LIGHT', 1244],
    [344, 1268, 90, A1A_PALETTE.charcoal, '90 PX/CELL · DARK', 1244],
    [654, 1343, 40, A1A_PALETTE.floor, '40 PX/CELL · LIGHT', 1318],
    [804, 1343, 40, A1A_PALETTE.charcoal, '40 PX/CELL · DARK', 1318],
  ];
  for (const [x, y, cellPixels, floorFill, label, labelY] of compactViews) {
    const matrixPixels = cellPixels * gate.compactMatrix.length;
    parts.push(
    ...
    parts.push(
      await singleOpenSouthwestCrossJunctionMatrixWindow(
        options,
        gate.compactMatrix,
        x,
        y,
  ...
  const compactNotes = [
    'north join: mask_31 between masks 20 and 26',
    'east join: mask_42 and mask_34',
    'west join: mask_16, with no duplicate service seam',
    'southwest cell remains genuine visible floor',
    'coral frame identifies only the accepted mask_41 source',
  ];
  for (const [index, note] of compactNotes.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      1000,
      1590,
      'PASS ONLY IF THE THREE SOLID QUADRANTS READ AS ONE MASS AND SW STILL READS AS FLOOR.',
      10,
      840,
      '#B65F4D',
    ),
  ...
  parts.push(
    text(100, 1830, '1-CELL · 40 PX · LIGHT', 9, 840, '#B65F4D', 'middle'),
  );
  parts.push(
    await singleOpenSouthwestCrossJunctionMatrixWindow(
      options,
  ...
  parts.push(crossSourceCellFrame(80, 1890, 40, 2));
  parts.push(
    text(350, 1830, '3-CELL ARMS · 40 PX · LIGHT', 9, 840, '#4E7D79', 'middle'),
  );
  parts.push(
    await singleOpenSouthwestCrossJunctionMatrixWindow(
      options,
  ...
  parts.push(crossSourceCellFrame(330, 1970, 40, 2));
  parts.push(
    text(770, 1830, '6-CELL ARMS · 40 PX · LIGHT', 9, 840, '#B65F4D', 'middle'),
  );
  parts.push(
    await singleOpenSouthwestCrossJunctionMatrixWindow(
      options,
  ...
  parts.push(crossSourceCellFrame(750, 2090, 40, 2));
  parts.push(
    text(1310, 1830, '6-CELL ARMS · 40 PX · DARK', 9, 840, '#4E7D79', 'middle'),
  );
  parts.push(
    await singleOpenSouthwestCrossJunctionMatrixWindow(
      options,
  ...
  parts.push(crossSourceCellFrame(1290, 2090, 40, 2));
  parts.push(
    text(
      800,
      2430,
      'SAME MASK_41 SOURCE AT EVERY EXTENT · ACCEPTED NEIGHBORS: 32 NORTH · 43 EAST · 18 WEST · 17 SOUTH',
      10,
      840,
      '#294B3C',
      'middle',
  ...
  parts.push(
    text(
      800,
      2474,
      'The southwest crook must remain legible in the compact room, not merely at the end of a long run.',
      10,
      720,
      MUTED,
      'middle',
  ...
  const acceptedLines = [
    'Lock one flattened 128 by 128 base / upper source with no transform',
    'Keep exact N/E/S/W sockets, solid NE/SE/NW, and open SW',
    'Require one continuous three-quarter cream plane at 240/90/40 px',
    'Keep buried joins clear; reject a peak, post, patch, stair-step, or closed floor crook',
    'Acceptance covers only mask_41 at the proof layer',
  ];
  for (const [index, line] of acceptedLines.entries()) {
    parts.push(
      text(
  ...
  const boundaryLines: ReadonlyArray<readonly [string, string]> = [
    ['OWNER ACCEPTED · MASK_41 DIRECT PROOF SOURCE', '#4E7D79'],
    ['accepted ledger · 28 direct / 19 derived / 0 synthetic', MUTED],
    ['2 EXTERNAL PROOF SVGs · TEMPORARY FRAMES', '#4E7D79'],
    ['NO PRODUCTION / CANONICAL / EXPORT / ATLAS / SCHEMA / BLOB / UNITY', '#9A493D'],
  ];
  for (const [index, [line, color]] of boundaryLines.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      824,
      2948,
      'Acceptance covers proof-layer provenance only. Production registration, canonical export, atlas, schema, blob mapping, and Unity remain unchanged.',
      9,
      720,
      MUTED,
    ),
...
async function renderEqualHeightDoubleFilledSouthCrossJunctionGate(
  options: CliOptions,
  root: string,
): Promise<void> {
  const gate = EQUAL_HEIGHT_DOUBLE_FILLED_SOUTH_CROSS_JUNCTION_GATE;
  const width = 1600;
  const height = 2930;
  const panelFill = '#ECE5D5';
  const panel = (x: number, y: number, w: number, h: number): string =>
  ...
  const parts: string[] = [
    ...
    text(
      24,
      38,
      'QUOTACO EQUAL-HEIGHT WALLS — SOUTH-FILLED SLAB CROSS-JUNCTION GATE',
      22,
      820,
    ),
    text(
      24,
      68,
      'OWNER ACCEPTED · one independently authored fixed-view union; mask_32 is a direct proof-layer source',
      13,
      650,
      MUTED,
    ),
    text(1576, 38, 'MASK_32 · ACCEPTED', 11, 820, '#4E7D79', 'end'),
    text(
      1576,
      62,
      'accepted ledger · 28 direct · 19 derived · 0 synthetic',
    ...
    text(
      44,
      126,
      'OCCUPANCY LAW — TWO SOUTHERN CROOKS CLOSE INTO ONE SLAB',
      14,
      820,
    ),
    text(
      44,
      150,
      'mask_23 and mask_29 constrain the southern closures; mask_39 is an opposite-orientation control only. The accepted source is one authored union, never a stack or Y mirror.',
      10,
      650,
      MUTED,
    ),

    panel(20, 520, 1560, 540),
    text(
      44,
      554,
      'SOURCE READ — BASE / UPPER / COMPOSED AT 240; COMPOSED AT GAME DISTANCE',
      14,
      820,
    ),
    text(
      44,
      578,
      'One cream owner forms the south slab and centered north spur. Only the exposed north-spur side register belongs to mask_32.',
      10,
      650,
      MUTED,
    ),

    panel(20, 1080, 1560, 520),
    text(
      44,
      1114,
      'COMPACT INSTALLED MASS — NORTH SPUR + TWO-ROW SOUTH SLAB',
      14,
      820,
    ),
    text(
      44,
      1138,
      'Exact matrix: empty / 4 / empty; 20 / 32 / 26; 16 / 38 / 34. The foreground row owns the south-facing fascia.',
      10,
      650,
      MUTED,
    ),

    panel(20, 1620, 1560, 850),
    text(
      44,
      1654,
      'EXTENT GATE — SYMMETRIC FOUR-ARM MASS, SAME CENTER AT 1 / 3 / 6 CELLS',
      14,
      820,
    ),
    text(
      44,
      1678,
      'Long arms may not camouflage a pinched center, buried fascia, broken socket, or stair-step. Every surrounding mask is already accepted.',
      10,
      650,
      MUTED,
    ),

    panel(20, 2490, 1560, 410),
    text(44, 2526, 'ACCEPTED CONTRACT', 14, 820),
    text(824, 2526, 'PROOF BOUNDARY', 14, 820),
  ];

  const stateCells: ReadonlyArray<
    readonly [CompositionCell, number, string, string, boolean]
  > = [
    [
      singleFilledSoutheastCrossJunctionCandidateCell(0, 0),
  ...
  for (const [cell, x, label, subtitle, underReview] of stateCells) {
    const color = underReview ? '#B65F4D' : '#4E7D79';
    parts.push(text(x + 90, 188, label, 9, 840, color, 'middle'));
    parts.push(
      await compositionWindow(
        options,
  ...
  const stateNotes = [
    'NE + NW remain real floor crooks',
    'SE + SW form one flat cream slab',
    'fixed-light source, not a Y mirror',
    'no center peak, cap, post, or overlay',
  ];
  for (const [index, note] of stateNotes.entries()) {
    parts.push(
      text(
        1216,
        218 + index * 50,
  ...
  parts.push(
    text(
      800,
      474,
      'AUTHOR ONE SOUTH SLAB · MASK_23 + MASK_29 ARE GEOMETRY CUES ONLY · NO SOURCE STACK',
      10,
      840,
      '#294B3C',
      'middle',
  ...
  for (const [layer, x, label] of sourceViews) {
    parts.push(
      await compositionWindow(
        options,
        [doubleFilledSouthCrossJunctionCandidateCell(0, 0, layer)],
        1,
        1,
        x,
        612,
  ...
  const sourceNotes = [
    'cream crosses the spur-to-slab shoulder without a ridge',
    'vertical coral + green end at the slab shoulder',
    'no horizontal coral or green survives on the buried rear row',
    'the installed foreground row supplies shade, belts, plinth, and contact',
    'the accepted SVG pair remains an external proof source only',
  ];
  for (const [index, note] of sourceNotes.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      884,
      1018,
      'REJECT · T-ON-A-BLOCK · HOURGLASS NECK · BURIED FASCIA · LOST NORTH CROOKS',
      10,
      840,
      '#9A493D',
    ),
  ...
  parts.push(
    await doubleFilledSouthCrossJunctionMatrixWindow(
      options,
      gate.compactMatrix,
      70,
      1198,
  ...
  parts.push(installedMassFrame(70, 1318, 3, 2, 120));
  parts.push(crossSourceCellFrame(190, 1318, 120, 4));
  parts.push(
    text(
      560,
      1174,
  ...
  parts.push(installedMassFrame(500, 1238, 3, 2, 40));
  parts.push(crossSourceCellFrame(540, 1238, 40, 2));
  const compactNotes = [
    'north end: accepted mask_4 terminus',
    'rear row: mask_20 / 32 / 26',
    'front row: mask_16 / 38 / 34',
    'coral frame identifies only mask_32',
    'foreground frontage must remain one continuous band',
  ];
  for (const [index, note] of compactNotes.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      760,
      1500,
      'PASS ONLY IF BOTH NORTHERN FLOOR CROOKS AND THE SOUTH-FACING DEPTH SURVIVE AT 40 PX.',
      10,
      840,
      '#B65F4D',
    ),
  ...
  parts.push(crossSourceCellFrame(250, 1874, 40, 2));
  parts.push(
    text(
      760,
      1734,
  ...
  parts.push(crossSourceCellFrame(740, 1994, 40, 2));
  parts.push(
    text(
      1300,
      1734,
  ...
  parts.push(crossSourceCellFrame(1280, 1994, 40, 2));
  parts.push(
    text(
      800,
      2406,
      'SAME MASK_32 SOURCE AT EVERY EXTENT · LONG RUNS MAY NOT HIDE A PINCH, PEAK, OR BROKEN CREAM PLANE',
      10,
      840,
      '#294B3C',
      'middle',
  ...
  const acceptedLines = [
    'Lock one direct base/upper source with no transform',
    'Keep exact N/E/S/W sockets, solid SE/SW, and open NE/NW',
    'Require one flat cream south slab at 240, 90, and 40 px',
    'Reject a Y-flipped mask_39, stacked controls, or duplicate frontage',
    'Acceptance covers only mask_32 at the proof layer',
  ];
  for (const [index, line] of acceptedLines.entries()) {
    parts.push(
      text(
  ...
  const boundaryLines: ReadonlyArray<readonly [string, string]> = [
    ['OWNER ACCEPTED · MASK_32 DIRECT PROOF SOURCE', '#4E7D79'],
    ['accepted ledger · 28 direct / 19 derived / 0 synthetic', MUTED],
    ['2 EXTERNAL PROOF SVGs · TEMPORARY FRAMES', '#4E7D79'],
    ['NO CANONICAL / EXPORT / ATLAS / SCHEMA / BLOB / UNITY', '#9A493D'],
  ];
  ...
  parts.push(
    text(
      824,
      2798,
      'The foreground row owns the south-facing shade and tri-tone fascia; mask_32 owns only the buried cream slab and exposed north-spur register.',
      9,
      720,
      MUTED,
    ),
...
async function renderEqualHeightDoubleFilledNorthCrossJunctionGate(
  options: CliOptions,
  root: string,
): Promise<void> {
...
export async function renderEqualHeightDoubleFilledNorthCrossJunctionAcceptedArchive(
  ...
  const parts: string[] = [
    ...
    text(
      24,
      38,
      'QUOTACO EQUAL-HEIGHT WALLS — NORTH-FILLED SLAB CROSS-JUNCTION GATE',
      23,
      820,
    ),
    text(
      24,
      68,
      'OWNER ACCEPTED · one authored west-fixed union; mask_39 is a direct proof-layer source',
      13,
      650,
      MUTED,
    ),
    text(1576, 38, 'MASK_39 · ACCEPTED', 11, 820, '#4E7D79', 'end'),
    text(
      1576,
      62,
      'accepted ledger · 28 direct · 19 derived · 0 synthetic',
    ...
    text(44, 126, 'STATE DIAMOND — CLOSE THE NORTHWEST CROOK, NOT THE WHOLE HUB', 14, 820),
    text(
      44,
      150,
      'mask_19 fixes the west-facing branch register; mask_37 supplies only the northwest closure law. The accepted source is one new union, never an overlay.',
      10,
      650,
      MUTED,
    ),

    panel(20, 542, 1560, 570),
    text(44, 576, 'SOURCE READ — BASE / UPPER / COMPOSED AT 240; COMPOSED AT GAME DISTANCE', 14, 820),
    text(
      44,
      600,
      'One cream owner spans the north slab and south spur. Coral, green, and shade exist only on the exposed southern frontage.',
      10,
      650,
      MUTED,
    ),

    panel(20, 1132, 1560, 510),
    text(44, 1166, 'COMPACT INSTALLED MASS — TWO-ROW SLAB + SOUTH SPUR', 14, 820),
    text(
      44,
      1190,
      'Exact matrix: 20 / 31 / 26; 16 / 39 / 34; empty / 1 / empty. Both southern crooks must stay visibly open.',
      10,
      650,
      MUTED,
    ),

    panel(20, 1662, 1560, 770),
    text(44, 1696, 'EXTENT GATE — LONG NORTH SLAB, ONE SOUTH SPUR, ACCEPTED NEIGHBORS ONLY', 14, 820),
    text(
      44,
      1720,
      'At the mask_39 checkpoint, a symmetric four-arm proof was excluded because mask_32 was unaccepted; that proof now lives in the accepted mask_32 gate.',
      10,
      650,
      MUTED,
    ),

    panel(20, 2452, 1560, 380),
    text(44, 2488, 'ACCEPTED CONTRACT', 14, 820),
    text(824, 2488, 'PROOF BOUNDARY', 14, 820),
  ];
  ...
  for (const [cell, x, label, subtitle, color] of stateCells) {
    parts.push(text(x + 90, 188, label, 9, 840, color, 'middle'));
    parts.push(
      await compositionWindow(
        options,
    ...
    parts.push(crossAcceptedFrame(x, 206, 180, color));
    parts.push(text(x + 90, 416, subtitle, 8, 820, color, 'middle'));
  }
  const stateNotes = [
    'one continuous cream north plane',
    'west-fixed south branch stays flush',
    'SE + SW remain real floor crooks',
    'no peak, post, patch, or doubled belts',
  ];
  for (const [index, note] of stateNotes.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      800,
      486,
      'AUTHOR ONE UNION · MASK_19 REGISTER + MASK_37 CLOSURE LAW · NO SOURCE STACK',
      10,
      840,
      '#294B3C',
      'middle',
  ...
  for (const [layer, x, label] of sourceViews) {
    parts.push(
      await compositionWindow(
        options,
        [doubleFilledNorthCrossJunctionCandidateCell(0, 0, layer)],
        1,
        1,
        x,
        632,
  ...
  const distanceViews: ReadonlyArray<
    readonly [number, number, number, string, string]
  > = [
    [884, 652, 90, A1A_PALETTE.floor, '90 · LIGHT'],
    [994, 652, 90, A1A_PALETTE.charcoal, '90 · DARK'],
    [1104, 677, 40, A1A_PALETTE.floor, '40 · LIGHT'],
    [1164, 677, 40, A1A_PALETTE.charcoal, '40 · DARK'],
  ];
  for (const [x, y, size, floorFill, label] of distanceViews) {
    parts.push(
      await compositionWindow(
  ...
  const sourceNotes = [
    'cream crosses the full north socket without an internal ridge',
    'cream remains visible through the top-to-top south socket',
    'frontage shade and belts yield before wrapping down the branch',
    'north, east, and west joins carry no buried fascia',
    'source files are accepted direct proof provenance, not production art',
  ];
  for (const [index, note] of sourceNotes.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      884,
      1036,
      'REJECT · CENTER PEAK · RAISED CAP · BURIED BELTS · LOST SOUTH CROOKS',
      10,
      840,
      '#9A493D',
    ),
  ...
  parts.push(text(250, 1236, '90 PX/CELL · LIGHT', 10, 840, '#B65F4D', 'middle'));
  parts.push(
    await doubleFilledNorthCrossJunctionMatrixWindow(
      options,
      gate.compactMatrix,
      70,
      1260,
  ...
  parts.push(installedMassFrame(70, 1260, 3, 2, 120));
  parts.push(crossSourceCellFrame(190, 1380, 120, 4));
  parts.push(text(560, 1236, '40 PX/CELL · DARK', 10, 840, '#4E7D79', 'middle'));
  parts.push(
    await doubleFilledNorthCrossJunctionMatrixWindow(
      options,
      gate.compactMatrix,
  ...
  parts.push(installedMassFrame(500, 1260, 3, 2, 40));
  parts.push(crossSourceCellFrame(540, 1300, 40, 2));
  const compactNotes = [
    'rear row: mask_20 / 31 / 26',
    'front row: mask_16 / 39 / 34',
    'south spur: accepted mask_1 terminus',
    'teal brackets own the slab; coral owns only mask_39',
    'no seam may appear between masks 31, 38, and 39',
  ];
  for (const [index, note] of compactNotes.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      760,
      1556,
      'PASS ONLY IF BOTH SOUTHERN FLOOR CROOKS SURVIVE AT 40 PX.',
      10,
      840,
      '#B65F4D',
    ),
  ...
  parts.push(text(270, 1764, '3-CELL EXTENT · 40 PX/CELL · LIGHT', 10, 840, '#4E7D79', 'middle'));
  parts.push(
    await doubleFilledNorthCrossJunctionMatrixWindow(
      options,
      gate.threeCellExtentMatrix,
  ...
  parts.push(installedMassFrame(130, 1864, 7, 2, 40));
  parts.push(crossSourceCellFrame(250, 1904, 40, 2));
  parts.push(text(760, 1764, '6-CELL EXTENT · 40 PX/CELL · LIGHT', 10, 840, '#B65F4D', 'middle'));
  parts.push(
    await doubleFilledNorthCrossJunctionMatrixWindow(
      options,
  ...
  parts.push(installedMassFrame(500, 1984, 13, 2, 40));
  parts.push(crossSourceCellFrame(740, 2024, 40, 2));
  parts.push(text(1300, 1764, '6-CELL EXTENT · 40 PX/CELL · DARK', 10, 840, '#4E7D79', 'middle'));
  parts.push(
    await doubleFilledNorthCrossJunctionMatrixWindow(
      options,
      gate.sixCellExtentMatrix,
  ...
  parts.push(installedMassFrame(1040, 1984, 13, 2, 40));
  parts.push(crossSourceCellFrame(1280, 2024, 40, 2));
  parts.push(
    text(
      800,
      2368,
      'SAME MASK_39 SOURCE AT EVERY EXTENT · LONG RUNS MAY NOT HIDE A RIDGE, REGISTER STEP, OR BROKEN FRONTAGE',
      10,
      840,
      '#294B3C',
      'middle',
  ...
  const reviewLines = [
    'Judge one direct base/upper source with no transform',
    'Keep exact N/E/S/W sockets, solid NE/NW, and open SE/SW',
    'Require a flat two-row slab at 90 and 40 px on both grounds',
    'Reject stacked mask_19 + mask_37 art, a center post, or duplicate material belts',
    'Proof-layer acceptance does not register production art',
  ];
  for (const [index, line] of reviewLines.entries()) {
    parts.push(
      text(
  ...
  const boundaryLines: ReadonlyArray<readonly [string, string]> = [
    ['OWNER ACCEPTED · MASK_39 DIRECT PROOF SOURCE', '#4E7D79'],
    ['accepted ledger · 28 direct / 19 derived / 0 synthetic', MUTED],
    ['2 EXTERNAL PROOF SVGs · TEMPORARY FRAMES', '#4E7D79'],
    ['NO CANONICAL / EXPORT / ATLAS / SCHEMA / BLOB / UNITY', '#9A493D'],
  ];
  ...
  parts.push(
    text(
      824,
      2750,
      'The cream top owns the south socket; frontage shade and belts yield at the top-to-top join.',
      9,
      720,
      MUTED,
    ),
...
async function renderEqualHeightSingleFilledSouthwestCrossJunctionGate(
  ...
  const parts: string[] = [
    ...
    text(
      24,
      38,
      'QUOTACO EQUAL-HEIGHT WALLS — SOUTHWEST-FILLED CROSS-JUNCTION GATE',
      23,
      820,
    ),
    text(
      24,
      68,
      'OWNER ACCEPTED · whole-cell X mirror of accepted mask_23; no new authored source',
      13,
      650,
      MUTED,
    ),
    text(1576, 38, 'MASK_29 · ACCEPTED', 11, 820, '#4E7D79', 'end'),
    text(
      1576,
      62,
      'accepted ledger · 28 direct · 19 derived · 0 synthetic',
    ...
    text(
      44,
      126,
      'ACCEPTED MIRROR — MASK_23 → MASK_29',
      14,
      820,
    ),
    text(
      44,
      150,
      'Mirror the complete 128×128 source around x=64. Do not mirror paths separately, add a seam filter, or author replacement SVGs.',
      10,
      650,
      MUTED,
    ),
    ...
    text(
      44,
      616,
      'MIRRORED SOURCE READ — BASE / UPPER / COMPOSED AT 240 / 90 / 40 PX',
      14,
      820,
    ),
    text(
      44,
      640,
      'The southwest cream field must meet west and south sockets while the open southeast shoulder retains one tri-tone foreground boundary.',
      10,
      650,
      MUTED,
    ),
    ...
    text(
      44,
      1276,
      'BOUNDARY CONTROLS — MASK_27 + MASK_28 ARE LAWS, NOT OVERLAYS',
      14,
      820,
    ),
    text(
      44,
      1300,
      'The accepted east-register T states demonstrate the same southwest closure. mask_29 must still read as one mirrored four-way union.',
      10,
      650,
      MUTED,
    ),
    ...
    text(
      44,
      1750,
      'The southwest fill and three floor crooks must remain legible while the mirrored east register joins every accepted neighbor flush.',
      10,
      650,
      MUTED,
    ),
    ...
    text(
      44,
      2246,
      'EXTENT GATE — THREE-CELL AND SIX-CELL ARMS ON LIGHT / DARK FLOOR',
      14,
      820,
    ),
    text(
      44,
      2270,
      'The center and its south-facing wrap stay unchanged as the arms grow. Long runs may not hide a socket step, doubled shade, or disconnected cream field.',
      10,
      650,
      MUTED,
    ),
  ...
  parts.push(
    text(
      326,
      190,
      'MASK_23 · ACCEPTED SOURCE · SE FILLED',
      10,
      840,
      '#4E7D79',
      'middle',
  ...
  parts.push(text(800, 316, 'WHOLE-CELL X MIRROR', 18, 820, '#B65F4D', 'middle'));
  parts.push(text(800, 348, 'x = 128 − x', 11, 740, MUTED, 'middle'));
  parts.push(
    text(
      1274,
      190,
      'MASK_29 · ACCEPTED · SW FILLED',
      10,
      840,
      '#B65F4D',
      'middle',
  ...
  parts.push(text(326, 490, '.#. / ### / .##', 10, 800, MUTED, 'middle'));
  parts.push(text(1274, 490, '.#. / ### / ##.', 10, 820, '#B65F4D', 'middle'));
  parts.push(
    text(
      800,
      528,
      '0 NEW SOURCE FILES · ONE COMPLETE-CELL TRANSFORM · OWNER-APPROVED DERIVATION',
      10,
      820,
      '#294B3C',
      'middle',
  ...
  const sourceViews: ReadonlyArray<
    readonly ['base' | 'upper' | 'composed', number, string]
  > = [
    ['base', 54, 'MIRRORED BASE · 240'],
    ['upper', 314, 'MIRRORED UPPER · 240'],
    ['composed', 574, 'MIRRORED COMPOSED · 240'],
  ];
  for (const [layer, x, label] of sourceViews) {
    parts.push(await compositionWindow(
      options,
      [singleFilledSouthwestCrossJunctionCandidateCell(0, 0, layer)],
      1,
      1,
      x,
      688,
  ...
  const sourceNotes = [
    'SW cream field reaches the exact west and south sockets',
    'north and east sockets retain the accepted open-hub register',
    'NE / SE / NW remain genuine floor crooks',
    'coral, green, and south shadow begin once at the open SE shoulder',
    'no buried SW fascia, doubled contour, cap, peak, post, or patch',
  ];
  for (const [index, note] of sourceNotes.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      1120,
      990,
      'ACCEPTED DERIVATION · SOURCE MASK_23 · MIRROR-X',
      10,
      820,
      '#B65F4D',
    ),
  ...
  const boundaryControls: ReadonlyArray<
    readonly [CompositionCell, number, string]
  > = [
    [
      eastPartialTJunctionCandidateCell(27, 0, 0),
      60,
      'MASK_27 · N/S/W · SW FILLED',
    ],
    [
      horizontalPartialTJunctionCandidateCell(28, 0, 0),
      360,
      'MASK_28 · E/S/W · SW FILLED',
    ],
    [
      singleFilledSouthwestCrossJunctionCandidateCell(0, 0),
      1280,
      'MASK_29 · N/E/S/W · SW FILLED',
    ],
  ];
  for (const [cell, x, label] of boundaryControls) {
    parts.push(
      text(
        x + 120,
        1344,
  ...
  parts.push(text(690, 1460, 'MIRROR ONE COMPLETE FOUR-WAY UNION', 10, 840, '#294B3C'));
  parts.push(text(690, 1510, 'REJECT · PEAK / POST / PATCH', 10, 840, '#9A493D'));
  parts.push(
    text(
      690,
      1545,
      'REJECT · CREAM STOPPING BEFORE WEST OR SOUTH SOCKET',
      10,
      840,
      '#9A493D',
    ),
  ...
  parts.push(
    text(
      690,
      1580,
      'REJECT · TOP SWALLOWING SOUTH FRONT / DOUBLE SHADOW',
      10,
      840,
      '#9A493D',
    ),
  ...
  parts.push(
    text(
      690,
      1615,
      'PASS · CONTINUOUS SW TOP / THREE OPEN FLOOR CROOKS',
      10,
      820,
      '#294B3C',
    ),
  ...
  parts.push(
    text(
      246,
      1792,
      '90 PX/CELL · LIGHT · MIRRORED EAST REGISTER',
      10,
      840,
      '#B65F4D',
      'middle',
  ...
  parts.push(await singleFilledSouthwestCrossJunctionMatrixWindow(
    options,
    gate.compactMatrix,
    111,
    1812,
  ...
  parts.push(crossInstalledUnionFrame(111, 1902, 90));
  parts.push(crossSourceCellFrame(201, 1902, 90, 3));
  parts.push(
    text(
      558,
      1792,
      '40 PX/CELL · DARK · SAME EAST REGISTER',
      10,
      840,
      '#4E7D79',
      'middle',
  ...
  parts.push(crossInstalledUnionFrame(498, 1852, 40));
  parts.push(crossSourceCellFrame(538, 1852, 40, 2));
  const compactNotes = [
    'four cardinal sockets disappear into accepted neighbors',
    'the southwest fill remains one continuous 2×2 wall mass',
    'northeast, southeast, and northwest stay readable floor',
    'south frontage wraps once without a register step or buried rail',
  ];
  for (const [index, note] of compactNotes.entries()) {
    parts.push(
      text(
  ...
  parts.push(
    text(
      760,
      2088,
      'Reject a cream gap, perspective switch, doubled seam, shadow pinch, or lost floor crook.',
      11,
      820,
      '#9A493D',
    ),
  ...
  parts.push(
    text(
      111,
      2126,
      'TEAL BRACKETS · FULL 2×2 INSTALLED UNION · 20 / 29 / 16 / 34',
      9,
      820,
      '#4E7D79',
    ),
  );
  parts.push(text(111, 2160, 'CORAL CELL · MASK_29 ACCEPTED', 9, 820, '#4E7D79'));
  ...
  parts.push(crossInstalledUnionFrame(130, 2450, 40));
  parts.push(crossSourceCellFrame(170, 2450, 40, 2));
  parts.push(
    text(
      616,
      2310,
      '6-CELL ARMS · 40 PX/CELL · LIGHT · EAST REGISTER',
      10,
      840,
      '#4E7D79',
      'middle',
  ...
  parts.push(crossInstalledUnionFrame(556, 2570, 40));
  parts.push(crossSourceCellFrame(596, 2570, 40, 2));
  parts.push(
    text(
      1254,
      2310,
      '6-CELL ARMS · 40 PX/CELL · DARK · SAME REGISTER',
      10,
      840,
      '#B65F4D',
      'middle',
  ...
  parts.push(crossInstalledUnionFrame(1194, 2570, 40));
  parts.push(crossSourceCellFrame(1234, 2570, 40, 2));
  parts.push(
    text(
      190,
      2660,
      'TEAL BRACKETS · INSTALLED 2×2 UNION',
      10,
      820,
      '#4E7D79',
      'middle',
  ...
  const reviewLines = [
    'Judge one whole-cell X mirror of accepted mask_23',
    'Keep exact N/E/S/W sockets and exactly three open floor crooks',
    'Demand one continuous southwest cream field and one south-facing band',
    'Reject gaps, doubled shade, buried fascia, peaks, posts, or patches',
  ];
  for (const [index, line] of reviewLines.entries()) {
    parts.push(
      text(
...
async function renderEqualHeightDoubleFilledWestCrossJunctionGate(
  options: CliOptions,
  root: string,
): Promise<void> {
  const gate = EQUAL_HEIGHT_DOUBLE_FILLED_WEST_CROSS_JUNCTION_GATE;
  const width = 1600;
  const height = 3000;
  const panelFill = '#ECE5D5';
  const panel = (x: number, y: number, w: number, h: number): string =>
  ...
  const parts: string[] = [
    ...
    text(1576, 62, 'accepted ledger · 28 direct · 19 derived · 0 synthetic', 10, 700, MUTED, 'end'),

    panel(20, 92, 1560, 470),
    text(44, 126, 'ACCEPTED MIRROR — ONE AUTHORED UNION, ONE WHOLE-CELL X TRANSFORM', 14, 820),
    text(44, 150, 'The source stays mask_25. The accepted derivation reverses only X, preserving every Y-based top/front cue and the polished shoulder-shade handoff.', 10, 650, MUTED),

    panel(20, 582, 1560, 620),
    text(44, 616, 'CANDIDATE READ — BASE / UPPER / COMPOSED AT 240 / 90 / 40 PX', 14, 820),
    text(44, 640, 'The east branch enters one continuous west cream field. Northeast and southeast remain genuine open floor crooks.', 10, 650, MUTED),

    panel(20, 1222, 1560, 500),
    text(44, 1256, 'COMPACT INSTALLED TRUTH — 2×3 WEST SLAB + EAST BRANCH', 14, 820),
    text(44, 1280, 'The literal ##. / ### / ##. occupancy must resolve without west fascia, center seam, peak, post, or lost east crook.', 10, 650, MUTED),

    panel(20, 1742, 1560, 710),
    text(44, 1776, 'LENGTH GATE — 24 / 42 SLAB REPEATS + 10 / 8 EAST BRANCH', 14, 820),
    text(44, 1800, 'Three- and six-cell extents keep the same mask_43 center. Long runs may not hide a bad hub or a broken branch socket.', 10, 650, MUTED),

    panel(20, 2472, 1560, 504),
    text(44, 2508, 'ACCEPTED CONTRACT', 14, 820),
    text(824, 2508, 'PROOF BOUNDARY', 14, 820),
  ];

  parts.push(text(188, 188, 'ACCEPTED SOURCE · MASK_25 · 0x3F', 9, 840, '#4E7D79', 'middle'));
  parts.push(await compositionWindow(
    options,
    [doubleFilledEastCrossJunctionCandidateCell(0, 0)],
    1,
  ...
  parts.push(text(468, 330, 'WHOLE-CELL X MIRROR', 12, 840, '#B65F4D', 'middle'));
  parts.push(text(468, 360, 'x = 128 − x', 10, 720, MUTED, 'middle'));
  parts.push(text(748, 188, 'ACCEPTED · MASK_43 · 0xCF', 9, 840, '#4E7D79', 'middle'));
  parts.push(await compositionWindow(
    options,
    [doubleFilledWestCrossJunctionCandidateCell(0, 0)],
    1,
    1,
    628,
    208,
  ...
  parts.push(crossSourceCellFrame(628, 208, 240));
  const mirrorNotes = [
    'solid diagonals: northwest + southwest',
    'open floor crooks: northeast + southeast',
    'all N / E / S / W sockets remain exact',
    'plain mirror only — no seam filter and no stacked source art',
    'owner approval records this as one accepted derivation',
  ];
  for (const [index, note] of mirrorNotes.entries()) {
    parts.push(text(980, 218 + index * 52, `• ${note}`, 11, 720, index < 4 ? '#294B3C' : '#9A493D'));
  }
  parts.push(text(188, 500, '.## / ### / .##', 9, 820, MUTED, 'middle'));
  parts.push(text(748, 500, '##. / ### / ##.', 9, 820, '#B65F4D', 'middle'));
  ...
  for (const [layer, x, label] of sourceViews) {
    parts.push(await compositionWindow(
      options,
      [doubleFilledWestCrossJunctionCandidateCell(0, 0, layer)],
      1,
      1,
      x,
      688,
  ...
  parts.push(text(1036, 806, '40 PX · DARK', 9, 820, '#B65F4D', 'middle'));
  const sourceNotes = [
    'west slab stays one quiet cream field',
    'east reveal, coral, green, and shade stop at the open shoulder',
    'upper shade ends at y=97; base alone owns the lower shoulder',
    'no west fascia, internal rail, service tick, or duplicate belt',
    'one outline and one cream owner; no center peak, cap, or post',
  ];
  for (const [index, note] of sourceNotes.entries()) {
    parts.push(text(1120, 710 + index * 48, `• ${note}`, 10, 720, index < 4 ? '#294B3C' : MUTED));
  }
  parts.push(text(1120, 990, 'ACCEPTED DERIVATION · SOURCE MASK_25 · MIRROR-X', 10, 820, '#4E7D79'));

  parts.push(text(252, 1324, '90 PX/CELL · LIGHT', 10, 840, '#B65F4D', 'middle'));
  parts.push(await doubleFilledWestCrossJunctionMatrixWindow(
    options,
    gate.compactMatrix,
    117,
    1344,
  ...
  parts.push(installedMassFrame(117, 1344, 2, 3, 90));
  parts.push(crossSourceCellFrame(207, 1434, 90, 3));
  parts.push(text(564, 1324, '40 PX/CELL · DARK', 10, 840, '#4E7D79', 'middle'));
  parts.push(await doubleFilledWestCrossJunctionMatrixWindow(
    options,
    gate.compactMatrix,
    504,
  ...
  parts.push(installedMassFrame(504, 1344, 2, 3, 40));
  parts.push(crossSourceCellFrame(544, 1384, 40, 2));
  const compactNotes = [
    'teal brackets contain the accepted 2×3 west slab mass',
    'the east branch disappears flush into mask_43',
    'northeast and southeast crooks stay open at gameplay distance',
    'the west socket disappears into accepted mask_24',
    'the south-facing shoulder shade remains local instead of crossing the slab',
  ];
  for (const [index, note] of compactNotes.entries()) {
    parts.push(text(760, 1342 + index * 54, `• ${note}`, 11, 740, index < 4 ? '#294B3C' : MUTED));
  }
  parts.push(text(117, 1668, 'TEAL · FULL 2×3 SLAB · 20/26 · 24/43 · 16/34', 9, 820, '#4E7D79'));
  parts.push(text(117, 1698, 'CORAL · MASK_43 ACCEPTED CELL', 9, 820, '#B65F4D'));

  parts.push(text(190, 1840, '3-CELL ARMS · 40 PX/CELL · LIGHT', 10, 840, '#B65F4D', 'middle'));
  parts.push(await doubleFilledWestCrossJunctionMatrixWindow(
    options,
    gate.threeCellExtentMatrix,
    50,
  ...
  parts.push(crossSourceCellFrame(170, 1980, 40, 2));
  parts.push(text(616, 1840, '6-CELL ARMS · 40 PX/CELL · LIGHT', 10, 840, '#4E7D79', 'middle'));
  parts.push(await doubleFilledWestCrossJunctionMatrixWindow(
    options,
    gate.sixCellExtentMatrix,
    356,
  ...
  parts.push(crossSourceCellFrame(596, 2100, 40, 2));
  parts.push(text(1254, 1840, '6-CELL ARMS · 40 PX/CELL · DARK', 10, 840, '#B65F4D', 'middle'));
  parts.push(await doubleFilledWestCrossJunctionMatrixWindow(
    options,
    gate.sixCellExtentMatrix,
    994,
  ...
  parts.push(crossSourceCellFrame(1234, 2100, 40, 2));
  parts.push(text(190, 2200, 'CENTER ROW · 24 / 43 / 10… / 8', 10, 820, '#B65F4D', 'middle'));
  parts.push(text(190, 2240, 'VERTICAL SLAB · 20/26 → 24/42 → 16/34', 10, 820, '#4E7D79', 'middle'));
  parts.push(text(190, 2280, 'No long-run camouflage.', 10, 820, '#9A493D', 'middle'));
  parts.push(text(950, 2408, 'THE CENTER REMAINS ONE MIRRORED UNION AT EVERY EXTENT; MASK_25 STAYS THE ACCEPTED SOURCE.', 10, 840, '#9A493D', 'middle'));

  const reviewLines = [
    'Judge one east branch entering an accepted 2×N west slab',
    'Keep exact N/E/S/W sockets and exactly two east floor crooks',
    'Demand one quiet west cream field with east-only exposed frontage',
    'Reject west fascia, doubled belts, center seams, peaks, posts, caps, or gaps',
    'Approval would add one derivation; it would not add authored geometry',
  ];
  for (const [index, line] of reviewLines.entries()) {
    parts.push(text(44, 2552 + index * 52, `• ${line}`, 10, 720, index < 3 ? '#294B3C' : MUTED));
  }
  const boundaryLines: ReadonlyArray<readonly [string, string]> = [
  ...
  for (const [index, [line, color]] of boundaryLines.entries()) {
    parts.push(text(824, 2552 + index * 52, line, 10, index === 0 || index >= 2 ? 820 : 700, color));
  }
...
async function renderEqualHeightDoubleFilledEastCrossJunctionProposal(
  options: CliOptions,
  root: string,
): Promise<void> {
  const gate = EQUAL_HEIGHT_DOUBLE_FILLED_EAST_CROSS_JUNCTION_GATE;
  const width = 1600;
  const height = 2850;
  const panelFill = '#ECE5D5';
  const panel = (x: number, y: number, w: number, h: number): string =>
  ...
  const parts: string[] = [
    ...
    text(44, 150, 'mask_19 and mask_23 explain the two filled east crooks. mask_24 explains the quiet slab plane. mask_25 must be one new union.', 10, 650, MUTED),

    panel(20, 582, 1560, 620),
    text(44, 616, 'SOURCE READ — BASE / UPPER / COMPOSED AT 240 / 90 / 40 PX', 14, 820),
    text(44, 640, 'The west branch enters one continuous east cream field. Only the two west floor crooks retain exposed tri-tone frontage.', 10, 650, MUTED),

    panel(20, 1222, 1560, 500),
    text(44, 1256, 'COMPACT INSTALLED TRUTH — WEST BRANCH + 2×3 STRUCTURAL SLAB', 14, 820),
    text(44, 1280, 'The literal .## / ### / .## occupancy must resolve without an internal center seam, buried east fascia, or lost west crook.', 10, 650, MUTED),

    panel(20, 1742, 1560, 710),
    text(44, 1776, 'LENGTH GATE — ACCEPTED 24 / 42 REPEATS ABOVE AND BELOW MASK_25', 14, 820),
    text(44, 1800, 'Three- and six-cell slab extents keep mask_25 unchanged. The separately accepted mask_43 mirrors this union for the west-filled companion.', 10, 650, MUTED),

    panel(20, 2472, 1560, 354),
    text(44, 2508, 'ACCEPTANCE CONTRACT', 14, 820),
    text(824, 2508, 'PROOF BOUNDARY', 14, 820),
  ];

  const ladder: ReadonlyArray<readonly [CompositionCell, number, string, string]> = [
    [openPocketCrossJunctionCandidateCell(0, 0), 64, 'MASK_15', 'FOUR CROOKS OPEN'],
    [singleFilledCrossJunctionCandidateCell(0, 0), 396, 'MASK_19', 'NE FILLED'],
    [singleFilledSoutheastCrossJunctionCandidateCell(0, 0), 728, 'MASK_23', 'SE FILLED'],
    [doubleFilledEastCrossJunctionCandidateCell(0, 0), 1264, 'MASK_25', 'NE + SE FILLED'],
  ];
  for (const [cell, x, label, subtitle] of ladder) {
    const target = label === 'MASK_25';
    parts.push(text(x + 100, 188, `${label} · ACCEPTED`, 9, 840, '#4E7D79', 'middle'));
    parts.push(await compositionWindow(options, [cell], 1, 1, x, 208, 200, 200, fileOverrides));
    if (target) parts.push(crossSourceCellFrame(x, 208, 200));
    parts.push(text(x + 100, 432, subtitle, 9, 800, target ? '#B65F4D' : MUTED, 'middle'));
  }
  parts.push(text(316, 310, '→', 34, 760, '#B65F4D', 'middle'));
  parts.push(text(650, 310, '+', 28, 760, '#B65F4D', 'middle'));
  parts.push(text(1062, 310, '→ ONE UNION', 18, 780, '#B65F4D', 'middle'));
  parts.push(text(800, 510, 'DO NOT OVERLAY MASK_19 + MASK_23 · PRESERVE MASK_24’S SEAM-FREE EAST SLAB', 10, 840, '#294B3C', 'middle'));
  ...
  for (const [layer, x, label] of sourceViews) {
    parts.push(await compositionWindow(
      options,
      [doubleFilledEastCrossJunctionCandidateCell(0, 0, layer)],
      1,
      1,
      x,
      688,
  ...
  parts.push(await compositionWindow(options, [doubleFilledEastCrossJunctionCandidateCell(0, 0)], 1, 1, 884, 714, 90, 90, fileOverrides));
  parts.push(await compositionWindow(options, [doubleFilledEastCrossJunctionCandidateCell(0, 0)], 1, 1, 1016, 739, 40, 40, fileOverrides, undefined, true, A1A_PALETTE.charcoal));
  parts.push(text(929, 824, '90 PX', 9, 820, '#B65F4D', 'middle'));
  parts.push(text(1036, 806, '40 PX · DARK', 9, 820, '#B65F4D', 'middle'));
  const sourceNotes = [
    'east half matches the accepted mask_24 seam-free cream slab',
    'west reveal, coral, green, and shade stop at the filled shoulder',
    'northwest and southwest remain genuine floor crooks',
    'both east crooks contain no buried fascia, rail, or service tick',
    'one outline and one cream owner; no center peak, post, cap, or waist',
  ];
  for (const [index, note] of sourceNotes.entries()) {
    parts.push(text(1120, 710 + index * 48, `• ${note}`, 10, 720, index < 4 ? '#294B3C' : MUTED));
  }
  parts.push(text(1120, 990, 'ACCEPTED DIRECT SOURCE · LEDGER ROW MASK_25', 10, 820, '#294B3C'));

  parts.push(text(252, 1324, '90 PX/CELL · LIGHT', 10, 840, '#B65F4D', 'middle'));
  parts.push(await doubleFilledEastCrossJunctionMatrixWindow(options, gate.compactMatrix, 117, 1344, 270, 270, fileOverrides));
  parts.push(installedMassFrame(207, 1344, 2, 3, 90));
  parts.push(crossSourceCellFrame(207, 1434, 90, 3));
  parts.push(text(564, 1324, '40 PX/CELL · DARK', 10, 840, '#4E7D79', 'middle'));
  parts.push(await doubleFilledEastCrossJunctionMatrixWindow(options, gate.compactMatrix, 504, 1344, 120, 120, fileOverrides, A1A_PALETTE.charcoal));
  parts.push(installedMassFrame(544, 1344, 2, 3, 40));
  parts.push(crossSourceCellFrame(544, 1384, 40, 2));
  const compactNotes = [
    'teal brackets contain the accepted 2×3 slab mass',
    'the west branch disappears flush into mask_25',
    'the two west crooks remain open at gameplay distance',
    'the east socket disappears into accepted mask_42',
    'south-facing shade ends at the southwest crook instead of crossing the slab',
  ];
  for (const [index, note] of compactNotes.entries()) {
    parts.push(text(760, 1342 + index * 54, `• ${note}`, 11, 740, index < 4 ? '#294B3C' : MUTED));
  }
  parts.push(text(117, 1668, 'TEAL · FULL 2×3 SLAB · 20/26 · 25/42 · 16/34', 9, 820, '#4E7D79'));
  parts.push(text(117, 1698, 'CORAL · MASK_25 ACCEPTED SOURCE CELL', 9, 820, '#B65F4D'));

  parts.push(text(190, 1840, '3-CELL SLAB · 40 PX/CELL · LIGHT', 10, 840, '#B65F4D', 'middle'));
  parts.push(await doubleFilledEastCrossJunctionMatrixWindow(options, gate.threeCellExtentMatrix, 50, 1860, 280, 280, fileOverrides));
  parts.push(crossSourceCellFrame(170, 1980, 40, 2));
  parts.push(text(616, 1840, '6-CELL SLAB · 40 PX/CELL · LIGHT', 10, 840, '#4E7D79', 'middle'));
  parts.push(await doubleFilledEastCrossJunctionMatrixWindow(options, gate.sixCellExtentMatrix, 356, 1860, 520, 520, fileOverrides));
  parts.push(crossSourceCellFrame(596, 2100, 40, 2));
  parts.push(text(1254, 1840, '6-CELL SLAB · 40 PX/CELL · DARK', 10, 840, '#B65F4D', 'middle'));
  parts.push(await doubleFilledEastCrossJunctionMatrixWindow(options, gate.sixCellExtentMatrix, 994, 1860, 520, 520, fileOverrides, A1A_PALETTE.charcoal));
  parts.push(crossSourceCellFrame(1234, 2100, 40, 2));
  parts.push(text(190, 2200, 'CENTER ROW · 2 / 10… / 25 / 42', 10, 820, '#B65F4D', 'middle'));
  parts.push(text(190, 2240, 'VERTICAL SLAB · 20/26 → 24/42 → 16/34', 10, 820, '#4E7D79', 'middle'));
  parts.push(text(190, 2280, 'No long-run camouflage.', 10, 820, '#9A493D', 'middle'));
  parts.push(text(950, 2408, 'EASTWARD HORIZONTAL CONTINUATION IS NOT SHOWN HERE: ACCEPTED MASK_43 OWNS THAT MIRRORED COMPANION STATE.', 10, 840, '#9A493D', 'middle'));

  const acceptanceLines = [
    'Judge one west branch entering an accepted 2×N slab',
    'Keep exact N/E/S/W topology sockets and exactly two west floor crooks',
    'Demand one quiet east cream field with west-only exposed tri-tone frontage',
    'Reject stacked controls, east fascia, center seams, peaks, posts, caps, or gaps',
    'Keep mask_43 acceptance independent from this mask_25 direct-source proof',
  ];
  for (const [index, line] of acceptanceLines.entries()) {
    parts.push(text(44, 2552 + index * 42, `• ${line}`, 10, 720, index < 3 ? '#294B3C' : MUTED));
  }
  const boundaryLines: ReadonlyArray<readonly [string, string]> = [
  ...
  for (const [index, [line, color]] of boundaryLines.entries()) {
    parts.push(text(824, 2552 + index * 42, line, 10, index === 0 || index >= 2 ? 820 : 700, color));
  }
...
async function renderEqualHeightSingleFilledSoutheastCrossJunctionProposal(
  options: CliOptions,
  root: string,
): Promise<void> {
  const directGate =
...
export async function renderEqualHeightSingleFilledSoutheastCrossJunctionAcceptedArchive(
  ...
  const parts: string[] = [
    ...
    text(44, 150, 'Keep all four cardinal sockets exact. Fill only the southeast crook; northeast, southwest, and northwest remain visible floor.', 10, 650, MUTED),

    panel(20, 582, 1560, 640),
    text(44, 616, 'SOURCE READ — BASE / UPPER / COMPOSED AT 240 / 90 / 40 PX', 14, 820),
    text(44, 640, 'The southeast closure must become one continuous cream mass without letting buried fascia bleed across its top.', 10, 650, MUTED),

    panel(20, 1242, 1560, 430),
    text(44, 1276, 'CONSTRUCTION CONTROLS — RELATED T STATES ARE REFERENCES, NOT OVERLAYS', 14, 820),
    text(44, 1300, 'mask_21 and mask_22 define the same SE closure at different three-way sockets. mask_23 must remain one authored four-way union.', 10, 650, MUTED),

    panel(20, 1692, 1560, 500),
    text(44, 1726, 'COMPACT CROSSING — ONE-CELL ARMS AT 90 / 40 PX PER CELL', 14, 820),
    text(44, 1750, 'The southeast fill and three open floor crooks must survive both grounds while the south-facing wrap keeps one clear owner.', 10, 650, MUTED),

    panel(20, 2212, 1560, 660),
    text(44, 2246, 'EXTENT GATE — THREE-CELL AND SIX-CELL ARMS ON LIGHT / DARK FLOOR', 14, 820),
    text(44, 2270, 'The center stays the same size as the arms grow. Long runs may not hide a bad wrap, disconnected cream field, or buried face.', 10, 650, MUTED),
  ...
  parts.push(text(1274, 190, 'MASK_23 · ACCEPTED · SE CROOK FILLED', 10, 840, '#4E7D79', 'middle'));
  parts.push(await compositionWindow(
    options,
    [singleFilledSoutheastCrossJunctionCandidateCell(0, 0)],
    1,
  ...
  for (const [layer, x, label] of sourceViews) {
    parts.push(await compositionWindow(
      options,
      [singleFilledSoutheastCrossJunctionCandidateCell(0, 0, layer)],
      1,
      1,
      x,
      688,
  ...
  parts.push(await compositionWindow(options, [singleFilledSoutheastCrossJunctionCandidateCell(0, 0)], 1, 1, 884, 714, 90, 90, fileOverrides));
  parts.push(await compositionWindow(options, [singleFilledSoutheastCrossJunctionCandidateCell(0, 0)], 1, 1, 1016, 739, 40, 40, fileOverrides, undefined, true, A1A_PALETTE.charcoal));
  parts.push(text(929, 824, '90 PX', 9, 820, '#B65F4D', 'middle'));
  parts.push(text(1036, 806, '40 PX · DARK', 9, 820, '#B65F4D', 'middle'));
  const sourceNotes = [
    'SE cream field reaches the exact east and south sockets',
    'north and west sockets retain the accepted open-hub register',
    'NE / SW / NW remain genuine floor crooks',
    'south-facing coral and green stop before buried SE fascia',
    'no cap, badge, patch, or hidden interior rail',
  ];
  for (const [index, note] of sourceNotes.entries()) {
    parts.push(text(1120, 712 + index * 48, `• ${note}`, 10, 720, index < 4 ? '#294B3C' : MUTED));
  }
  parts.push(text(1120, 990, 'ACCEPTED PROOF SOURCE · MASK_23 DIRECT LEDGER PROVENANCE', 10, 820, '#294B3C'));

  const constructionControls: ReadonlyArray<readonly [CompositionCell, number, string]> = [
    [westPartialTJunctionCandidateCell(21, 0, 0), 60, 'MASK_21 · N/E/S · SE FILLED'],
    [horizontalPartialTJunctionCandidateCell(22, 0, 0), 360, 'MASK_22 · E/S/W · SE FILLED'],
    [singleFilledSoutheastCrossJunctionCandidateCell(0, 0), 1280, 'MASK_23 · N/E/S/W · SE FILLED'],
  ];
  ...
  parts.push(text(690, 1545, 'REJECT · EAST TOP BLEEDING ACROSS SOUTH FRONT', 10, 840, '#9A493D'));
  parts.push(text(690, 1580, 'REJECT · DOUBLE CORAL / GREEN BANDS', 10, 840, '#9A493D'));
  parts.push(text(690, 1615, 'PASS · CONTINUOUS SE TOP / THREE OPEN FLOOR CROOKS', 10, 820, '#294B3C'));

  parts.push(text(246, 1792, '90 PX/CELL · LIGHT · FIXED-LIGHT SOURCE REGISTER', 10, 840, '#B65F4D', 'middle'));
  parts.push(await singleFilledSoutheastCrossJunctionMatrixWindow(options, gate.compactMatrix, 111, 1812, 270, 270, fileOverrides));
  parts.push(crossInstalledUnionFrame(201, 1902, 90));
  parts.push(crossSourceCellFrame(201, 1902, 90, 3));
  parts.push(text(558, 1792, '40 PX/CELL · DARK · SAME SOURCE REGISTER', 10, 840, '#4E7D79', 'middle'));
  parts.push(await singleFilledSoutheastCrossJunctionMatrixWindow(options, gate.compactMatrix, 498, 1812, 120, 120, fileOverrides, A1A_PALETTE.charcoal));
  parts.push(crossInstalledUnionFrame(538, 1852, 40));
  parts.push(crossSourceCellFrame(538, 1852, 40, 2));
  const compactNotes = [
    'four cardinal sockets disappear into accepted neighbors',
    'the southeast fill remains one continuous wall mass',
    'three open crooks remain readable at gameplay distance',
    'south frontage wraps once and never becomes a buried rail',
  ];
  for (const [index, note] of compactNotes.entries()) {
    parts.push(text(760, 1836 + index * 58, `• ${note}`, 11, 740, index < 3 ? '#294B3C' : MUTED));
  }
  parts.push(text(760, 2088, 'Reject a center bulge, perspective switch, cream gap, doubled seam, or lost floor crook.', 11, 820, '#9A493D'));
  parts.push(text(111, 2126, 'TEAL BRACKETS · FULL 2×2 INSTALLED UNION · 23 / 26 / 16 / 34', 9, 820, '#4E7D79'));
  parts.push(text(111, 2160, 'CORAL CELL · MASK_23 SOURCE', 9, 820, '#B65F4D'));

  parts.push(text(190, 2310, '3-CELL ARMS · 40 PX/CELL · LIGHT', 10, 840, '#B65F4D', 'middle'));
  parts.push(await singleFilledSoutheastCrossJunctionMatrixWindow(options, gate.threeCellArmMatrix, 50, 2330, 280, 280, fileOverrides));
  parts.push(crossInstalledUnionFrame(170, 2450, 40));
  parts.push(crossSourceCellFrame(170, 2450, 40, 2));
  parts.push(text(616, 2310, '6-CELL ARMS · 40 PX/CELL · LIGHT · FIXED-LIGHT REGISTER', 10, 840, '#4E7D79', 'middle'));
  parts.push(await singleFilledSoutheastCrossJunctionMatrixWindow(options, gate.sixCellArmMatrix, 356, 2330, 520, 520, fileOverrides));
  parts.push(crossInstalledUnionFrame(596, 2570, 40));
  parts.push(crossSourceCellFrame(596, 2570, 40, 2));
  parts.push(text(1254, 2310, '6-CELL ARMS · 40 PX/CELL · DARK · SAME REGISTER', 10, 840, '#B65F4D', 'middle'));
  parts.push(await singleFilledSoutheastCrossJunctionMatrixWindow(options, gate.sixCellArmMatrix, 994, 2330, 520, 520, fileOverrides, A1A_PALETTE.charcoal));
  parts.push(crossInstalledUnionFrame(1234, 2570, 40));
  parts.push(crossSourceCellFrame(1234, 2570, 40, 2));
  parts.push(text(190, 2660, 'TEAL BRACKETS · INSTALLED 2×2 UNION', 10, 820, '#4E7D79', 'middle'));
  parts.push(text(190, 2700, '23 / 28 / 17 / 34 · CORAL = MASK_23', 10, 820, '#B65F4D', 'middle'));
  parts.push(text(190, 2740, 'No long-run camouflage.', 10, 820, '#9A493D', 'middle'));

  const acceptanceLines = [
    'Judge one occupancy step from accepted open mask_15',
    'Keep exact N/E/S/W sockets and exactly three open floor crooks',
    'Demand one continuous southeast cream field with no buried fascia',
    'Reject east-top bleed, doubled bands, central peaks, posts, or patches',
  ];
  for (const [index, line] of acceptanceLines.entries()) {
...
async function renderEqualHeightSingleFilledCrossJunctionGate(
  ...
  const targetUpper: readonly CompositionCell[] = [
    singleFilledCrossJunctionCandidateCell(0, 0, 'upper'),
  ];
  const controlUpper: readonly CompositionCell[] = [
    doubleFilledNorthCrossJunctionCandidateCell(0, 0, 'upper'),
  ];
  const parts: string[] = [
    `<rect width="${width}" height="${height}" rx="16" fill="${PANEL}"/>`,
    text(22, 31, 'MASK 19 — ACCEPTED SHARED TURN', 19, 840),
    text(
      22,
      55,
      'Accepted proof checkpoint · cream, arris, shade, and seam curve together; one combined coral/green return closes behind',
      10,
      650,
      MUTED,
    ),
    ...
    panel(18, 72, 964, 354),
    text(156, 99, 'MASK 19 ACCEPTED · UPPER · 240 PX', 10, 840, '#294B3C', 'middle'),
    text(470, 99, 'MASK 39 ACCEPTED TURN · 240 PX', 10, 840, '#4E7D79', 'middle'),
    text(825, 99, 'INSTALLED MASK 19', 10, 840, '#294B3C', 'middle'),

    panel(18, 440, 964, 296),
    text(500, 461, '3-CELL EXTENTS', 10, 840, MUTED, 'middle'),
    text(500, 477, 'LITERAL 40 PX/CELL', 8, 760, MUTED, 'middle'),
  ];
  ...
  parts.push(text(
    156,
    385,
    'ONE CURVED HANDOFF · NO DUPLICATE BELTS',
    8,
    840,
    '#B65F4D',
    'middle',
  ...
  parts.push(
    '<rect x="513" y="279" width="65" height="58" rx="5" fill="none" ' +
      'stroke="#4E7D79" stroke-width="2.5" stroke-dasharray="6 4"/>',
  );
  parts.push(text(
    470,
    385,
    'ACCEPTED CREAM / ARRIS / SHADE / SEAM PHASE',
    8,
    820,
    '#4E7D79',
    'middle',
  ...
  parts.push(await singleFilledCrossJunctionMatrixWindow(
    options,
    gate.compactMatrix,
    690,
    105,
  ...
  parts.push(text(
    825,
    417,
    'NE FILLED · SE / SW / NW FLOOR CROOKS OPEN',
    8,
    760,
    MUTED,
    'middle',
  ...
  const extentViews = [
    [36, A1A_PALETTE.floor, 'MASK 19 · 3-CELL · LIGHT'],
    [664, A1A_PALETTE.charcoal, 'MASK 19 · 3-CELL · DARK'],
  ] as const;
  for (const [x, floorFill, label] of extentViews) {
    parts.push(await singleFilledCrossJunctionMatrixWindow(
      options,
  ...
  parts.push(text(
    22,
    758,
    'ACCEPTED: ONE PARALLEL CREAM / ARRIS / SHADE / SEAM TURN',
    8,
    840,
    '#294B3C',
  ));
  parts.push(text(
    978,
    758,
    'SOCKETS FROZEN · LEDGER 28 / 19 / 0 / 0 · NO EXPORT / UNITY CHANGE',
    8,
    820,
    '#9A493D',
    'end',
...
/** Explicit-only legacy Mask 19 archive renderer; ordinary paths use the compact accepted gate. */
export async function renderEqualHeightSingleFilledCrossJunctionAcceptedArchive(
  options: CliOptions,
  root: string,
): Promise<void> {
  const gate = EQUAL_HEIGHT_SINGLE_FILLED_CROSS_JUNCTION_GATE;
...
async function renderEqualHeightOpenPocketCrossJunctionGate(
  ...
  const boundaryLines: ReadonlyArray<readonly [string, string]> = [
    ...
    ['accepted ledger · 28 direct / 19 derived / 0 synthetic', MUTED],
    ['all cross-junction rows have accepted proof provenance', MUTED],
    ['2 EXTERNAL PROOF SVGs · TEMPORARY FRAMES', '#4E7D79'],
    ['NO CANONICAL / EXPORT / ATLAS / SCHEMA / BLOB / UNITY', '#9A493D'],
  ];
  for (const [index, [line, color]] of boundaryLines.entries()) {
...
async function renderEqualHeightHorizontalPartialTJunctionGate(
  ...
  const directUpper: readonly CompositionCell[] = [
    horizontalPartialTJunctionCandidateCell(22, 0, 0, 'upper'),
  ];
  const controlUpper: readonly CompositionCell[] = [
    singleFilledSoutheastCrossJunctionCandidateCell(0, 0, 'upper'),
  ];
  const parts: string[] = [
    `<rect width="${width}" height="${height}" rx="16" fill="${PANEL}"/>`,
    text(22, 31, 'MASKS 22 / 28 — SHARED REVEAL ACCEPTED', 19, 840),
    text(22, 55, 'The cream ledge keeps one exposure across the direct open-to-solid turn and its plain whole-cell X mirror.', 10, 650, MUTED),
    text(978, 31, 'OWNER ACCEPTED', 9, 840, '#294B3C', 'end'),

    panel(18, 72, 964, 354),
    text(156, 99, 'MASK 22 DIRECT · UPPER · 240 PX', 10, 840, '#294B3C', 'middle'),
    text(470, 99, 'MASK 23 CONTROL · UPPER · 240 PX', 10, 840, '#4E7D79', 'middle'),
    text(825, 99, 'INSTALLED MASK 22', 10, 840, '#294B3C', 'middle'),

    panel(18, 440, 964, 296),
    text(36, 465, 'ACCEPTED CONFIRMATION — DIRECT MASK 22 + PLAIN-MIRROR MASK 28 AT 40 PX/CELL', 10, 840, MUTED),
  ];
  ...
  parts.push('<rect x="34" y="226" width="244" height="16" rx="4" fill="none" stroke="#B65F4D" stroke-width="2.5" stroke-dasharray="6 4"/>');
  parts.push(text(156, 385, 'ONE EXPOSURE · OWNER SPLIT AT X=58', 8, 840, '#B65F4D', 'middle'));
  ...
  parts.push('<rect x="348" y="226" width="244" height="16" rx="4" fill="none" stroke="#4E7D79" stroke-width="2.5" stroke-dasharray="6 4"/>');
  parts.push(text(470, 385, 'MATCH THE ACCEPTED REVEAL PHASE', 8, 820, '#4E7D79', 'middle'));

  parts.push(await horizontalPartialTJunctionMatrixWindow(
    options,
    gate.compactMatrices.mask22,
    22,
    690,
    105,
    270,
  ...
  parts.push(westPartialTJunctionCandidateFrame(780, 195, 90, '#B65F4D'));
  parts.push(text(825, 400, 'COMPACT OCCUPANCY · 90 PX/CELL', 8, 760, MUTED, 'middle'));
  parts.push(text(825, 417, 'NO BRIGHT STEP ACROSS THE CREAM LEDGE', 8, 760, MUTED, 'middle'));

  const longRuns = [
    [
      50,
      gate.longMatrices.mask22,
      22,
      'MASK 22 · DIRECT · LIGHT',
      A1A_PALETTE.floor,
      4,
    ],
    [
      550,
      gate.longMatrices.mask28,
      28,
      'MASK 28 · PLAIN X · DARK',
      A1A_PALETTE.charcoal,
      5,
    ],
  ] as const;
  for (const [x, matrix, maskIndex, label, floorFill, candidateCol] of longRuns) {
    parts.push(text(
      x + 200,
      488,
      label,
  ...
  parts.push(text(22, 748, 'ACCEPTED: ONE SHARED CREAM-REVEAL EXPOSURE', 8, 840, '#294B3C'));
  parts.push(text(978, 748, 'PROOF ONLY · LEDGER 28 / 19 / 0 / 0 · NO EXPORT / UNITY CHANGE', 8, 820, '#9A493D', 'end'));
...
async function renderEqualHeightVerticalTerminusGate(
  ...
  const parts: string[] = [
    ...
    text(1976, 62, '28 direct · 19 derived · 0 synthetic · 0 unresolved', 10, 700, MUTED, 'end'),
  ];
...
async function renderEqualHeightMaskConsistencyReview(
  ...
  const parts: string[] = [
    ...
    text(
      margin,
      76,
      '112 SOURCE FOOTPRINT ACCEPTED · CONSISTENCY REVIEW PAUSED · 47 topology rows · 50 visual presentations · ledger frozen at 28 direct / 19 derived / 0 synthetic / 0 unresolved',
      15,
      720,
      A1A_PALETTE.green,
    ),
    ...
    text(
      width - 790,
      80,
      '112 source footprint accepted; ledger, atlas, exporter, schema, blob, and Unity remain unchanged.',
      11,
      700,
      '#9FC7A9',
    ),
    text(
      width - 790,
      101,
      'Any later family-coherence correction remains a review proposal until owner approval.',
      11,
      650,
      '#83A9A6',
    ),
  ...
  parts.push(
    ...
    text(
      width - 34,
      sectionY + 170,
      'PAUSED CONSISTENCY REVIEW · NO FURTHER PROMOTION IMPLIED',
      12,
      850,
      '#83A9A6',
      'end',
...
function equalHeightMaskOperationLabel(entry: EqualHeightMaskLedgerEntry): string {
  ...
  if (entry.resolution.kind === 'approved-derivation') {
    if (entry.resolution.variants.some(({ derivation }) =>
      derivation ===
        'accepted-opposite-diagonal-boundary-seam-filter')) {
      return 'mirror-X + boundary filter';
    }
    if (entry.resolution.variants.some(({ derivation }) =>
...
async function renderEqualHeightMaskLedger(options: CliOptions, root: string): Promise<void> {
  ...
  parts.push(text(926, 130, 'All proof rows accepted · no canonical or production mutation', 10, 700, MUTED));
  ...
  const resolvedLines = [
    ...
    'T hubs · 7 / 13 direct / filtered · 11 / 14 direct pair',
    'cross hubs · 15 / 19 / 23 / 25 / 30 / 32 / 37 / 39 / 41 direct · 40 filtered mirror',
  ...
  parts.push(text(railX + 24, gridY + 1420, 'mask_15 / 19 / 23 / 25 / 30 / 32 / 37 / 39 / 41 / 46 are accepted direct cross hubs.', 11, 650, MUTED));
  parts.push(text(railX + 24, gridY + 1442, 'mask_40 is the accepted filtered mirror-X derivation of mask_30.', 11, 650, MUTED));

  parts.push(text(railX + 24, gridY + 1474, 'PROOF VOCABULARY COMPLETE', 14, 850, A1A_PALETTE.green));
  parts.push(text(railX + 24, gridY + 1502, '0 solid elbows · all four have accepted proof sources', 11, 700, MUTED));
  parts.push(text(railX + 24, gridY + 1524, '0 T cases · every open and single-filled-pocket T row is accepted', 11, 700, MUTED));
  parts.push(text(railX + 24, gridY + 1546, '0 cross cases · mask_46 closes the final buried-center topology', 11, 700, MUTED));
  parts.push(text(railX + 24, gridY + 1574, 'All topology previews now trace accepted direct or derived proof provenance.', 11, 750, A1A_PALETTE.green));

  parts.push(text(railX + 24, gridY + 1598, 'OUT OF SCOPE', 14, 850));
  const outOfScope = [
  ...
  for (const [index, line] of outOfScope.entries()) {
    parts.push(text(railX + 24, gridY + 1622 + index * 20, `• ${line}`, 11, 650, MUTED));
  }
  ...
  parts.push(text(railX + 42, gridY + 1888, '✓ 47 / 47 rows have accepted direct or derived proof-layer provenance', 11, 750, '#294B3C'));
...
async function renderFullHeightSouthProof(options: CliOptions): Promise<void> {
  ...
  const parts: string[] = [
    ...
    text(564, 132, 'Exact north source, exact 112-unit envelope, exact contact baseline.', 11, 600, MUTED),
...
async function renderContextMocksSafely(options: CliOptions, root: string): Promise<void> {
  try {
    ...
    await renderEqualHeightDoubleFilledSouthCrossJunctionGate(
      options,
      root,
    );
    await renderEqualHeightDoubleFilledOppositeDiagonalCrossJunctionProposal(
    ...
    status.fullyFilledCrossJunctionRenderedAt = renderedAt;
    status.singleOpenNorthwestCrossJunctionRenderedAt = renderedAt;
    status.singleOpenNortheastCrossJunctionRenderedAt = renderedAt;
    status.singleOpenSoutheastCrossJunctionRenderedAt = renderedAt;
    status.singleOpenSouthwestCrossJunctionRenderedAt = renderedAt;
    status.doubleFilledSouthCrossJunctionRenderedAt = renderedAt;
    status.doubleFilledOppositeDiagonalCrossJunctionRenderedAt = renderedAt;
    status.doubleFilledDiagonalCrossJunctionRenderedAt = renderedAt;
    ...
    status.singleFilledSoutheastCrossJunctionRenderedAt = renderedAt;
  ...
  if (existsSync(proofsDirectory)) {
    watch(proofsDirectory, { recursive: true }, (_event, fileName) => {
      ...
      if (
        ...
        normalizedProofFile.startsWith('fully-filled-cross-junction/') ||
        normalizedProofFile.startsWith('single-open-northwest-cross-junction/') ||
        normalizedProofFile.startsWith('single-open-southwest-cross-junction/') ||
        normalizedProofFile.startsWith('double-filled-south-cross-junction/') ||
        normalizedProofFile.startsWith('double-filled-diagonal-cross-junction/') ||
