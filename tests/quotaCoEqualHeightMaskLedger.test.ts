...
describe('QuotaCo owner-accepted proof-layer equal-height 47-mask ledger', () => {
  ...
  it('makes all forty-seven accepted source mappings and their facing provenance explicit', () => {
    ...
    expect(byIndex.get(33)?.resolution).toEqual({
      kind: 'direct-reuse',
      status: 'accepted-source-mapping',
      variants: [{
    ...
    expect(byIndex.get(41)?.resolution).toEqual({
      kind: 'direct-reuse',
      status: 'accepted-source-mapping',
      variants: [{
    ...
    expect(byIndex.get(45)?.resolution).toEqual({
      kind: 'approved-derivation',
      status: 'accepted-source-mapping',
      variants: [{
    ...
    expect(byIndex.get(46)?.resolution).toEqual({
      kind: 'direct-reuse',
      status: 'accepted-source-mapping',
      variants: [{
        ...
        transform: 'none',
        derivation: 'none',
        facingRule: 'connected north, east, south, and west with all four diagonal crooks solid; independently authored fully buried center with one structural underlay and one uninterrupted cream top while accepted perimeter rows own every exposed face cue',
      }],
      note: 'Accepted fully filled cross-junction center directly reuses one independently authored flattened buried-center pair; accepted masks 16/20/24/26/31/34/38/42 own its visible perimeter without becoming stacked source provenance.',
    });
  ...
  it('records a complete accepted mapping after closing every authored-source gap', () => {
  ...
  it('fails loudly on missing, duplicated, drifted, forbidden, or over-promoted rows', () => {
    ...
    const demotedFinalSource = clone() as any;
    demotedFinalSource.entries[46].resolution.status = 'proof-only-candidate';
    expect(() => validateEqualHeightMaskLedger(demotedFinalSource)).toThrow(
      /direct reuse has invalid status at mask_46/,
    );
